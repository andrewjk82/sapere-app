...
function buildGraphData(text) {
  ...
  const pointRegex = /(open|closed|solid)[^0-9-]*(-?\d+(?:\.\d+)?)/gi;
  let m;
...
async function run() {
  ...
  for (const doc of qs.docs) {
    ...
    const newOptions = data.options.map(opt => {
      ...
      if (!t.includes('circle') && !t.includes('arrow') && !t.includes('segment')) return opt;
      
      const gd = buildGraphData(opt.text);
      if (gd) {
        modified = true;
        // Clean out jsxGraph, set numberLine ONLY
        return {
          ...opt,
          graphData: gd
        };
  ...
  console.log(`Updated ${count} questions with clean numberLine graphData.`);
}
