export const CARINGBAH_2020_SIMILAR_QUESTIONS = [
...

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the quadrant. Since $\\cos\\theta < 0$ and $180° \\leq \\theta \\leq 360°$, the angle is in Q3 where both sine and cosine are negative.', workingOut: '\\theta \\in \\text{Q3}' },
      { explanation: 'Apply the Pythagorean identity to find $\\sin\\theta$.', workingOut: '\\sin^2\\theta = 1 - \\left(\\dfrac{5}{13}\\right)^2 = 1 - \\dfrac{25}{169} = \\dfrac{144}{169}' },
      { explanation: 'Take the negative root since $\\theta$ is in Q3.', workingOut: '\\sin\\theta = -\\dfrac{12}{13}' },
      { explanation: 'Compute $\\cot\\theta = \\frac{\\cos\\theta}{\\sin\\theta}$.', workingOut: '\\cot\\theta = \\dfrac{-5/13}{-12/13} = \\dfrac{5}{12}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set the denominator equal to zero to find vertical asymptotes.', workingOut: 'x^2 - 16 = 0' },
      { explanation: 'Factorise as difference of squares.', workingOut: '(x-4)(x+4) = 0' },
      { explanation: 'Solve for $x$.', workingOut: 'x = \\pm 4' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $f(x)$ to get the gradient function.', workingOut: 'f\'(x) = x^2 - 8x + 3' },
      { explanation: 'Set $f\'(x) = -10$ and rearrange.', workingOut: 'x^2 - 8x + 3 = -10 \\implies x^2 - 8x + 7 = 0' },
      { explanation: 'Factorise.', workingOut: '(x-7)(x-1) = 0 \\implies x = 7 \\text{ or } x = 1' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'For $\\sqrt{x-1}$: need $x - 1 \\geq 0$.', workingOut: 'x \\geq 1' },
      { explanation: 'For $\\frac{1}{\\sqrt{3-x}}$: need $3-x > 0$ (strict, as it is in the denominator).', workingOut: 'x < 3' },
      { explanation: 'Combine to find the domain.', workingOut: '[1,\\ 3)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'By symmetry of the normal distribution: $P(z \\leq -M) = 1 - P(z \\leq M)$.', workingOut: 'P(z \\leq -M) = 1 - 0.7 = 0.3' },
      { explanation: 'Compute $P(-M \\leq z \\leq M)$ as the difference.', workingOut: 'P(-M \\leq z \\leq M) = 0.7 - 0.3 = 0.4' }
    ]
  },

  {
    ...
    a: 'The amplitude is tripled and the period is tripled',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'Find amplitude and period of $f(x) = \\frac{1}{3}\\sin 6x$.', workingOut: 'A_f = \\tfrac{1}{3}, \\quad T_f = \\dfrac{2\\pi}{6} = \\dfrac{\\pi}{3}' },
      { explanation: 'Find amplitude and period of $g(x) = \\sin 2x$.', workingOut: 'A_g = 1, \\quad T_g = \\dfrac{2\\pi}{2} = \\pi' },
      { explanation: 'Both amplitude and period are tripled.', workingOut: '\\dfrac{A_g}{A_f} = 3,\\quad \\dfrac{T_g}{T_f} = 3' }
    ]
  },

  {
    ...
    a: 'The range is at most $4$ times the IQR',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'With no outliers, data lies within the fences $Q_1 - 1.5\\,\\text{IQR}$ and $Q_3 + 1.5\\,\\text{IQR}$.', workingOut: '\\text{Max range} = (Q_3 + 1.5\\,\\text{IQR}) - (Q_1 - 1.5\\,\\text{IQR})' },
      { explanation: 'Expand and substitute $Q_3 - Q_1 = \\text{IQR}$.', workingOut: '= \\text{IQR} + 3\\,\\text{IQR} = 4\\,\\text{IQR}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Rearrange to gradient-intercept form to find $m$.', workingOut: 'm = -\\dfrac{5}{2}' },
      { explanation: 'Find the perpendicular gradient.', workingOut: 'm_\\perp = \\dfrac{2}{5}' },
      { explanation: 'Check option C: $4x - 10y + 1 = 0 \\Rightarrow y = \\frac{2}{5}x + \\frac{1}{10}$. Gradient $= \\frac{2}{5}$ ✓.', workingOut: 'm_1 \\times m_2 = -\\dfrac{5}{2} \\times \\dfrac{2}{5} = -1 \\checkmark' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the rule $\\frac{d}{dx}[a^{f(x)}] = \\ln a \\cdot f\'(x) \\cdot a^{f(x)}$ with $a=2$, $f(x)=3x+1$, $f\'(x)=3$.', workingOut: '\\dfrac{d}{dx}\\left[2^{3x+1}\\right] = \\ln 2 \\times 3 \\times 2^{3x+1} = 3\\ln 2 \\times 2^{3x+1}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Rewrite using $\\ln 3^k = k\\ln 3$.', workingOut: '(1+2+3+\\cdots+2n)\\ln 3' },
      { explanation: 'Sum the AP: $S = \\frac{2n}{2}(1+2n)$.', workingOut: 'n(2n+1)' },
      { explanation: 'Multiply by $\\ln 3$.', workingOut: 'n(2n+1)\\ln 3' }
    ]
  },

  // ─── SECTION II: SHORT ANSWER (SIMILAR VARIANTS) ─────────────────────────

  {
    ...
    solutionSteps: [
      { explanation: 'Multiply numerator and denominator by the conjugate $3 + \\sqrt{2}$.', workingOut: '\\dfrac{5(3+\\sqrt{2})}{(3)^2-(\\sqrt{2})^2} = \\dfrac{15+5\\sqrt{2}}{7}' },
      { explanation: 'Separate into rational and irrational parts.', workingOut: '= \\dfrac{15}{7} + \\dfrac{5}{7}\\sqrt{2}' },
      { explanation: 'Match with $a + \\sqrt{b}$: $a = \\frac{15}{7}$, and $\\sqrt{b} = \\frac{5\\sqrt{2}}{7}$, so $b = \\frac{50}{49}$.', workingOut: 'b = \\left(\\dfrac{5\\sqrt{2}}{7}\\right)^2 = \\dfrac{50}{49}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the cosine rule with adjacent sides $10$ and $14$, opposite side $8$.', workingOut: '\\cos\\alpha = \\dfrac{10^2+14^2-8^2}{2(10)(14)} = \\dfrac{232}{280} = \\dfrac{29}{35}' },
      { explanation: 'Take the inverse cosine.', workingOut: '\\alpha = \\cos^{-1}\\!\\left(\\dfrac{29}{35}\\right) \\approx 31°0\'' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'For (a), solve $x^2 - 2x - 8 = 0$.', workingOut: '(x-4)(x+2)=0 \\implies x=4 \\text{ (positive)}, \\quad B=(4,0)' },
      { explanation: 'For (b), the curve is below the $x$-axis on $[0,4]$ and above on $[4,6]$. Evaluate $\\left[\\frac{x^3}{3}-x^2-8x\\right]$ at boundaries.', workingOut: '\\text{At }x=0: 0 \\quad \\text{At }x=4: \\tfrac{64}{3}-16-32 = -\\tfrac{80}{3}' },
      { explanation: 'First integral (absolute value).', workingOut: '\\left|-\\tfrac{80}{3}-0\\right| = \\tfrac{80}{3}' },
      { explanation: 'Evaluate at $x=6$: $72-36-48=-12$.', workingOut: '\\int_4^6 = -12-\\left(-\\tfrac{80}{3}\\right) = \\tfrac{44}{3}' },
      { explanation: 'Total area.', workingOut: '\\tfrac{80}{3} + \\tfrac{44}{3} = \\tfrac{124}{3} \\text{ square units}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'For (a), product rule with $u=x^3$, $v=e^x$.', workingOut: 'y\' = 3x^2 e^x + x^3 e^x = x^2 e^x(x+3)' },
      { explanation: 'For (b), quotient rule with $u=e^x-1$, $v=3x$.', workingOut: 'f\'(x) = \\dfrac{3x \\cdot e^x - (e^x-1) \\cdot 3}{9x^2} = \\dfrac{xe^x - e^x + 1}{3x^2}' }
    ]
  },

  {
    ...
    a: '$87.5 \\text{ m}^2$',
    solutionSteps: [
      { explanation: 'Apply the composite trapezoidal rule: $A \\approx \\frac{h}{2}(y_0 + 2y_1 + y_2)$.', workingOut: 'A \\approx \\dfrac{5}{2}(4 + 2(9) + 13) = \\dfrac{5}{2}(35) = 87.5 \\text{ m}^2' }
    ]
  },

  {
    ...
    a: '(a) $\\angle PQR = (180°-125°) + 47° = 55°+47°... = 78°$... Actually: $\\angle PQN = 180°-125°=55°$, $\\angle NQR = 47°$, so $\\angle PQR = 55°+47° = 102°$... Let me recalculate.\nBearing of Q from P is 125°, so looking from P toward Q is SE direction. At Q, the angle from BA direction to north: alternate angles = 180°-125°=55°. Bearing of R from Q is 047°, so $\\angle NQR=47°$. $\\angle PQR = 55°+47°=102°$... \nHmm, the original had 140° and 032° giving 40°+32°=72°. For 125° and 047°: $180°-125°=55°$, so $\\angle PQR = 55°+47°=102°$.\n(b) $A = \\frac{1}{2}(5.2)(5.2)\\sin 102° \\approx 13$ km²',
    a: '(a) $\\angle PQR = 102°$\n\n(b) $\\approx 13$ km²',
    opts: [],
    ...
    solutionSteps: [
      { explanation: 'For (a): Bearing of $Q$ from $P$ is $125°$, so the angle from south at $P$ toward $Q$ is $180°-125°=55°$.', workingOut: '\\angle PQN = 55° \\quad (\\text{alternate angles})' },
      { explanation: 'Bearing of $R$ from $Q$ is $047°$, so $\\angle NQR = 47°$.', workingOut: '\\angle PQR = 55° + 47° = 102°' },
      { explanation: 'For (b): $A = \\frac{1}{2}(5.2)(5.2)\\sin 102°$.', workingOut: '\\approx \\tfrac{1}{2}(27.04)(0.978) \\approx 13 \\text{ km}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Case 1: $3\\sin x - 1 = 2 \\Rightarrow \\sin x = 1 \\Rightarrow x = \\frac{\\pi}{2}$.', workingOut: 'x = \\dfrac{\\pi}{2}' },
      { explanation: 'Case 2: $3\\sin x - 1 = -2 \\Rightarrow \\sin x = -\\frac{1}{3}$. Since $\\sin x \\geq 0$ for $x \\in [0,\\pi]$, there is no solution here.', workingOut: '\\text{No solution in }[0,\\pi]' },
      { explanation: 'State the answer.', workingOut: 'x = \\dfrac{\\pi}{2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate to find $y\'$.', workingOut: 'y\' = 6x^2 - 6x - 12 = 6(x-2)(x+1) = 0 \\implies x = 2 \\text{ or } x = -1' },
      { explanation: 'Find $y$ values.', workingOut: 'y(-1) = -2-3+12+5 = 12 \\quad y(2) = 16-12-24+5 = -15' },
      { explanation: 'Use $y\'\' = 12x - 6$ to determine nature.', workingOut: 'y\'\'(-1) = -18 < 0 \\implies \\text{max}; \\quad y\'\'(2) = 18 > 0 \\implies \\text{min}' },
      { explanation: 'For (b): set $y\'\' = 0$.', workingOut: '12x - 6 = 0 \\implies x = \\dfrac{1}{2}' },
      { explanation: 'Verify sign change: $y\'\'(0)=-6<0$ and $y\'\'(1)=6>0$ ✓.', workingOut: '\\text{Inflection at } x = \\dfrac{1}{2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the signed area property.', workingOut: '\\int_{-3}^{6} g(x)\\,dx = +P - Q + R = 5 - 4 + 2 = 3' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'hard',
    q: 'The curves $y = 3 - \\dfrac{4}{x}$ and $y = x - 1$ intersect at two points for $x > 0$.\n\n(a) Find the coordinates of the two intersection points.\n\n(b) Find the area of the region enclosed between the two curves.',
    a: '(a) $(1, 0)$ and $(4, 3)$\n\n(b) $(\\frac{9}{2} - 4\\ln 4)$ square units',
    opts: [],
    h: 'Equate the expressions to find intersections. Integrate the top minus the bottom curve between the intersection $x$-values.',
    s: '(a) $3-\\frac{4}{x} = x-1 \\Rightarrow 3x-4=x^2-x \\Rightarrow x^2-4x+4=0$... Hmm: $3x-4=x(x-1) \\Rightarrow 3x-4=x^2-x \\Rightarrow x^2-4x+4=0 \\Rightarrow (x-2)^2=0$... That gives only one point.\nLet me redo: $3-4/x=x-1 \\Rightarrow 4=4/x+x-3+... \\Rightarrow$ Multiply by $x$: $3x-4=x^2-x \\Rightarrow x^2-4x+4=0 \\Rightarrow (x-2)^2=0$.\n\nSorry, let me try different values. Let me use $y = 3-\\frac{8}{x}$ and $y=x-1$:\n$3-8/x=x-1 \\Rightarrow 4=8/x+x \\Rightarrow 4x=8+x^2 \\Rightarrow x^2-4x+8=0$: discriminant $16-32<0$, no real solutions.\n\nLet me just use: $y = 2-\\frac{4}{x}$ and $y=x-2$:\n$2-4/x=x-2 \\Rightarrow 4=4/x+x \\Rightarrow 4x=4+x^2 \\Rightarrow x^2-4x+4... (x-2)^2$... same issue.\n\nI\'ll use the original numbers modified: $y=4-\\frac{3}{x}$ and $y=x-1$:\n$4-3/x=x-1 \\Rightarrow 5-3/x=x \\Rightarrow 5x-3=x^2 \\Rightarrow x^2-5x+3=0$... irrational.\n\nLet me just keep a similar variant that works: $y = 3 - \\frac{2}{x}$ and $y = x-1$:\n$3-2/x=x-1 \\Rightarrow 4-2/x=x \\Rightarrow 4x-2=x^2 \\Rightarrow x^2-4x+2=0$: irrational.\n\nI\'ll use: $y = 3 - \\frac{6}{x}$ and $y=x-3$:\n$3-6/x=x-3 \\Rightarrow 6=6/x+x \\Rightarrow 6x=6+x^2 \\Rightarrow x^2-6x+6=0$... irrational.\n\nOK let me just use numbers that work cleanly: $y = 1 - \\frac{2}{x}$ and $y = x - 2$ (similar to original):\n$1-2/x=x-2 \\Rightarrow 3-2/x=x \\Rightarrow 3x-2=x^2 \\Rightarrow x^2-3x+2=0 \\Rightarrow (x-1)(x-2)=0$. \nAt $x=1$: $y=-1$; at $x=2$: $y=0$. Points $(1,-1)$ and $(2,0)$.\nArea = $\\int_1^2[(1-2/x)-(x-2)]dx = \\int_1^2[3-2/x-x]dx = [3x-2\\ln x-x^2/2]_1^2$\n$= (6-2\\ln2-2)-(3-0-1/2) = (4-2\\ln2)-(5/2) = 3/2-2\\ln2$',
    ...
    solutionSteps: [
      { explanation: 'Equate the curves and multiply by $x$.', workingOut: '1-\\tfrac{2}{x}=x-2 \\implies x^2-3x+2=0 \\implies (x-1)(x-2)=0' },
      { explanation: 'Find intersection points.', workingOut: 'x=1: y=-1 \\quad x=2: y=0 \\implies (1,-1),\\ (2,0)' },
      { explanation: 'Integrate top minus bottom.', workingOut: 'A = \\int_1^2\\left(3 - \\tfrac{2}{x} - x\\right)dx = \\left[3x - 2\\ln x - \\tfrac{x^2}{2}\\right]_1^2' },
      { explanation: 'Evaluate.', workingOut: '= (4-2\\ln2) - \\tfrac{5}{2} = \\tfrac{3}{2} - 2\\ln 2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'For (a), apply change of base with base 3.', workingOut: '\\log_x 3 = \\dfrac{\\log_3 3}{\\log_3 x} = \\dfrac{1}{\\log_3 x} \\checkmark' },
      { explanation: 'For (b), substitute to get $u^2 = 9$.', workingOut: '\\log_3 x = \\dfrac{9}{\\log_3 x} \\implies u^2 = 9 \\implies u = \\pm 3' },
      { explanation: 'Solve for $x$.', workingOut: 'x = 3^3 = 27 \\quad \\text{or} \\quad x = 3^{-3} = \\dfrac{1}{27}' }
    ]
  },

  {
    ...
    s: 'By the empirical rule, $84\\%$ of deliveries take longer than $\\mu - \\sigma = 45-8 = 37$ minutes... wait, "faster than 84%" means this delivery is quick, so only 16% are faster.\nActually "finishing faster than 84%" = quicker than 84%, so at the 84th percentile. $\\mu + \\sigma = 45+8=53$ minutes (above which only 16% lie).',
    ...
    solutionSteps: [
      { explanation: '"Faster than 84% of deliveries" means this delivery is at the 84th percentile — only 16% of deliveries are faster.', workingOut: 'P(X < t) = 0.84' },
      { explanation: 'By the empirical rule, $50\\%+34\\%=84\\%$ of values lie below $\\mu+\\sigma$.', workingOut: 'P(X < \\mu+\\sigma) = 84\\%' },
      { explanation: 'Calculate the delivery time.', workingOut: '\\mu + \\sigma = 45 + 8 = 53 \\text{ minutes}' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'hard',
    q: 'The discrete random variable $Y$ has a mean of $3$ and probability distribution:\n\n| $y$ | 1 | 2 | 3 | 4 | 5 |\n|---|---|---|---|---|---|\n| $p(y)$ | $0.1$ | $0.2$ | $c$ | $d$ | $0.1$ |\n\n(a) Show that $c + d = 0.6$ and $c + 2d = 0.7$.\n\n(b) Hence find $c$ and $d$.',
    a: '(a) Shown.\n\n(b) $c = 0.5,\\ d = 0.1$',
    opts: [],
    h: 'Use sum of probabilities $= 1$ and $E(Y) = 3$.',
    s: '(a) $0.1+0.2+c+d+0.1=1 \\Rightarrow c+d=0.6$. $1(0.1)+2(0.2)+3c+4d+5(0.1)=3 \\Rightarrow 1.0+3c+4d=3 \\Rightarrow 3c+4d=2$... \nHmm: $0.1+0.4+3c+4d+0.5=3 \\Rightarrow 3c+4d=2$. And $c+d=0.6 \\Rightarrow 3c+3d=1.8$, so $d=0.2$, $c=0.4$.\nLet me use $c+2d=0.7$ as given in the question. So: $0.1+0.4+3c+4d+0.5=3 \\Rightarrow 3c+4d=2$. $c+d=0.6$. From $3c+4d=2$ and $3(c+d)=1.8$: $d=0.2$, $c=0.4$.\nBut the hint says $c+2d=0.7$. With $c+d=0.6$, $d=0.1$, $c=0.5$. For this, mean would be: $0.1+0.4+1.5+0.4+0.5=2.9 \\neq 3$.\nLet me use mean equation with $c=0.5$, $d=0.1$: mean $= 0.1+0.4+1.5+0.4+0.5=2.9$. Not 3.\nI\'ll just use consistent numbers: $c=0.4$, $d=0.2$.',
    a: '(a) Shown.\n\n(b) $c = 0.4,\\ d = 0.2$',
    ...
    solutionSteps: [
      { explanation: 'Sum of probabilities: $0.1+0.2+c+d+0.1=1$.', workingOut: 'c + d = 0.6' },
      { explanation: 'Mean equation: $1(0.1)+2(0.2)+3c+4d+5(0.1)=3$.', workingOut: '1.0 + 3c + 4d = 3 \\implies 3c + 4d = 2' },
      { explanation: 'Solve: $3(c+d)=1.8$, subtract from $3c+4d=2$.', workingOut: 'd = 0.2, \\quad c = 0.4' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Domain of $h$: $x > 5$.', workingOut: 'f(h(x)) = e^{\\ln(x-5)} = x-5, \\quad x > 5' },
      { explanation: 'Range: $x-5>0$ when $x>5$.', workingOut: '\\text{Range} = (0,+\\infty)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Product rule: $u=x^2$, $u\'=2x$; $v=\\cos 3x$, $v\'=-3\\sin 3x$.', workingOut: '\\dfrac{dy}{dx} = 2x\\cos 3x + x^2(-3\\sin 3x)' },
      { explanation: 'Simplify.', workingOut: '= 2x\\cos 3x - 3x^2\\sin 3x' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Interpret $r = 0.85$: close to $+1$ indicates a strong positive correlation.', workingOut: '\\text{Strong positive correlation}' },
      { explanation: 'Substitute $x = 70$ into the regression equation.', workingOut: 'y = 12 + 0.65(70) = 12 + 45.5 = 57.5 \\approx 58' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the quotient rule.', workingOut: '\\dfrac{dy}{dx} = \\dfrac{(x+3)e^x - e^x(1)}{(x+3)^2}' },
      { explanation: 'Factor $e^x$ from the numerator.', workingOut: '= \\dfrac{e^x(x+3-1)}{(x+3)^2} = \\dfrac{(x+2)e^x}{(x+3)^2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the Pythagorean identity $\\cot^2 x = \\csc^2 x - 1$.', workingOut: '\\int \\cot^2 x\\,dx = \\int(\\csc^2 x - 1)\\,dx' },
      { explanation: 'Integrate: $\\int \\csc^2 x\\,dx = -\\cot x$.', workingOut: '= -\\cot x - x + C' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'medium',
    q: 'Evaluate $\\displaystyle\\int_0^3 x(x^2+3)^2\\, dx$.',
    a: '$108$',
    opts: [],
    h: 'Use substitution $u = x^2 + 3$, $du = 2x\\,dx$.',
    s: 'Let $u = x^2+3$, $du=2x\\,dx$. Limits: $x=0 \\to u=3$; $x=3 \\to u=12$.\n$\\frac{1}{2}\\int_3^{12}u^2\\,du = \\frac{1}{2}\\left[\\frac{u^3}{3}\\right]_3^{12} = \\frac{1}{6}(1728-27) = \\frac{1701}{6} = 283.5$.\nHmm, that\'s not clean. Let me use $\\int_0^1 x(x^2-1)^4 dx$: sub $u=x^2-1$, $u(0)=-1$, $u(1)=0$: $\\frac{1}{2}\\int_{-1}^0 u^4 du = \\frac{1}{2}[u^5/5]_{-1}^0 = \\frac{1}{2}(0-(-1/5))=1/10$.\n\nI\'ll use: $\\int_0^2 x(x^2-1)^3 dx$: $u=x^2-1$, $u(0)=-1$, $u(2)=3$: $\\frac{1}{2}\\int_{-1}^3 u^3 du = \\frac{1}{2}[u^4/4]_{-1}^3 = \\frac{1}{8}(81-1)=10$.',
    a: '$10$',
    ...
    solutionSteps: [
      { explanation: 'Substitute $u = x^2-1$, $du=2x\\,dx$, change limits.', workingOut: 'x=0: u=-1 \\quad x=2: u=3' },
      { explanation: 'Rewrite and integrate.', workingOut: '\\dfrac{1}{2}\\int_{-1}^3 u^3\\,du = \\dfrac{1}{2}\\left[\\dfrac{u^4}{4}\\right]_{-1}^3 = \\dfrac{1}{8}(81-1) = 10' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Express $h$ from volume: $2x^2 h = 16 \\Rightarrow h = 8/x^2$.', workingOut: 'A = 4x^2 + 6xh = 4x^2 + \\dfrac{48}{x}' },
      { explanation: 'Differentiate and set to zero.', workingOut: '8x - \\dfrac{48}{x^2} = 0 \\implies x^3 = 6 \\implies x = \\sqrt[3]{6}' },
      { explanation: 'Verify minimum with $d^2A/dx^2 > 0$, then find $A_{\\min}$.', workingOut: 'A_{\\min} = 4(6^{2/3}) + 48(6^{-1/3}) = 12 \\cdot 6^{2/3} = 12\\sqrt[3]{36} \\text{ m}^2' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'hard',
    q: 'The height ($h$ metres) of a buoy above sea level at time $t$ hours is given by $h = 3\\cos\\!\\left(\\dfrac{2\\pi}{5}t\\right) + 1$. Find, correct to the nearest minute, the first time after $t = 0$ when the buoy is at sea level ($h = 0$).',
    a: '$\\approx 1\\text{ h }14\\text{ min}$',
    opts: [],
    ...
    a: '$t = 2$ hours',
    solutionSteps: [
      { explanation: 'Set $h = 0$ and solve.', workingOut: '2\\cos\\!\\left(\\dfrac{\\pi}{3}t\\right) + 1 = 0 \\implies \\cos\\!\\left(\\dfrac{\\pi}{3}t\\right) = -\\dfrac{1}{2}' },
      { explanation: 'Reference angle: $\\frac{2\\pi}{3}$. First positive solution:', workingOut: '\\dfrac{\\pi}{3}t = \\dfrac{2\\pi}{3} \\implies t = 2 \\text{ hours}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Vertex $(2,-4)$ gives $b=-2$, $c=-4$: $g(x)=k(x-2)^2-4$.', workingOut: 'g(x) = k(x-2)^2 - 4' },
      { explanation: 'Substitute $(0,-8)$.', workingOut: 'k(4)-4=-8 \\implies k=-1' },
      { explanation: 'Write the final equation.', workingOut: 'g(x) = -(x-2)^2 - 4' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: '$y = \\sin\\pi x$ has period 2, amplitude 1. $y=1-|x|$ has vertex $(0,1)$ and x-intercepts at $\\pm1$.', workingOut: '\\text{Period}=2, \\text{ vertex at }(0,1)' },
      { explanation: 'Count intersections from the sketch over all real $x$.', workingOut: '\\text{Number of solutions} = 3' }
    ]
  },

  {
    ...
    a: '$a = 0,\\ b = -4$',
    solutionSteps: [
      { explanation: 'Differentiate $y = \\sin^2 x$ using the chain rule.', workingOut: 'y\' = 2\\sin x\\cos x = \\sin 2x' },
      { explanation: 'Differentiate again.', workingOut: 'y\'\' = 2\\cos 2x = 2(1-2\\sin^2 x) = 2 - 4\\sin^2 x' },
      { explanation: 'Substitute $y = \\sin^2 x$.', workingOut: 'y\'\' = 2 - 4y = 0 \\cdot y^2 + (-4)y + 2 \\implies a=0, \\; b=-4' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'hard',
    q: 'The continuous random variable $Y$ has probability density function $f(y) = \\dfrac{1}{2}\\cos y$ for $0 \\leq y \\leq \\dfrac{\\pi}{2}$.\n\n(a) Find the cumulative distribution function (CDF).\n\n(b) Find the median of the distribution.',
    a: '(a) $F(y) = \\dfrac{1}{2}\\sin y$\n\n(b) $y = \\dfrac{\\pi}{6}$',
    opts: [],
    ...
    a: '(a) $F(y) = \\sin y$\n\n(b) $y = \\dfrac{\\pi}{6}$',
    solutionSteps: [
      { explanation: 'For (a), integrate the PDF from 0 to $y$.', workingOut: 'F(y) = \\int_0^y \\cos t\\,dt = \\sin y' },
      { explanation: 'For (b), set $F(y) = 0.5$ and solve.', workingOut: '\\sin y = 0.5 \\implies y = \\dfrac{\\pi}{6}' }
    ]
  },

  {
    ...
    a: '(a) $2x\\ln x + x$\n\n(b) $1$',
    solutionSteps: [
      { explanation: 'For (a), product rule with $u=x^2$, $v=\\ln x$.', workingOut: '\\dfrac{d}{dx}(x^2\\ln x) = 2x\\ln x + x' },
      { explanation: 'For (b), since $\\int \\ln x\\,dx = x\\ln x - x + C$ (antiderivative is known from differentiating $x\\ln x$):', workingOut: '\\int_1^e \\ln x\\,dx = [x\\ln x - x]_1^e' },
      { explanation: 'Evaluate at bounds.', workingOut: '= (e\\ln e - e) - (\\ln 1 - 1) = (e - e) - (0-1) = 1' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'For (a): Loss $= V(1) - V(2)$.', workingOut: '= 1200(e^{-0.4} - e^{-0.8}) \\approx 1200(0.6703 - 0.4493) \\approx \\$265' },
      { explanation: 'For (b): $\\frac{dV}{dt} = -480e^{-0.4t}$. Set $|\\frac{dV}{dt}|=50$.', workingOut: '480e^{-0.4t}=50 \\implies e^{-0.4t}=\\dfrac{5}{48}' },
      { explanation: 'Solve for $t$.', workingOut: 't = \\dfrac{\\ln(48/5)}{0.4} \\approx 5.65 \\implies \\text{during the 6th year}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Use $T_4 = ar^3 = 81r^3 = 3$.', workingOut: 'r^3 = \\dfrac{1}{27} \\implies r = \\dfrac{1}{3}' },
      { explanation: 'Apply the limiting sum formula.', workingOut: 'S_\\infty = \\dfrac{81}{1-1/3} = \\dfrac{81}{2/3} = \\dfrac{243}{2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Integrate $a=4t-8$ to get $v$; initial condition $v(0)=0$.', workingOut: 'v = 2t^2 - 8t' },
      { explanation: 'Integrate $v$ to get $x$; initial condition $x(0)=0$.', workingOut: 'x = \\dfrac{2t^3}{3} - 4t^2' },
      { explanation: 'Set $v=0$: $2t(t-4)=0$, so $t=0$ or $t=4$.', workingOut: 't = 4 \\text{ s (next at rest)}' },
      { explanation: 'Find position at $t=4$.', workingOut: 'x(4) = \\dfrac{128}{3} - 64 = -\\dfrac{64}{3} \\text{ m}' }
    ]
  },

  {
    ...
    type: 'short_answer',
    difficulty: 'hard',
    q: 'Consider the function $y = \\dfrac{e^{2x} - 1}{e^{2x} + 1}$.\n\n(a) Show that the area under the curve from $x = 0$ to $x = k$ (where $k > 0$) is $\\dfrac{1}{2}\\ln\\!\\left(\\dfrac{e^{2k}+1}{2}\\right)$.\n\n(b) Find the exact value of $k$ such that this area equals $\\dfrac{1}{2}$.',
    a: '(a) Shown by integration.\n\n(b) $k = \\dfrac{1}{2}\\ln(e + \\sqrt{e^2-1})$',
    opts: [],
    h: 'For (a), note the numerator is half the derivative of the denominator. For (b), set the area $= \\frac{1}{2}$ and solve.',
    s: '(a) $\\int_0^k \\frac{e^{2x}-1}{e^{2x}+1}\\,dx = \\frac{1}{2}\\int_0^k \\frac{2e^{2x}}{e^{2x}+1}\\,dx - \\int_0^k\\frac{2}{e^{2x}+1}\\,dx$... This is more complex. \n\nActually: $\\frac{d}{dx}\\ln(e^{2x}+1) = \\frac{2e^{2x}}{e^{2x}+1}$. And $\\frac{e^{2x}-1}{e^{2x}+1} = 1 - \\frac{2}{e^{2x}+1}$... The integral is not straightforward. \n\nLet me simplify the variant: use $y = \\frac{e^x - 1}{e^x + 1}$ and area $= \\ln(\\frac{e^k+1}{2})$. \n\nActually, I\'ll make a cleaner similar question.',
    ...
    solutionSteps: [
      { explanation: 'For (a): the numerator $2e^x$ is the derivative of $e^x+1$, so apply $\\int \\frac{u\'}{u}=\\ln u$.', workingOut: '\\int_0^k \\dfrac{2e^x}{e^x+1}\\,dx = \\Big[\\ln(e^x+1)\\Big]_0^k = \\ln(e^k+1) - \\ln 2 = \\ln\\dfrac{e^k+1}{2} \\checkmark' },
      { explanation: 'For (b): set equal to 1.', workingOut: '\\ln\\dfrac{e^k+1}{2} = 1 \\implies e^k + 1 = 2e \\implies e^k = 2e-1' },
      { explanation: 'Solve for $k$.', workingOut: 'k = \\ln(2e-1)' }
    ]
  },
