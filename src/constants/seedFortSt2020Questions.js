/**
 * Fort Street 2020 HSC Trial Exam — Advanced Mathematics
 * Section 1 (MC Q1-10) + Section 2 (Q1-Q19, Parts A-F).
 * topicId matches curriculumData.js topic IDs exactly.
 * Answers verified against the official Fort Street solutions.
 */
export const FORTST_2020_QUESTIONS = [
...

  {
    ...
    solutionSteps: [
      { explanation: 'Write the fourth term using the arithmetic term formula $T_n = a + (n-1)d$.', workingOut: 'T_4 = a + 3d = 27' },
      { explanation: 'Write the seventh term the same way.', workingOut: 'T_7 = a + 6d = 12' },
      { explanation: 'Subtract the first equation from the second to eliminate $a$.', workingOut: '(a + 6d) - (a + 3d) = 12 - 27 \\Rightarrow 3d = -15' },
      { explanation: 'Solve for the common difference.', workingOut: 'd = -5' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Reflect in the $x$-axis by negating the whole function.', workingOut: 'y = -3x^{5/2}' },
      { explanation: 'Translate $3$ units right by replacing $x$ with $x - 3$.', workingOut: 'y = -3(x-3)^{5/2}' },
      { explanation: 'Translate $4$ units down by subtracting $4$.', workingOut: 'y = -3(x-3)^{5/2} - 4' }
    ]
  },

  {
    ...
    a: '$x = \\dfrac{\\pi}{6}, \\dfrac{5\\pi}{6}, \\dfrac{-5\\pi}{6}, \\dfrac{-\\pi}{6}$',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'Substitute $u = 2x$ and expand the domain by multiplying the bounds by $2$.', workingOut: '-\\pi \\le x \\le \\pi \\Rightarrow -2\\pi \\le u \\le 2\\pi' },
      { explanation: 'The reference angle for $\\cos u = \\frac{1}{2}$ is $\\frac{\\pi}{3}$. Cosine is positive in the 1st and 4th quadrants.', workingOut: 'u = \\pm\\frac{\\pi}{3}' },
      { explanation: 'Add $\\pm 2\\pi$ to find all values within $[-2\\pi, 2\\pi]$.', workingOut: 'u = \\frac{\\pi}{3}, -\\frac{\\pi}{3}, \\frac{5\\pi}{3}, -\\frac{5\\pi}{3}' },
      { explanation: 'Divide every value by $2$ to recover $x$.', workingOut: 'x = \\frac{\\pi}{6}, -\\frac{\\pi}{6}, \\frac{5\\pi}{6}, -\\frac{5\\pi}{6}' }
    ]
  },

  {
    ...
    a: '$71 \\text{ m}^2$',
    opts: ['$31 \\text{ m}^2$', '$62 \\text{ m}^2$', '$71 \\text{ m}^2$', '$74 \\text{ m}^2$'],
    graphData: {
      jsxGraph: {
        width: 420,
        height: 300,
        boundingbox: [-3, 7, 23, -1.5],
    ...
    solutionSteps: [
      { explanation: 'State the trapezoidal rule for several strips: the first and last ordinates are counted once, the interior ones twice. Here the strip width is $h = 5$.', workingOut: 'A \\approx \\frac{h}{2}[y_1 + y_5 + 2(y_2 + y_3 + y_4)]' },
      { explanation: 'Substitute the five ordinates $2, 4.5, 5.1, 3.6, 0$.', workingOut: 'A \\approx \\frac{5}{2}[2 + 0 + 2(4.5 + 5.1 + 3.6)]' },
      { explanation: 'Add the interior ordinates.', workingOut: '= \\frac{5}{2}[2 + 2(13.2)]' },
      { explanation: 'Simplify inside the bracket.', workingOut: '= \\frac{5}{2}(28.4)' },
      { explanation: 'Multiply to find the area.', workingOut: '= 71 \\text{ m}^2' }
    ]
  },

  {
    ...
    a: 'Domain: $(9, \\infty)$, Range: $(0, \\infty)$',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'The square root requires its argument to be non-negative, and since it is in the denominator it cannot be zero. So $x - 9 > 0$ strictly.', workingOut: 'x - 9 > 0 \\Rightarrow x > 9' },
      { explanation: 'State the domain in interval notation (open at $9$).', workingOut: '\\text{Domain} = (9, \\infty)' },
      { explanation: 'For all $x > 9$, $\\sqrt{x-9} > 0$, so $\\frac{1}{\\sqrt{x-9}}$ is always positive and can take any positive value.', workingOut: '\\text{Range} = (0, \\infty)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The angle of inclination $\\theta$ is found from $\\tan\\theta = m$, where $m$ is the gradient.', workingOut: '\\tan\\theta = -\\sqrt{3}' },
      { explanation: 'Find the reference angle from $\\tan^{-1}(\\sqrt{3}) = 60°$.', workingOut: '\\text{reference angle} = 60°' },
      { explanation: 'A negative gradient means the line slopes downward, so the angle of inclination is obtuse: $180° - 60°$.', workingOut: '\\theta = 180° - 60° = 120°' }
    ]
  },

  {
    ...
    a: 'A horizontal point of inflexion at $x = -4$',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'Stationary points of $f$ occur where $f\'(x) = 0$, i.e. at $x = -4$ and $x = 2$.', workingOut: "f'(x) = 0 \\text{ at } x = -4, \\; 2" },
      { explanation: 'At $x = -4$ the gradient function touches the $x$-axis (a double root), so $f\'$ does not change sign. A stationary point where $f\'$ keeps the same sign is a horizontal point of inflexion.', workingOut: "x = -4: \\text{ touch} \\Rightarrow \\text{horizontal inflexion}" },
      { explanation: 'At $x = 2$ the gradient function crosses the axis, so $f\'$ changes sign — this is a turning point (not an inflexion).', workingOut: "x = 2: \\text{ cross} \\Rightarrow \\text{turning point}" },
      { explanation: 'Select the correct statement.', workingOut: '\\text{horizontal point of inflexion at } x = -4' }
    ]
  },

  {
    ...
    a: '$4\\pi$',
    opts: ['$2\\pi$', '$4\\pi$', '$6\\pi$', '$10\\pi$'],
    graphData: {
      jsxGraph: {
        width: 340,
        height: 300,
        boundingbox: [-5.5, 5, 6, -2],
    ...
    solutionSteps: [
      { explanation: 'Use the sector area formula to find the angle $\\theta$ (in radians).', workingOut: 'A = \\frac{1}{2}r^2\\theta \\Rightarrow 10\\pi = \\frac{1}{2}(5)^2\\theta' },
      { explanation: 'Solve for $\\theta$.', workingOut: '\\theta = \\frac{10\\pi}{12.5} = \\frac{4\\pi}{5}' },
      { explanation: 'Apply the arc-length formula $\\ell = r\\theta$.', workingOut: '\\ell = 5 \\times \\frac{4\\pi}{5}' },
      { explanation: 'Simplify.', workingOut: '\\ell = 4\\pi \\text{ cm}' }
    ]
  },

  {
    ...
    a: '$\\dfrac{17}{8}$',
    opts: ['$\\dfrac{1}{16}$', '$2$', '$\\dfrac{35}{16}$', '$\\dfrac{17}{8}$'],
    ...
    solutionSteps: [
      { explanation: 'All probabilities sum to $1$; add them to find $a$.', workingOut: 'a + 3a + 5a + 7a = 16a = 1 \\Rightarrow a = \\frac{1}{16}' },
      { explanation: 'Apply the expected value formula $E(X) = \\sum x\\,P(x)$.', workingOut: 'E(X) = 0(a) + 1(3a) + 2(5a) + 3(7a)' },
      { explanation: 'Collect the multiples of $a$.', workingOut: '= 3a + 10a + 21a = 34a' },
      { explanation: 'Substitute $a = \\frac{1}{16}$ and simplify.', workingOut: 'E(X) = \\frac{34}{16} = \\frac{17}{8}' }
    ]
  },

  {
    ...
    a: 'A positive cubic ($y = x^3 - 4x + C$) with a local maximum left of the origin and a local minimum right of it (Option B).',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'Find $f(x)$ by integrating $f\'(x)$; the constant $C$ shifts the graph vertically.', workingOut: 'f(x) = \\int (3x^2 - 4)\\,dx = x^3 - 4x + C' },
      { explanation: 'Factor to see the basic shape: a positive cubic with roots near $-2, 0, 2$ (before the vertical shift).', workingOut: 'f(x) = x(x-2)(x+2) + C' },
      { explanation: 'Locate the turning points by setting $f\'(x) = 0$.', workingOut: '3x^2 - 4 = 0 \\Rightarrow x = \\pm\\frac{2}{\\sqrt{3}}' },
      { explanation: 'Since the leading coefficient is positive, the cubic rises, has a local maximum, then a local minimum, then rises again — this matches Option B.', workingOut: '\\text{positive cubic with max then min (Option B)}' },
      { explanation: 'Sketch of the shape of $y = x^3 - 4x$ confirming two turning points.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 360,
            height: 300,
...

  {
    ...
    graphData: {
      geometry: {
        width: 300,
        points: { "A": [0, 0], "B": [2.6, 2.2], "C": [5.2, -2.4] },
        segments: [
          { from: "A", to: "B" },
          { from: "B", to: "C" },
          { from: "C", to: "A" }
        ...
        sideLabels: [
          { between: ["A", "B"], text: "1500 m" },
          { between: ["B", "C"], text: "2 km" }
        ]
      }
    },
    ...
    solutionSteps: [
      { explanation: 'Find the back-bearing of $A$ from $B$ by adding $180°$ to the forward bearing of $B$ from $A$.', workingOut: '\\text{bearing of } A \\text{ from } B = 68°30\' + 180° = 248°30\'' },
      { explanation: 'The angle $\\angle ABC$ is the difference between the bearing of $A$ from $B$ and the bearing of $C$ from $B$.', workingOut: '\\angle ABC = 248°30\' - 145°30\'' },
      { explanation: 'Subtract.', workingOut: '\\angle ABC = 103°' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'State the cosine rule for the side $AC$ opposite the angle $\\angle ABC$.', workingOut: 'AC^2 = AB^2 + BC^2 - 2\\,AB\\cdot BC\\cos(\\angle ABC)' },
      { explanation: 'Substitute $AB = 1500$, $BC = 2000$ and $\\angle ABC = 103°$.', workingOut: 'AC^2 = 1500^2 + 2000^2 - 2(1500)(2000)\\cos 103°' },
      { explanation: 'Evaluate the right-hand side.', workingOut: 'AC^2 = 7\\,599\\,706.3' },
      { explanation: 'Take the square root (remembering to do so!) and round to the nearest metre.', workingOut: 'AC = \\sqrt{7\\,599\\,706.3} \\approx 2757 \\text{ m}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set up the quotient rule $\\frac{vu\' - uv\'}{v^2}$ with $u = e^{3x}$ and $v = x-1$.', workingOut: 'u = e^{3x}, \\; u\' = 3e^{3x}; \\quad v = x-1, \\; v\' = 1' },
      { explanation: 'Substitute into the quotient rule.', workingOut: '\\frac{dy}{dx} = \\frac{(x-1)(3e^{3x}) - e^{3x}(1)}{(x-1)^2}' },
      { explanation: 'Expand the numerator.', workingOut: '= \\frac{3xe^{3x} - 3e^{3x} - e^{3x}}{(x-1)^2}' },
      { explanation: 'Factor $e^{3x}$ from the numerator and simplify.', workingOut: '= \\frac{e^{3x}(3x - 4)}{(x-1)^2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Simplify the expression before differentiating, using $\\tan x = \\frac{\\sin x}{\\cos x}$.', workingOut: '\\tan x\\cos x = \\frac{\\sin x}{\\cos x}\\cdot\\cos x = \\sin x' },
      { explanation: 'Differentiate the simplified function.', workingOut: '\\frac{d}{dx}(\\sin x) = \\cos x' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the rule for differentiating a logarithm with base $a$: $\\frac{d}{dx}\\log_a u = \\frac{u\'}{(\\ln a)\\,u}$.', workingOut: '\\frac{dy}{dx} = \\frac{2x}{(\\ln 4)(x^2 + 3)}' },
      { explanation: 'Simplify using $\\ln 4 = 2\\ln 2$.', workingOut: '= \\frac{2x}{2\\ln 2\\,(x^2 + 3)} = \\frac{x}{(\\ln 2)(x^2 + 3)}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Use the primitive of $\\sin(ax) = -\\frac{1}{a}\\cos(ax)$. Here $a = \\frac{1}{3}$, so divide by $\\frac{1}{3}$ (i.e. multiply by $3$).', workingOut: '\\int \\sin\\!\\left(\\frac{x}{3}\\right) dx = -3\\cos\\!\\left(\\frac{x}{3}\\right) + C' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the inner function $u = x^5 - 2$ whose derivative $5x^4$ is proportional to the $x^4$ factor outside.', workingOut: '\\frac{d}{dx}(x^5 - 2) = 5x^4' },
      { explanation: 'Adjust by a constant so the integrand contains $5x^4$, factoring out $\\frac{1}{20}$ (since the power increases to $4$).', workingOut: '\\int x^4(x^5-2)^3 dx = \\frac{1}{20}\\int 5x^4(x^5-2)^3 dx' },
      { explanation: 'Apply the reverse chain rule: raise the power by $1$ and divide by the new power.', workingOut: '= \\frac{1}{20}(x^5 - 2)^4 + C' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Notice the derivative of the denominator is $2x - 2 = 2(x-1)$. Rewrite the numerator as $\\frac{1}{2}(2x-2)$ so the integrand has the form $\\frac{f\'(x)}{f(x)}$.', workingOut: '\\int_0^1 \\frac{x-1}{x^2-2x+4}dx = \\frac{1}{2}\\int_0^1 \\frac{2x-2}{x^2-2x+4}dx' },
      { explanation: 'Integrate using $\\int \\frac{f\'}{f}dx = \\ln|f|$.', workingOut: '= \\frac{1}{2}\\big[\\ln(x^2 - 2x + 4)\\big]_0^1' },
      { explanation: 'Substitute the limits: at $x=1$, $1-2+4 = 3$; at $x=0$, $4$.', workingOut: '= \\frac{1}{2}(\\ln 3 - \\ln 4)' },
      { explanation: 'Combine the logarithms.', workingOut: '= \\frac{1}{2}\\ln\\frac{3}{4}' },
      { explanation: 'Bring the $\\frac{1}{2}$ inside as a square root to simplify fully.', workingOut: '= \\ln\\sqrt{\\frac{3}{4}} = \\ln\\frac{\\sqrt{3}}{2}' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'For real roots the discriminant must be non-negative (note: $\\geq 0$ includes equal roots).', workingOut: '\\Delta = b^2 - 4ac \\geq 0' },
      { explanation: 'Substitute $a = 5$, $b = -2$, $c = 8k - 15$.', workingOut: '(-2)^2 - 4(5)(8k - 15) \\geq 0' },
      { explanation: 'Expand.', workingOut: '4 - 160k + 300 \\geq 0' },
      { explanation: 'Collect terms.', workingOut: '304 \\geq 160k' },
      { explanation: 'Solve for $k$.', workingOut: 'k \\leq \\frac{304}{160} = \\frac{19}{10}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find the vertical asymptote where the denominator equals zero.', workingOut: 'x + 1 = 0 \\Rightarrow x = -1' },
      { explanation: 'Find the horizontal asymptote: as $x \\to \\pm\\infty$, $\\frac{4}{x+1} \\to 0$, so $y \\to 2$.', workingOut: 'y = 2' },
      { explanation: 'Find the $x$-intercept by setting $y = 0$.', workingOut: '2 - \\frac{4}{x+1} = 0 \\Rightarrow x + 1 = 2 \\Rightarrow x = 1' },
      { explanation: 'Find the $y$-intercept by setting $x = 0$.', workingOut: 'y = 2 - \\frac{4}{1} = -2' },
      { explanation: 'Sketch the hyperbola with both asymptotes and the two intercepts.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 400,
            height: 360,
...

  {
    ...
    solutionSteps: [
      { explanation: 'Compare the shape and skewness. Central Park\'s box and whiskers are balanced about the median (symmetric); East Park has a long right whisker (positively skewed).', workingOut: '\\text{Central: symmetric}; \\quad \\text{East: positively skewed}' },
      { explanation: 'Compare location using the medians. Central Park\'s median is about $7.5$ m, East Park\'s about $4$ m, so Central Park\'s trees are typically taller.', workingOut: '\\text{median}_C \\approx 7.5 > \\text{median}_E \\approx 4' },
      { explanation: 'Compare spread using the range (and IQR). East Park\'s range is much larger.', workingOut: '\\text{range}_E \\approx 16 > \\text{range}_C \\approx 6' },
      { explanation: 'Interpret in context: East Park\'s heights are more spread out and less consistent; half of East Park\'s trees are shorter than the shortest tree in Central Park.', workingOut: '\\text{East Park: more variable, generally shorter trees}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Use logarithm laws to split the function so it is easier to differentiate.', workingOut: 'y = \\ln(2x-1) - \\ln(x+1)' },
      { explanation: 'Differentiate each term using $\\frac{d}{dx}\\ln(ax+b) = \\frac{a}{ax+b}$.', workingOut: '\\frac{dy}{dx} = \\frac{2}{2x-1} - \\frac{1}{x+1}' },
      { explanation: 'Evaluate the tangent gradient at $x = 2$.', workingOut: '\\frac{2}{3} - \\frac{1}{3} = \\frac{1}{3}' },
      { explanation: 'The normal is perpendicular, so its gradient is the negative reciprocal.', workingOut: 'm_{\\text{normal}} = -3' },
      { explanation: 'Find the point of contact at $x = 2$.', workingOut: 'y = \\ln\\!\\left(\\frac{3}{3}\\right) = \\ln 1 = 0 \\Rightarrow (2, 0)' },
      { explanation: 'Use point–gradient form for the normal line.', workingOut: 'y - 0 = -3(x - 2) \\Rightarrow y = -3x + 6' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Group the $x$ terms and $y$ terms, ready to complete the square.', workingOut: '(x^2 + 4x) + (y^2 - 10y) = -16' },
      { explanation: 'Complete the square for $x$ (add $4$) and for $y$ (add $25$) to both sides.', workingOut: '(x^2 + 4x + 4) + (y^2 - 10y + 25) = -16 + 4 + 25' },
      { explanation: 'Write each group as a perfect square and simplify the right side.', workingOut: '(x + 2)^2 + (y - 5)^2 = 13' },
      { explanation: 'Read off the centre and radius from the standard form $(x-h)^2 + (y-k)^2 = r^2$.', workingOut: '\\text{Centre } (-2, 5), \\quad r = \\sqrt{13}' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'Find the $x$-intercepts by setting the function to zero.', workingOut: '(x+2)(x-2)^3 = 0 \\Rightarrow x = -2, \\; 2' },
      { explanation: 'Find the $y$-intercept by substituting $x = 0$.', workingOut: 'f(0) = (2)(-2)^3 = -16' },
      { explanation: 'State the intercepts.', workingOut: '(-2, 0), \\; (2, 0), \\; (0, -16)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $f(x) = (x+2)(x-2)^3$ using the product rule.', workingOut: "f'(x) = (x-2)^3(1) + (x+2)\\cdot 3(x-2)^2" },
      { explanation: 'Factor out the common factor $(x-2)^2$.', workingOut: "= (x-2)^2[(x-2) + 3(x+2)]" },
      { explanation: 'Simplify the bracket.', workingOut: "= (x-2)^2(4x + 4) = 4(x-2)^2(x+1)" },
      { explanation: 'Differentiate $f\'(x)$ using the product rule on $4(x-2)^2(x+1)$.', workingOut: "f''(x) = 4[2(x-2)(x+1) + (x-2)^2]" },
      { explanation: 'Factor out $(x-2)$ and simplify.', workingOut: "= 4(x-2)[2(x+1) + (x-2)] = 4(x-2)(3x) = 12x(x-2)" }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find the stationary points where $f\'(x) = 0$.', workingOut: '4(x-2)^2(x+1) = 0 \\Rightarrow x = -1, \\; 2' },
      { explanation: 'Find the $y$-coordinates.', workingOut: 'f(-1) = (1)(-3)^3 = -27; \\quad f(2) = 0' },
      { explanation: 'Test $x = -1$ with the second derivative.', workingOut: "f''(-1) = 12(-1)(-3) = 36 > 0 \\Rightarrow \\text{minimum}" },
      { explanation: 'At $x = 2$, $f\'\'(2) = 0$, so the test is inconclusive. Use a concavity table around $x = 2$.', workingOut: "f''(1.5) = -9 < 0, \\quad f''(2.5) = 15 > 0" },
      { explanation: 'Since concavity changes and the tangent is horizontal, $(2, 0)$ is a horizontal point of inflexion.', workingOut: '(-1, -27)\\text{ min}, \\quad (2, 0)\\text{ horizontal inflexion}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find candidate inflection points where the second derivative is zero.', workingOut: "f''(x) = 12x(x-2) = 0 \\Rightarrow x = 0, \\; 2" },
      { explanation: 'We already established $(2, 0)$ is a horizontal point of inflexion. Test $x = 0$ for a change in concavity.', workingOut: "f''(-0.5) = 15 > 0, \\quad f''(0.5) = -9 < 0" },
      { explanation: 'Concavity changes at $x = 0$, so it is a point of inflexion; find its $y$-value.', workingOut: 'f(0) = -16 \\Rightarrow (0, -16)' },
      { explanation: 'State all points of inflection.', workingOut: '(0, -16) \\text{ and } (2, 0)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Collect all features from parts (a), (c) and (d).', workingOut: '(-2,0),(2,0),(0,-16),(-1,-27)\\text{ min},(2,0)\\text{ horiz. POI}' },
      { explanation: 'Note the end behaviour: a positive quartic rises to $+\\infty$ on both sides.', workingOut: 'x \\to \\pm\\infty \\Rightarrow f(x) \\to +\\infty' },
      { explanation: 'Sketch the curve through all key points (using different scales on the axes).', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 400,
            height: 340,
            boundingbox: [-3.2, 12, 3.6, -32],
            boardOptions: { keepaspectratio: false },
...

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the horizontal dilation from the factor multiplying $x$: a factor of $2$ inside means a dilation by $\\frac{1}{2}$ horizontally.', workingOut: '\\text{horizontal dilation, factor } \\tfrac{1}{2}' },
      { explanation: 'Identify the horizontal translation from $-\\frac{1}{4}$ inside the bracket.', workingOut: '\\text{horizontal translation } \\tfrac{1}{4} \\text{ unit right}' },
      { explanation: 'State the order: the dilation (closest to $x$) is applied first, then the translation.', workingOut: '\\text{dilate } \\tfrac{1}{2} \\text{ THEN translate right } \\tfrac{1}{4}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the mapping $x \\mapsto \\frac{x}{2} + \\frac{1}{4}$ to each feature (dilation by $\\frac{1}{2}$ then shift right $\\frac{1}{4}$); $y$-values are unchanged.', workingOut: '(x_0, y_0) \\mapsto \\left(\\tfrac{x_0}{2} + \\tfrac{1}{4}, \\; y_0\\right)' },
      { explanation: 'Transform the $x$-intercepts.', workingOut: '(-2,0)\\to(-\\tfrac{3}{4},0); \\quad (2,0)\\to(\\tfrac{5}{4},0)' },
      { explanation: 'Transform the minimum and the inflection at the origin region.', workingOut: '(-1,-27)\\to(-\\tfrac{1}{4},-27); \\quad (0,-16)\\to(\\tfrac{1}{4},-16)' },
      { explanation: 'Sketch the transformed quartic through the new key points.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 400,
            height: 340,
            boundingbox: [-2, 12, 2.4, -32],
            boardOptions: { keepaspectratio: false },
...

  {
    ...
    solutionSteps: [
      { explanation: 'The yearly growths form a geometric sequence with first term $50$ and ratio $0.9$.', workingOut: '50, \\; 50(0.9), \\; 50(0.9)^2' },
      { explanation: 'Add the growths for the first three years.', workingOut: '50 + 45 + 40.5' },
      { explanation: 'Total growth after $3$ years.', workingOut: '= 135.5 \\text{ cm}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Since $|r| = 0.9 < 1$, the total growth has a finite limiting sum.', workingOut: 'S_\\infty = \\frac{a}{1 - r} = \\frac{50}{1 - 0.9}' },
      { explanation: 'Evaluate the limiting growth.', workingOut: 'S_\\infty = 500 \\text{ cm}' },
      { explanation: 'Add the initial height of $80$ cm to find the maximum possible height.', workingOut: '80 + 500 = 580 \\text{ cm}' },
      { explanation: 'Compare with $6$ m $= 600$ cm.', workingOut: '580 < 600 \\Rightarrow \\text{never reaches } 6\\text{ m}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'A height of $5$ m $= 500$ cm requires total growth of $500 - 80 = 420$ cm.', workingOut: '\\text{growth needed} = 420 \\text{ cm}' },
      { explanation: 'Set the GP sum equal to $420$.', workingOut: '\\frac{50(1 - 0.9^n)}{1 - 0.9} = 420' },
      { explanation: 'Simplify.', workingOut: '500(1 - 0.9^n) = 420 \\Rightarrow 0.9^n = 0.16' },
      { explanation: 'Take logarithms and solve for $n$.', workingOut: 'n = \\frac{\\ln 0.16}{\\ln 0.9} \\approx 17.39' },
      { explanation: 'Since $17 < n < 18$, the height is reached during the $18^{\\text{th}}$ year.', workingOut: '\\text{18th year}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Substitute $t = 0$ into $C = C_0 e^{-kt}$, using $e^0 = 1$.', workingOut: 'C = C_0 e^{0} = C_0 = 6' },
      { explanation: 'Substitute $t = 1$ and $C = 2.4$.', workingOut: '2.4 = 6e^{-k}' },
      { explanation: 'Solve for $e^{-k}$.', workingOut: 'e^{-k} = \\frac{2.4}{6} = 0.4 = \\frac{2}{5}' },
      { explanation: 'Take natural logarithms to isolate $k$.', workingOut: '-k = \\ln\\tfrac{2}{5} \\Rightarrow k = -\\ln\\tfrac{2}{5}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set the concentration equal to the safe threshold.', workingOut: '0.2 = 6e^{-kt}' },
      { explanation: 'Solve for the exponential.', workingOut: 'e^{-kt} = \\frac{0.2}{6} = \\frac{1}{30}' },
      { explanation: 'Take logarithms; using $k = -\\ln\\frac{2}{5}$, so $-kt = \\ln\\frac{2}{5}\\cdot t$.', workingOut: '\\ln\\tfrac{1}{30} = \\ln\\tfrac{2}{5}\\cdot t \\Rightarrow t = \\frac{\\ln(1/30)}{\\ln(2/5)}' },
      { explanation: 'Evaluate.', workingOut: 't \\approx 3.712 \\text{ years}' },
      { explanation: 'Convert the decimal part to months ($0.712 \\times 12 \\approx 9$).', workingOut: 't \\approx 3 \\text{ years } 9 \\text{ months}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Each throw is a two-branch event: a win (W, game ends) or a miss (W\'). Play continues only along miss branches.', workingOut: "P(\\text{W}) \\text{ or } P(\\text{W}')" },
      { explanation: 'List the win/miss probabilities for each thrower in order.', workingOut: "A: \\tfrac{1}{30},\\tfrac{29}{30}; \\;\\; R: \\tfrac{1}{40},\\tfrac{39}{40}" },
      { explanation: 'Draw the tree as a chain of four throws along the "miss" branches.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 460,
            height: 300,
...

  {
    ...
    solutionSteps: [
      { explanation: 'Alice wins on her first throw with probability $\\frac{1}{30}$.', workingOut: 'P_1 = \\frac{1}{30}' },
      { explanation: 'For Alice to win on her second throw, she misses ($\\frac{29}{30}$), Raoul misses ($\\frac{39}{40}$), then Alice hits ($\\frac{1}{30}$).', workingOut: 'P_2 = \\frac{29}{30}\\cdot\\frac{39}{40}\\cdot\\frac{1}{30}' },
      { explanation: 'Add the two probabilities.', workingOut: 'P = \\frac{1}{30} + \\frac{1131}{36000}' },
      { explanation: 'Simplify to a single fraction.', workingOut: 'P = \\frac{259}{4000}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Alice wins on her 1st, 2nd, 3rd, … throw; these probabilities form a geometric series.', workingOut: 'P = \\frac{1}{30} + \\frac{29}{30}\\cdot\\frac{39}{40}\\cdot\\frac{1}{30} + \\cdots' },
      { explanation: 'Identify the first term and common ratio (the ratio is "both miss a full round").', workingOut: 'a = \\frac{1}{30}, \\quad r = \\frac{29}{30}\\cdot\\frac{39}{40} = \\frac{1131}{1200}' },
      { explanation: 'Apply the limiting sum formula $S_\\infty = \\frac{a}{1-r}$.', workingOut: 'S_\\infty = \\frac{1/30}{1 - \\frac{1131}{1200}} = \\frac{1/30}{\\frac{69}{1200}}' },
      { explanation: 'Simplify.', workingOut: 'S_\\infty = \\frac{1}{30}\\cdot\\frac{1200}{69} = \\frac{40}{69}' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'At the points of intersection the two $y$-values are equal.', workingOut: '4e^{-x} = e^x - 3' },
      { explanation: 'Multiply every term by $e^x$ (recall $e^{-x}\\cdot e^x = 1$ and $e^x\\cdot e^x = e^{2x}$).', workingOut: '4 = e^{2x} - 3e^x' },
      { explanation: 'Rearrange into the required form.', workingOut: 'e^{2x} - 3e^x - 4 = 0' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Substitute $u = e^x$ to turn the equation into a quadratic.', workingOut: 'u^2 - 3u - 4 = 0' },
      { explanation: 'Factorise.', workingOut: '(u - 4)(u + 1) = 0 \\Rightarrow u = 4 \\text{ or } u = -1' },
      { explanation: 'Reject $u = -1$ because $e^x > 0$ for all $x$.', workingOut: 'e^x = 4 \\quad (e^x \\neq -1)' },
      { explanation: 'Take natural logarithms.', workingOut: 'x = \\ln 4' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set up the area as the integral of the upper curve minus the lower curve over $[0, \\ln 4]$.', workingOut: 'A = \\int_0^{\\ln 4}\\big(4e^{-x} - (e^x - 3)\\big)\\,dx' },
      { explanation: 'Find the primitive term by term.', workingOut: '= \\big[-4e^{-x} - e^x + 3x\\big]_0^{\\ln 4}' },
      { explanation: 'Substitute $x = \\ln 4$, using $e^{-\\ln 4} = \\frac{1}{4}$ and $e^{\\ln 4} = 4$.', workingOut: '= \\left(-4\\cdot\\tfrac{1}{4} - 4 + 3\\ln 4\\right) = (-1 - 4 + 3\\ln 4)' },
      { explanation: 'Substitute $x = 0$ ($e^0 = 1$).', workingOut: '- (-4 - 1 + 0) = +5' },
      { explanation: 'Combine to get the exact area.', workingOut: 'A = -5 + 3\\ln 4 + 5 = 3\\ln 4 \\text{ units}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Acceleration is the gradient (derivative) of the velocity–time graph.', workingOut: 'a = \\frac{dv}{dt}' },
      { explanation: 'The gradient is zero where the velocity curve has turning points — its maximum and minimum.', workingOut: 't = 3 \\text{ (max)}, \\quad t = 7 \\text{ (min)}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Speed is $|v|$. It increases when the particle accelerates in its direction of motion — i.e. when $v$ and $a$ share the same sign.', workingOut: '\\text{speed} \\uparrow \\iff v, a \\text{ same sign}' },
      { explanation: 'On $1 < t < 3$ the velocity is positive and rising (so $a > 0$) — same sign.', workingOut: '1 < t < 3: \\; v > 0, \\; a > 0' },
      { explanation: 'On $5 < t < 7$ the velocity is negative and becoming more negative (so $a < 0$) — same sign.', workingOut: '5 < t < 7: \\; v < 0, \\; a < 0' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Write the velocity with a negative index and differentiate to get acceleration.', workingOut: 'v = 2(1+t)^{-1} - t \\Rightarrow a = -2(1+t)^{-2} - 1' },
      { explanation: 'Substitute $t = 0$.', workingOut: 'a = -\\frac{2}{(1)^2} - 1 = -3 \\text{ m/s}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Integrate the velocity to get displacement.', workingOut: 'x = \\int\\left(\\frac{2}{1+t} - t\\right)dt = 2\\ln(1+t) - \\frac{t^2}{2} + C' },
      { explanation: 'Apply the initial condition $x = 0$ at $t = 0$ (note $\\ln 1 = 0$).', workingOut: '0 = 2\\ln 1 - 0 + C \\Rightarrow C = 0' },
      { explanation: 'State the displacement function.', workingOut: 'x = 2\\ln(1+t) - \\frac{t^2}{2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The particle is stationary when velocity is zero.', workingOut: '\\frac{2}{1+t} - t = 0' },
      { explanation: 'Multiply through by $(1+t)$ to clear the fraction.', workingOut: '2 - t(1+t) = 0 \\Rightarrow t^2 + t - 2 = 0' },
      { explanation: 'Factorise.', workingOut: '(t + 2)(t - 1) = 0' },
      { explanation: 'Reject the negative time.', workingOut: 't = 1 \\text{ second} \\quad (t \\ge 0)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The particle reverses direction at $t = 1$ (from part c), so compute displacements at $t = 0, 1, 2$ separately.', workingOut: 'x(0) = 0' },
      { explanation: 'Find the displacement at $t = 1$.', workingOut: 'x(1) = 2\\ln 2 - \\tfrac{1}{2} \\approx 0.886' },
      { explanation: 'Find the displacement at $t = 2$.', workingOut: 'x(2) = 2\\ln 3 - 2 \\approx 0.197' },
      { explanation: 'Add the distance of each leg (out to $t=1$, then back).', workingOut: 'd = 0.886 + (0.886 - 0.197)' },
      { explanation: 'Total distance to three significant figures.', workingOut: 'd \\approx 1.58 \\text{ m}' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'Substitute $t = 10$ into the volume model.', workingOut: 'V = 3600\\left(1 - \\frac{10}{60}\\right)^2' },
      { explanation: 'Simplify the bracket.', workingOut: '= 3600\\left(\\frac{5}{6}\\right)^2' },
      { explanation: 'Evaluate.', workingOut: '= 3600\\cdot\\frac{25}{36} = 2500 \\text{ litres}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set the volume to half of $3600$, i.e. $1800$ L.', workingOut: '3600\\left(1 - \\frac{t}{60}\\right)^2 = 1800' },
      { explanation: 'Divide and take the square root of both sides (keep both signs).', workingOut: '\\left(1 - \\frac{t}{60}\\right)^2 = \\frac{1}{2} \\Rightarrow 1 - \\frac{t}{60} = \\pm\\frac{1}{\\sqrt2}' },
      { explanation: 'Solve for $t$ in both cases.', workingOut: 't = 60\\left(1 - \\tfrac{1}{\\sqrt2}\\right) \\approx 17.6 \\;\\text{ or }\\; t = 60\\left(1 + \\tfrac{1}{\\sqrt2}\\right) \\approx 102.4' },
      { explanation: 'Reject $t \\approx 102.4$ since $0 \\le t \\le 60$, and round the valid root.', workingOut: 't \\approx 17.6 \\approx 18 \\text{ minutes}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $V$ with respect to $t$ using the chain rule.', workingOut: '\\frac{dV}{dt} = 2\\cdot 3600\\left(1 - \\frac{t}{60}\\right)\\left(-\\frac{1}{60}\\right) = -120\\left(1 - \\frac{t}{60}\\right)' },
      { explanation: 'Substitute $t = 20$.', workingOut: '\\frac{dV}{dt} = -120\\left(1 - \\frac{20}{60}\\right) = -120\\cdot\\frac{2}{3}' },
      { explanation: 'Evaluate; the negative sign indicates draining.', workingOut: '= -80 \\Rightarrow 80 \\text{ L/min draining}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Write the magnitude of the drain rate from part (c).', workingOut: '\\left|\\frac{dV}{dt}\\right| = 120\\left(1 - \\frac{t}{60}\\right)' },
      { explanation: 'This expression decreases as $t$ increases, so it is largest at the start of the interval.', workingOut: '\\text{largest at } t = 0' },
      { explanation: 'Evaluate the fastest rate.', workingOut: '\\left.\\frac{dV}{dt}\\right|_{t=0} = -120 \\Rightarrow 120 \\text{ L/min}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The amplitude is $7$ and the vertical shift is $13$, and sine ranges between $-1$ and $1$.', workingOut: '-1 \\le \\sin(nx + 1.5) \\le 1' },
      { explanation: 'Maximum temperature when $\\sin = 1$.', workingOut: 'T_{\\max} = 7(1) + 13 = 20°' },
      { explanation: 'Minimum temperature when $\\sin = -1$.', workingOut: 'T_{\\min} = 7(-1) + 13 = 6°' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The period of a sine function with coefficient $n$ is $\\frac{2\\pi}{n}$. Set it equal to $12$.', workingOut: '\\frac{2\\pi}{n} = 12' },
      { explanation: 'Solve for $n$.', workingOut: 'n = \\frac{2\\pi}{12} = \\frac{\\pi}{6}' },
      { explanation: 'Evaluate to two decimal places.', workingOut: 'n \\approx 0.52' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The temperature is lowest when the sine equals $-1$, i.e. its argument is $\\frac{3\\pi}{2}$.', workingOut: '\\frac{\\pi}{6}x + 1.5 = \\frac{3\\pi}{2}' },
      { explanation: 'Solve for $x$.', workingOut: 'x = \\frac{\\frac{3\\pi}{2} - 1.5}{\\frac{\\pi}{6}} \\approx 6.1' },
      { explanation: 'Round to the nearest month.', workingOut: 'x \\approx 6 \\Rightarrow \\text{June}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the key values: amplitude $7$, centre $T = 13$, max $20$, min $6$, period $12$.', workingOut: 'T \\in [6, 20], \\text{ period } 12' },
      { explanation: 'Locate the minimum at month $\\approx 6$ (June) and the endpoints near the maximum.', workingOut: '(1, \\approx 20), (6, 6), (12, \\approx 20)' },
      { explanation: 'Sketch the sine curve over the domain $1 \\le x \\le 12$.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 440,
            height: 300,
            boundingbox: [-1, 23, 14, -2],
            boardOptions: { keepaspectratio: false },
...

  {
    ...
    solutionSteps: [
      { explanation: 'In right triangle $AHB$ (vertical side $AH = 1$ km, angle $\\theta$ at $H$), use cosine and tangent to find the walked side $HB$ and horizontal offset $AB$.', workingOut: 'HB = \\frac{1}{\\cos\\theta}, \\quad AB = \\tan\\theta' },
      { explanation: 'By symmetry the other walk $CS$ and offset $DC$ are equal. Total distance walked is $2HB$.', workingOut: '\\text{walked} = 2HB = \\frac{2}{\\cos\\theta}' },
      { explanation: 'Time walked $= \\frac{\\text{distance}}{4}$.', workingOut: '\\text{time walked} = \\frac{2/\\cos\\theta}{4} = \\frac{1}{2\\cos\\theta}' },
      { explanation: 'The skated distance $BC$ is the $6$ km span minus the two horizontal offsets $2\\tan\\theta$.', workingOut: 'BC = 6 - 2\\tan\\theta' },
      { explanation: 'Time skated $= \\frac{BC}{12}$.', workingOut: '\\text{time skated} = \\frac{6 - 2\\tan\\theta}{12} = \\frac{1}{2} - \\frac{\\tan\\theta}{6}' },
      { explanation: 'Add the walking and skating times.', workingOut: 'T = \\frac{1}{2\\cos\\theta} + \\frac{1}{2} - \\frac{\\tan\\theta}{6}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $T$. Write $\\frac{1}{2\\cos\\theta} = \\frac{1}{2}(\\cos\\theta)^{-1}$ and $\\frac{\\tan\\theta}{6}$, applying the chain rule.', workingOut: '\\frac{dT}{d\\theta} = \\frac{\\sin\\theta}{2\\cos^2\\theta} - \\frac{\\sec^2\\theta}{6}' },
      { explanation: 'Factor out $\\frac{1}{\\cos^2\\theta}$ (which is always positive).', workingOut: '= \\frac{1}{\\cos^2\\theta}\\left(\\frac{\\sin\\theta}{2} - \\frac{1}{6}\\right)' },
      { explanation: 'Set $\\frac{dT}{d\\theta} = 0$; since $\\frac{1}{\\cos^2\\theta} \\neq 0$, the bracket must be zero.', workingOut: '\\frac{\\sin\\theta}{2} - \\frac{1}{6} = 0 \\Rightarrow \\sin\\theta = \\frac{1}{3}' },
      { explanation: 'Solve for $\\theta$.', workingOut: '\\theta = \\sin^{-1}\\tfrac{1}{3} \\approx 19°28\'' },
      { explanation: 'Confirm it is a minimum using a table of slopes (gradient changes from negative to positive).', workingOut: '\\text{slope: } - \\;0\\; + \\Rightarrow \\text{minimum at } \\theta \\approx 19°28\'' }
    ]
  },
