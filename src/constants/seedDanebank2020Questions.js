/**
 * Danebank 2020 HSC Trial Exam (Task 4) — Advanced Mathematics
 ...
 * Answers verified against the official Danebank solutions.
 */
export const DANEBANK_2020_QUESTIONS = [
...

  {
    ...
    a: '$-\\dfrac{2}{3}$',
    opts: ['$-\\dfrac{3}{2}$', '$-\\dfrac{2}{3}$', '$\\dfrac{2}{3}$', '$\\dfrac{3}{2}$'],
    ...
    solutionSteps: [
      { explanation: 'To find the gradient, rearrange the equation into gradient–intercept form $y = mx + b$, where $m$ is the gradient. Start by isolating the $y$ term.', workingOut: '\\frac{y}{2} = 1 - \\frac{x}{3}' },
      { explanation: 'Multiply every term by $2$ to make $y$ the subject.', workingOut: 'y = 2 - \\frac{2}{3}x' },
      { explanation: 'Identify the gradient as the coefficient of $x$ in this form.', workingOut: 'm = -\\frac{2}{3}' }
    ]
  },

  {
    ...
    a: '$-\\dfrac{2}{x^3}$',
    opts: ['$-\\dfrac{2}{x^3}$', '$-\\dfrac{1}{x}$', '$\\dfrac{1}{x}$', '$\\dfrac{2}{x^3}$'],
    h: 'Rewrite $\\dfrac{1}{x^2}$ as $x^{-2}$ and apply the power rule.',
    s: 'Write $y = x^{-2}$. By the power rule, $\\frac{dy}{dx} = -2x^{-3} = -\\frac{2}{x^3}$.',
    solutionSteps: [
      { explanation: 'Rewrite the function using a negative index so the power rule can be applied directly.', workingOut: 'y = \\frac{1}{x^2} = x^{-2}' },
      { explanation: 'Apply the power rule $\\frac{d}{dx}(x^n) = nx^{n-1}$: bring down the power $-2$ and reduce the index by $1$.', workingOut: '\\frac{dy}{dx} = -2x^{-3}' },
      { explanation: 'Rewrite with a positive index to match the answer format.', workingOut: '\\frac{dy}{dx} = -\\frac{2}{x^3}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the centre of the circle from the standard form $(x-h)^2 + (y-k)^2 = r^2$.', workingOut: '\\text{Centre } (3, 2)' },
      { explanation: 'For a circle to touch (be tangent to) the $x$-axis, the radius must equal the perpendicular distance from the centre to the $x$-axis, which is the $y$-coordinate of the centre.', workingOut: 'r = 2' },
      { explanation: 'Since $c = r^2$ in this equation, substitute $r = 2$.', workingOut: 'c = r^2 = 2^2 = 4' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Analyse the first term $\\sqrt{x}$. A square root is defined only for non-negative arguments, so the radicand must be $\\geq 0$.', workingOut: 'x \\geq 0' },
      { explanation: 'Analyse the second term $\\frac{1}{\\sqrt{2-x}}$. The square root requires $2 - x \\geq 0$, but because it sits in the denominator it cannot be zero, so $2 - x > 0$ strictly.', workingOut: '2 - x > 0 \\Rightarrow x < 2' },
      { explanation: 'Combine both restrictions: $x$ must be at least $0$ (included) and strictly less than $2$.', workingOut: '0 \\leq x < 2' },
      { explanation: 'Write the domain in interval notation, with a square bracket at $0$ (included) and a round bracket at $2$ (excluded).', workingOut: '[0, 2)' }
    ]
  },

  {
    ...
    a: '$a = -3$ and $d = 8$',
    opts: ['$a = -3$ and $d = 8$', '$a = 3$ and $d = 8$', '$a = 8$ and $d = -3$', '$a = 8$ and $d = 3$'],
    ...
    solutionSteps: [
      { explanation: 'Write the $7^{\\text{th}}$ term using the arithmetic formula $T_n = a + (n-1)d$.', workingOut: 'T_7 = a + 6d = 45' },
      { explanation: 'Write the $11^{\\text{th}}$ term the same way.', workingOut: 'T_{11} = a + 10d = 77' },
      { explanation: 'Subtract the first equation from the second to eliminate $a$ and solve for $d$.', workingOut: '4d = 32 \\Rightarrow d = 8' },
      { explanation: 'Substitute $d = 8$ back into $a + 6d = 45$ to find $a$.', workingOut: 'a + 6(8) = 45 \\Rightarrow a = 45 - 48 = -3' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Determine the sign of the first derivative. $A$ is a maximum turning point and the curve falls from $A$ towards $C$, so on $A < x < B$ the function is decreasing.', workingOut: 'f\'(x) < 0 \\quad (\\text{curve decreasing})' },
      { explanation: 'Determine the sign of the second derivative. The point of inflection is at $B$, where concavity changes. Just to the left of $B$ (i.e. on $A<x<B$) the curve is concave down.', workingOut: 'f\'\'(x) < 0 \\quad (\\text{concave down})' },
      { explanation: 'Combine both: decreasing and concave down.', workingOut: 'f\'(x) < 0 \\text{ and } f\'\'(x) < 0' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Factorise the quadratic. Find two numbers that multiply to $2 \\times 2 = 4$ and add to $-5$, giving the factorisation by grouping.', workingOut: '2x^2 - 5x + 2 = (2x - 1)(x - 2)' },
      { explanation: 'Find the roots by setting each factor to zero.', workingOut: 'x = \\frac{1}{2}, \\quad x = 2' },
      { explanation: 'The coefficient of $x^2$ is positive, so the parabola opens upward and lies above or on the $x$-axis outside the two roots. Because the inequality is "$\\geq$", the roots are included.', workingOut: 'x \\leq \\frac{1}{2} \\quad \\text{or} \\quad x \\geq 2' },
      { explanation: 'Write the solution set in interval notation with closed brackets at the roots.', workingOut: '\\left(-\\infty, \\tfrac{1}{2}\\right] \\cup [2, \\infty)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find the amplitude and period of $f(x) = \\frac{1}{2}\\sin 4x$. The amplitude is the coefficient $\\frac{1}{2}$; the period is $\\frac{2\\pi}{b}$ with $b = 4$.', workingOut: '\\text{amp}_f = \\tfrac{1}{2}, \\quad T_f = \\frac{2\\pi}{4} = \\frac{\\pi}{2}' },
      { explanation: 'Find the amplitude and period of $g(x) = \\sin 2x$. The amplitude is $1$; the period uses $b = 2$.', workingOut: '\\text{amp}_g = 1, \\quad T_g = \\frac{2\\pi}{2} = \\pi' },
      { explanation: 'Compare the amplitudes: it changed from $\\frac{1}{2}$ to $1$, a factor of $2$.', workingOut: '\\frac{1}{1/2} = 2 \\Rightarrow \\text{amplitude doubled}' },
      { explanation: 'Compare the periods: it changed from $\\frac{\\pi}{2}$ to $\\pi$, a factor of $2$.', workingOut: '\\frac{\\pi}{\\pi/2} = 2 \\Rightarrow \\text{period doubled}' }
    ]
  },

  {
    ...
    a: 'The largest possible range is 4 times the interquartile range.',
    opts: [
    ...
    solutionSteps: [
      { explanation: 'Recall the outlier rule: a value is an outlier if it lies more than $1.5 \\times \\text{IQR}$ below $Q_1$ or above $Q_3$. So with no outliers, the minimum and maximum can each be at most $1.5 \\times \\text{IQR}$ from the nearest quartile.', workingOut: '\\text{each whisker} \\leq 1.5 \\times \\text{IQR}' },
      { explanation: 'The box (between $Q_1$ and $Q_3$) spans exactly one interquartile range.', workingOut: '\\text{box} = 1 \\times \\text{IQR}' },
      { explanation: 'The range is the distance from minimum to maximum: lower whisker + box + upper whisker.', workingOut: '\\text{range} = 1.5\\,\\text{IQR} + 1\\,\\text{IQR} + 1.5\\,\\text{IQR}' },
      { explanation: 'Add the multiples to find the largest possible range.', workingOut: '\\text{range} = 4 \\times \\text{IQR}' }
    ]
  },

  {
    ...
    a: '$\\dfrac{1}{4\\ln 2}$',
    opts: ['$\\dfrac{1}{4\\ln 2}$', '$\\dfrac{1}{2\\ln 2}$', '$\\dfrac{\\ln 2}{4}$', '$\\dfrac{\\ln 2}{2}$'],
    ...
    solutionSteps: [
      { explanation: 'Find the primitive. For base $a$, $\\int a^{x}\\,dx = \\frac{a^{x}}{\\ln a}$. Here the exponent is $-x$, so dividing by the inner derivative $-1$ gives a factor of $\\frac{1}{-\\ln 2}$.', workingOut: '\\int 2^{-x}\\,dx = \\frac{2^{-x}}{-\\ln 2}' },
      { explanation: 'Substitute the upper and lower limits $x = 2$ and $x = 1$.', workingOut: '\\left[\\frac{2^{-x}}{-\\ln 2}\\right]_1^2 = \\frac{1}{-\\ln 2}\\left(2^{-2} - 2^{-1}\\right)' },
      { explanation: 'Evaluate the powers of $2$.', workingOut: '= \\frac{1}{-\\ln 2}\\left(\\frac{1}{4} - \\frac{1}{2}\\right)' },
      { explanation: 'Simplify inside the bracket.', workingOut: '= \\frac{1}{-\\ln 2}\\left(-\\frac{1}{4}\\right)' },
      { explanation: 'The two negatives cancel, giving the final value.', workingOut: '= \\frac{1}{4\\ln 2}' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'State the trapezoidal rule for a single strip of width $h$: $A \\approx \\frac{h}{2}(\\text{left height} + \\text{right height})$. Here each strip is $6$ m wide.', workingOut: 'A \\approx \\frac{h}{2}(f_0 + f_1), \\quad h = 6' },
      { explanation: 'Apply the rule to the first strip, between the $6$ m and $7$ m ordinates.', workingOut: 'A_1 = \\frac{6}{2}(6 + 7) = 3 \\times 13 = 39' },
      { explanation: 'Apply the rule to the second strip, between the $7$ m and $10$ m ordinates.', workingOut: 'A_2 = \\frac{6}{2}(7 + 10) = 3 \\times 17 = 51' },
      { explanation: 'Add the two strip areas for the total approximation.', workingOut: 'A \\approx 39 + 51 = 90 \\text{ m}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Compare the left-hand sides of $(1)$ and $(2)$. Dividing $\\frac{1}{2}r^2\\theta$ by $r\\theta$ cancels $r\\theta$, leaving $\\frac{1}{2}r$, which is exactly the left side of $(3)$.', workingOut: '\\frac{\\frac{1}{2}r^2\\theta}{r\\theta} = \\frac{1}{2}r' },
      { explanation: 'Check the right-hand sides divide consistently to give $(3)$’s right side.', workingOut: '\\frac{25\\pi/6}{5\\pi/3} = \\frac{25\\pi}{6}\\times\\frac{3}{5\\pi} = \\frac{5}{2}' },
      { explanation: 'Conclude the operation performed.', workingOut: '(3) = (1) \\div (2)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'From equation $(3)$, solve $\\frac{1}{2}r = \\frac{5}{2}$ to confirm the radius.', workingOut: 'r = 5 \\text{ units}' },
      { explanation: 'Substitute $r = 5$ into the arc-length equation $r\\theta = \\frac{5\\pi}{3}$.', workingOut: '5\\theta = \\frac{5\\pi}{3}' },
      { explanation: 'Divide both sides by $5$ to solve for $\\theta$.', workingOut: '\\theta = \\frac{\\pi}{3}' }
    ]
  },

  {
    ...
    graphData: {
      geometry: {
        ...
        sideLabels: [
          { between: ["A", "B"], text: "6.4 km" },
          { between: ["B", "C"], text: "6.4 km" }
        ]
      }
    },
    ...
    solutionSteps: [
      { explanation: 'The bearing of $B$ from $A$ is $140°$ (measured clockwise from north at $A$). The north lines at $A$ and $B$ are parallel, so by alternate angles the angle between $BA$ and the north line at $B$ is $180° - 140° = 40°$.', workingOut: '\\angle ABN_1 = 180° - 140° = 40° \\quad [\\text{alternate angles}]' },
      { explanation: 'The bearing of $C$ from $B$ is $032°$, which is the angle between the north line at $B$ and $BC$.', workingOut: '\\angle N_1BC = 32°' },
      { explanation: 'Add the two angles at $B$ on either side of the north line to get the full angle $\\angle ABC$.', workingOut: '\\angle ABC = 40° + 32° = 72°' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'State the area formula for a triangle given two sides and the included angle. Here the included angle $\\angle ABC = 72°$ lies between the two equal sides.', workingOut: 'A = \\frac{1}{2}ac\\sin B' },
      { explanation: 'Substitute $a = c = 6.4$ km and $B = 72°$.', workingOut: 'A = \\frac{1}{2}(6.4)(6.4)\\sin 72°' },
      { explanation: 'Evaluate the expression.', workingOut: 'A = 19.4776\\ldots' },
      { explanation: 'Round to the nearest square kilometre.', workingOut: 'A \\approx 19 \\text{ km}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'At the points of intersection the two $y$-values are equal, so set the right-hand sides equal.', workingOut: 'x - 2 = 2 - \\frac{3}{x}' },
      { explanation: 'Multiply every term by $x$ to clear the fraction (valid since $x > 0$).', workingOut: 'x^2 - 2x = 2x - 3' },
      { explanation: 'Collect all terms on one side to form a quadratic equation in standard form.', workingOut: 'x^2 - 4x + 3 = 0' },
      { explanation: 'Factorise the quadratic.', workingOut: '(x - 3)(x - 1) = 0' },
      { explanation: 'Solve for $x$; the smaller value is the $x$-coordinate of $P$ and the larger is that of $Q$.', workingOut: 'x = 1 \\;(P), \\quad x = 3 \\;(Q)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set up the area as the integral of the upper curve minus the lower curve between the intersection points. Here $y = 2 - \\frac{3}{x}$ is above $y = x - 2$ on $1 < x < 3$.', workingOut: 'A = \\int_1^3 \\left[\\left(2 - \\frac{3}{x}\\right) - (x - 2)\\right] dx' },
      { explanation: 'Simplify the integrand by expanding the brackets.', workingOut: 'A = \\int_1^3 \\left(4 - \\frac{3}{x} - x\\right) dx' },
      { explanation: 'Find the primitive term by term, using $\\int \\frac{1}{x}\\,dx = \\ln x$.', workingOut: '= \\left[4x - 3\\ln x - \\frac{x^2}{2}\\right]_1^3' },
      { explanation: 'Substitute the upper limit $x = 3$.', workingOut: '= 12 - 3\\ln 3 - \\frac{9}{2}' },
      { explanation: 'Substitute the lower limit $x = 1$ (note $\\ln 1 = 0$).', workingOut: '- \\left(4 - 0 - \\frac{1}{2}\\right)' },
      { explanation: 'Combine the constants: $\\left(12 - \\frac{9}{2}\\right) - \\left(4 - \\frac{1}{2}\\right) = \\frac{15}{2} - \\frac{7}{2} = 4$.', workingOut: 'A = 4 - 3\\ln 3 \\text{ units}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Split the absolute value equation $|2\\cos x - 1| = 1$ into its two cases: the inside equals $+1$ or $-1$.', workingOut: '2\\cos x - 1 = 1 \\quad \\text{or} \\quad 2\\cos x - 1 = -1' },
      { explanation: 'Solve the first case: $2\\cos x - 1 = 1$ gives $\\cos x = 1$.', workingOut: '\\cos x = 1 \\Rightarrow x = 0' },
      { explanation: 'Solve the second case: $2\\cos x - 1 = -1$ gives $\\cos x = 0$.', workingOut: '\\cos x = 0 \\Rightarrow x = \\frac{\\pi}{2}' },
      { explanation: 'Check both solutions lie in the domain $0 \\leq x \\leq \\pi$ — they do.', workingOut: 'x = 0, \\; \\frac{\\pi}{2} \\in [0, \\pi] \\;\\checkmark' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find $D$, the midpoint of $AB$, using the midpoint formula $\\left(\\frac{x_1+x_2}{2}, \\frac{y_1+y_2}{2}\\right)$.', workingOut: 'D = \\left(\\frac{-1+(-3)}{2}, \\frac{16+2}{2}\\right) = (-2, 9)' },
      { explanation: 'Find $E$, the midpoint of $AC$, the same way.', workingOut: 'E = \\left(\\frac{-1+5}{2}, \\frac{16+(-2)}{2}\\right) = (2, 7)' },
      { explanation: 'Compute the gradient of $DE$ using $m = \\frac{y_2 - y_1}{x_2 - x_1}$.', workingOut: 'm_{DE} = \\frac{9 - 7}{-2 - 2} = \\frac{2}{-4} = -\\frac{1}{2}' },
      { explanation: 'Compute the gradient of $BC$.', workingOut: 'm_{BC} = \\frac{2 - (-2)}{-3 - 5} = \\frac{4}{-8} = -\\frac{1}{2}' },
      { explanation: 'Since the two gradients are equal, the lines are parallel.', workingOut: 'm_{DE} = m_{BC} \\Rightarrow DE \\parallel BC \\quad \\blacksquare' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the total number of people who were lying (the condition restricts the sample space to this group).', workingOut: '\\text{lying} = 96 + 24 = 120' },
      { explanation: 'Of those lying, count how many tested positive.', workingOut: '\\text{lying and positive} = 96' },
      { explanation: 'Form the conditional probability as the fraction of the lying group that tested positive, and simplify.', workingOut: 'P(\\text{positive} \\mid \\text{lying}) = \\frac{96}{120} = \\frac{4}{5}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the two ways the test gives a wrong result: a person who is lying but tests negative, or a person who is not lying but tests positive.', workingOut: '\\text{inaccurate} = (\\text{lying} \\cap \\text{negative}) \\cup (\\text{not lying} \\cap \\text{positive})' },
      { explanation: 'Read off and add the two counts from the table.', workingOut: '24 + 8 = 32' },
      { explanation: 'Divide by the total of $200$ people and simplify.', workingOut: 'P(\\text{inaccurate}) = \\frac{32}{200} = \\frac{4}{25}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Write the curve with a fractional index and differentiate using the power rule.', workingOut: 'y = x^{1/2} \\Rightarrow \\frac{dy}{dx} = \\frac{1}{2}x^{-1/2} = \\frac{1}{2\\sqrt{x}}' },
      { explanation: 'Evaluate the tangent gradient at $x = 4$.', workingOut: '\\left.\\frac{dy}{dx}\\right|_{x=4} = \\frac{1}{2\\sqrt{4}} = \\frac{1}{4}' },
      { explanation: 'The normal is perpendicular to the tangent, so its gradient is the negative reciprocal.', workingOut: 'm_{\\text{normal}} = -4' },
      { explanation: 'Find the point of contact by substituting $x = 4$ into the curve.', workingOut: 'y = \\sqrt{4} = 2 \\Rightarrow (4, 2)' },
      { explanation: 'Use point–gradient form for the normal line.', workingOut: 'y - 2 = -4(x - 4)' },
      { explanation: 'Expand and rearrange into general form $ax + by + c = 0$.', workingOut: 'y - 2 = -4x + 16 \\Rightarrow 4x + y - 18 = 0' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the change-of-base rule $\\log_b a = \\frac{\\log_c a}{\\log_c b}$, choosing base $c = 2$.', workingOut: '\\log_x 2 = \\frac{\\log_2 2}{\\log_2 x}' },
      { explanation: 'Simplify the numerator using $\\log_2 2 = 1$.', workingOut: '= \\frac{1}{\\log_2 x} \\quad \\blacksquare' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Substitute the result from part (a), $\\log_x 2 = \\frac{1}{\\log_2 x}$, into the equation.', workingOut: '\\log_2 x = 4 \\times \\frac{1}{\\log_2 x}' },
      { explanation: 'Multiply both sides by $\\log_2 x$ to clear the fraction.', workingOut: '(\\log_2 x)^2 = 4' },
      { explanation: 'Take square roots, remembering both signs.', workingOut: '\\log_2 x = 2 \\quad \\text{or} \\quad \\log_2 x = -2' },
      { explanation: 'Convert each logarithmic equation back to index form to solve for $x$.', workingOut: 'x = 2^2 = 4 \\quad \\text{or} \\quad x = 2^{-2} = \\frac{1}{4}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Integrate $f\'(x)$ term by term to recover $f(x)$, adding a constant of integration $C$.', workingOut: 'f(x) = 2x^3 + \\frac{5}{2}x^2 - x + C' },
      { explanation: 'Substitute the known value $f(-1) = 5$ to set up an equation for $C$.', workingOut: '2(-1)^3 + \\frac{5}{2}(-1)^2 - (-1) + C = 5' },
      { explanation: 'Simplify the left side.', workingOut: '-2 + \\frac{5}{2} + 1 + C = 5 \\Rightarrow \\frac{3}{2} + C = 5' },
      { explanation: 'Solve for $C$.', workingOut: 'C = 5 - \\frac{3}{2} = \\frac{7}{2}' },
      { explanation: 'Write the final expression for $f(x)$.', workingOut: 'f(x) = 2x^3 + \\frac{5}{2}x^2 - x + \\frac{7}{2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $f(x) = 4x^3 - x^4$ and factorise to find where the gradient is zero.', workingOut: 'f\'(x) = 12x^2 - 4x^3 = 4x^2(3 - x)' },
      { explanation: 'Set $f\'(x) = 0$ and solve for the $x$-values of the stationary points.', workingOut: '4x^2(3 - x) = 0 \\Rightarrow x = 0 \\text{ or } x = 3' },
      { explanation: 'Find the corresponding $y$-values by substituting into $f$.', workingOut: 'f(0) = 0, \\quad f(3) = 4(27) - 81 = 27' },
      { explanation: 'Find the second derivative to test the nature of each point.', workingOut: 'f\'\'(x) = 24x - 12x^2' },
      { explanation: 'At $x = 0$, $f\'\'(0) = 0$, so the test is inconclusive. A sign table of $f\'$ around $x = 0$ shows $f\'>0$ on both sides ($f\'(-1) = 16$, $f\'(1) = 8$), so the gradient does not change sign — a horizontal point of inflection.', workingOut: '(0, 0): \\text{ horizontal point of inflection}' },
      { explanation: 'At $x = 3$, $f\'\'(3) = 72 - 108 = -36 < 0$, indicating concave down, so this is a maximum.', workingOut: '(3, 27): \\text{ maximum turning point}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Find the $x$-intercepts by factorising $f(x) = 4x^3 - x^4 = x^3(4 - x)$ and setting it to zero.', workingOut: 'x^3(4 - x) = 0 \\Rightarrow x = 0,\\ 4' },
      { explanation: 'Recall the key features from part (a): a horizontal point of inflection at $(0,0)$ and a maximum at $(3,27)$.', workingOut: '(0,0)\\text{ horizontal POI}, \\quad (3,27)\\text{ max}' },
      { explanation: 'Note the end behaviour: the leading term $-x^4$ dominates, so $f(x) \\to -\\infty$ as $x \\to \\pm\\infty$.', workingOut: 'x \\to \\pm\\infty \\Rightarrow f(x) \\to -\\infty' },
      { explanation: 'Sketch the curve through all the key points with the correct shape.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 400,
            height: 320,
            boundingbox: [-1.5, 33, 5, -14],
            boardOptions: { keepaspectratio: false },
...

  {
    ...
    solutionSteps: [
      { explanation: 'Apply the first property of a probability distribution: all probabilities must sum to $1$.', workingOut: '0.3 + 0.45 + a + b = 1 \\Rightarrow a + b = 0.25' },
      { explanation: 'Apply the mean (expected value) formula $E(X) = \\sum x\\,P(x) = 2$, computing the known terms $1(0.3) + 2(0.45) = 1.2$.', workingOut: '0.3 + 0.9 + 3a + 4b = 2' },
      { explanation: 'Simplify to the second equation.', workingOut: '3a + 4b = 0.8' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Multiply the first equation $a + b = 0.25$ by $3$ so the $a$-terms match the second equation.', workingOut: '3a + 3b = 0.75' },
      { explanation: 'Subtract this from $3a + 4b = 0.8$ to eliminate $a$ and solve for $b$.', workingOut: '(3a + 4b) - (3a + 3b) = 0.8 - 0.75 \\Rightarrow b = 0.05' },
      { explanation: 'Substitute $b = 0.05$ back into $a + b = 0.25$.', workingOut: 'a = 0.25 - 0.05 = 0.2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify this as a product of two functions and set up the product rule $\\frac{dy}{dx} = uv\' + vu\'$.', workingOut: 'u = x, \\quad v = \\sin 2x' },
      { explanation: 'Differentiate each factor. The derivative of $\\sin 2x$ uses the chain rule, giving $2\\cos 2x$.', workingOut: 'u\' = 1, \\quad v\' = 2\\cos 2x' },
      { explanation: 'Apply the product rule and substitute.', workingOut: '\\frac{dy}{dx} = x(2\\cos 2x) + \\sin 2x(1)' },
      { explanation: 'Simplify.', workingOut: '\\frac{dy}{dx} = 2x\\cos 2x + \\sin 2x' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Substitute $g(x) = \\ln(x-2)$ into $f(x) = e^x$ wherever $x$ appears.', workingOut: 'f(g(x)) = e^{\\ln(x-2)}' },
      { explanation: 'Simplify using the inverse relationship $e^{\\ln A} = A$.', workingOut: 'f(g(x)) = x - 2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the domain restriction inherited from $g(x) = \\ln(x-2)$: the logarithm requires a positive argument.', workingOut: 'x - 2 > 0 \\Rightarrow x > 2' },
      { explanation: 'Apply this restriction to the simplified composite $y = x - 2$. As $x$ ranges over $(2, \\infty)$, compute the resulting $y$-values.', workingOut: 'x > 2 \\Rightarrow y = x - 2 > 0' },
      { explanation: 'State the range in interval notation.', workingOut: '\\text{Range} = (0, \\infty)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Enter the eleven $(x, y)$ pairs for students A–K into the statistics mode of a calculator and read off Pearson’s correlation coefficient $r$.', workingOut: 'r \\approx 0.9' },
      { explanation: 'Interpret the value: $r$ near $+1$ indicates a strong, positive linear relationship.', workingOut: 'r \\approx 0.9 \\Rightarrow \\text{strong positive correlation}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Use the linear regression function on the calculator to obtain the gradient and $y$-intercept of the least-squares line $y = bx + a$.', workingOut: 'b = 0.72, \\quad a = 18 \\Rightarrow y = 0.72x + 18' },
      { explanation: 'Substitute student L’s English mark $x = 80$ into the regression equation.', workingOut: 'y = 0.72(80) + 18' },
      { explanation: 'Evaluate and round to a whole mark.', workingOut: 'y = 75.6 \\approx 76' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Identify the quotient and set up the quotient rule $\\frac{dy}{dx} = \\frac{vu\' - uv\'}{v^2}$.', workingOut: 'u = e^x, \\quad v = x + 1' },
      { explanation: 'Differentiate the numerator and denominator.', workingOut: 'u\' = e^x, \\quad v\' = 1' },
      { explanation: 'Substitute into the quotient rule.', workingOut: '\\frac{dy}{dx} = \\frac{(x+1)e^x - e^x(1)}{(x+1)^2}' },
      { explanation: 'Factor $e^x$ from the numerator and simplify.', workingOut: '= \\frac{e^x(x + 1 - 1)}{(x+1)^2} = \\frac{xe^x}{(x+1)^2}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Integrate $\\cos 4x$. The primitive of $\\cos(ax)$ is $\\frac{1}{a}\\sin(ax)$, with $a = 4$.', workingOut: '\\int \\cos 4x\\,dx = \\frac{1}{4}\\sin 4x' },
      { explanation: 'Integrate $-\\sin x$. Since $\\int \\sin x\\,dx = -\\cos x$, the negative makes it $+\\cos x$.', workingOut: '\\int -\\sin x\\,dx = \\cos x' },
      { explanation: 'Combine the results and add the constant of integration.', workingOut: '\\frac{1}{4}\\sin 4x + \\cos x + C' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Recognise the series as geometric: each term is the previous one multiplied by $\\sin^2 x$. Identify the first term and common ratio.', workingOut: 'a = \\sin^2 x, \\quad r = \\sin^2 x' },
      { explanation: 'Since $0 < x < \\frac{\\pi}{2}$, $0 < \\sin^2 x < 1$, so a limiting sum exists. Apply $S_\\infty = \\frac{a}{1 - r}$.', workingOut: 'S_\\infty = \\frac{\\sin^2 x}{1 - \\sin^2 x}' },
      { explanation: 'Use the Pythagorean identity $1 - \\sin^2 x = \\cos^2 x$ in the denominator.', workingOut: '= \\frac{\\sin^2 x}{\\cos^2 x}' },
      { explanation: 'Simplify to a single trig function.', workingOut: '= \\tan^2 x' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Write the total surface area of the closed box: two faces of each pair (top/bottom, two sides, two ends).', workingOut: 'A = 2(2x \\cdot x) + 2(x \\cdot h) + 2(2x \\cdot h)' },
      { explanation: 'Simplify the surface-area expression.', workingOut: 'A = 4x^2 + 2xh + 4xh = 4x^2 + 6xh' },
      { explanation: 'Use the fixed volume to express $h$ in terms of $x$.', workingOut: 'V = 2x^2 h = 9 \\Rightarrow h = \\frac{9}{2x^2}' },
      { explanation: 'Substitute $h$ into the surface-area formula.', workingOut: 'A = 4x^2 + 6x \\cdot \\frac{9}{2x^2}' },
      { explanation: 'Simplify the second term.', workingOut: 'A = 4x^2 + \\frac{27}{x}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $A = 4x^2 + 27x^{-1}$ with respect to $x$.', workingOut: 'A\' = 8x - 27x^{-2} = 8x - \\frac{27}{x^2}' },
      { explanation: 'Set $A\' = 0$ to find the stationary point.', workingOut: '8x - \\frac{27}{x^2} = 0 \\Rightarrow 8x^3 = 27' },
      { explanation: 'Solve for $x$.', workingOut: 'x^3 = \\frac{27}{8} \\Rightarrow x = \\frac{3}{2}' },
      { explanation: 'Confirm it is a minimum using the second derivative.', workingOut: 'A\'\' = 8 + \\frac{54}{x^3}; \\quad A\'\'(1.5) = 24 > 0 \\Rightarrow \\text{min}' },
      { explanation: 'Substitute $x = 1.5$ into $A$ to find the minimum area.', workingOut: 'A = 4(1.5)^2 + \\frac{27}{1.5} = 9 + 18 = 27 \\text{ m}^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Determine the horizontal and vertical shifts from the vertex moving from $(0,0)$ to $(-1,-3)$: $1$ unit left and $3$ units down.', workingOut: 'x \\to x + 1 \\;(b = 1), \\quad c = -3' },
      { explanation: 'Write the partial form with the shifts in place.', workingOut: 'g(x) = k\\,f(x + 1) - 3' },
      { explanation: 'Use the known point $(0, -5)$ on the graph, noting $f(1) = 1^2 = 1$.', workingOut: '-5 = k\\,f(1) - 3 = k(1) - 3' },
      { explanation: 'Solve for the vertical dilation factor $k$.', workingOut: '-5 = k - 3 \\Rightarrow k = -2' },
      { explanation: 'State the final transformed function.', workingOut: 'g(x) = -2f(x + 1) - 3' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Isolate the sine function.', workingOut: '\\sin 3\\theta = \\frac{1}{2}' },
      { explanation: 'Expand the domain to match the argument $3\\theta$: multiply the bounds of $\\theta$ by $3$.', workingOut: '0 \\leq \\theta \\leq \\pi \\Rightarrow 0 \\leq 3\\theta \\leq 3\\pi' },
      { explanation: 'Sine is $\\frac{1}{2}$ at the reference angle $\\frac{\\pi}{6}$, in the 1st and 2nd quadrants. List all values of $3\\theta$ in $[0, 3\\pi]$ by adding $2\\pi$ to each base solution.', workingOut: '3\\theta = \\frac{\\pi}{6}, \\frac{5\\pi}{6}, \\frac{13\\pi}{6}, \\frac{17\\pi}{6}' },
      { explanation: 'Check that the next candidates $\\frac{25\\pi}{6}$ and $\\frac{29\\pi}{6}$ exceed $3\\pi$ and are rejected.', workingOut: '\\frac{25\\pi}{6}, \\frac{29\\pi}{6} > 3\\pi \\;\\text{(rejected)}' },
      { explanation: 'Divide every solution by $3$ to find $\\theta$.', workingOut: '\\theta = \\frac{\\pi}{18}, \\frac{5\\pi}{18}, \\frac{13\\pi}{18}, \\frac{17\\pi}{18}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Describe $y = \\cos\\pi x$: amplitude $1$, period $\\frac{2\\pi}{\\pi} = 2$. It equals $1$ at $x = 0, \\pm 2$ and $-1$ at $x = \\pm 1, \\pm 3$.', workingOut: 'y = \\cos\\pi x: \\text{ period } 2, \\text{ amplitude } 1' },
      { explanation: 'Describe $y = 1 - |x|$: a V-shape opening downward with apex at $(0, 1)$ and gradient $\\mp 1$.', workingOut: 'y = 1 - |x|: \\text{ apex } (0,1), \\text{ x-ints } (\\pm 1, 0)' },
      { explanation: 'Draw both on the same axes over $-3 \\leq x \\leq 3$.', workingOut: '',
        graphData: {
          jsxGraph: {
            width: 440,
            height: 300,
...

  {
    ...
    solutionSteps: [
      { explanation: 'Each solution of the equation corresponds to a point where the two graphs cross. Use the diagram from part (a).', workingOut: '\\cos\\pi x = 1 - |x| \\iff \\text{graphs intersect}' },
      { explanation: 'Note that $\\cos\\pi x \\in [-1, 1]$, while $1 - |x| < -1$ once $|x| > 2$. So all intersections must lie in $-2 \\leq x \\leq 2$.', workingOut: '1 - |x| < -1 \\text{ for } |x| > 2' },
      { explanation: 'Count the intersection points within this region from the sketch.', workingOut: '\\text{5 points of intersection} \\Rightarrow 5 \\text{ solutions}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The integral equals the signed area: regions above the $x$-axis are positive, below are negative. Break the path into its four geometric pieces.', workingOut: '\\int_0^8 f\\,dx = [AB] + [BC] + [CD] + [DE]' },
      { explanation: 'Quarter circle $AB$ has radius $2$ and lies above the axis.', workingOut: '[AB] = +\\tfrac{1}{4}\\pi(2)^2 = \\pi' },
      { explanation: 'Triangle $BC$ from $(2,0)$ to $(4,-2)$ lies below the axis: base $2$, height $2$.', workingOut: '[BC] = -\\tfrac{1}{2}(2)(2) = -2' },
      { explanation: 'Rectangle $CD$ from $(4,-2)$ to $(6,-2)$ lies below: width $2$, height $2$.', workingOut: '[CD] = -(2)(2) = -4' },
      { explanation: 'Quarter circle $DE$ has radius $2$ and lies below the axis.', workingOut: '[DE] = -\\tfrac{1}{4}\\pi(2)^2 = -\\pi' },
      { explanation: 'Add the four signed areas; the $\\pi$ terms cancel.', workingOut: '\\pi - 2 - 4 - \\pi = -6' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Recall that differentiability requires the curve to be smooth — no sharp corners or breaks — so the tangent gradient is well defined.', workingOut: '\\text{differentiable} \\Rightarrow \\text{smooth and continuous}' },
      { explanation: 'Locate the corners on the graph: where the quarter circle meets the line at $B$, and where the line meets the horizontal segment at $C$. At $D$ the horizontal segment meets the quarter circle smoothly (tangent), so it is differentiable there.', workingOut: '\\text{corners at } B(2,0) \\text{ and } C(4,-2)' },
      { explanation: 'State the $x$-values where the function is not differentiable.', workingOut: 'x = 2 \\text{ and } x = 4' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      { explanation: 'Differentiate $y = \\tan^2 x$ using the chain rule, then rewrite $\\sec^2 x = 1 + \\tan^2 x$ to express in powers of $\\tan x$.', workingOut: 'y\' = 2\\tan x\\sec^2 x = 2\\tan x(1+\\tan^2 x) = 2\\tan x + 2\\tan^3 x' },
      { explanation: 'Differentiate again term by term. $\\frac{d}{dx}(2\\tan x) = 2\\sec^2 x$ and $\\frac{d}{dx}(2\\tan^3 x) = 6\\tan^2 x\\sec^2 x$.', workingOut: 'y\'\' = 2\\sec^2 x + 6\\tan^2 x\\sec^2 x' },
      { explanation: 'Replace each $\\sec^2 x$ with $1 + \\tan^2 x$.', workingOut: '= 2(1+\\tan^2 x) + 6\\tan^2 x(1+\\tan^2 x)' },
      { explanation: 'Expand and collect like terms in powers of $\\tan x$.', workingOut: '= 2 + 2\\tan^2 x + 6\\tan^2 x + 6\\tan^4 x = 6\\tan^4 x + 8\\tan^2 x + 2' },
      { explanation: 'Since $y = \\tan^2 x$, we have $\\tan^4 x = y^2$. Match coefficients with $ay^2 + by + 2$.', workingOut: 'y\'\' = 6y^2 + 8y + 2 \\Rightarrow a = 6, \\; b = 8' }
    ]
  },
...

  {
    ...
    solutionSteps: [
      ...
      { explanation: 'Since $9 < t < 10$, this occurs during the $10^{\\text{th}}$ year.', workingOut: '\\text{during the } 10^{\\text{th}} \\text{ year}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Sum the geometric series in the braces (first term $1$, ratio $1.0025$, $n$ terms).', workingOut: '1 + 1.0025 + \\cdots + (1.0025)^{n-1} = \\frac{(1.0025)^n - 1}{0.0025}' },
      { explanation: 'Substitute into $A_n$ and set it equal to half the initial value, $300\\,000$.', workingOut: '300\\,000 = 600\\,000(1.0025)^n - 1\\,600\\,000((1.0025)^n - 1)' },
      { explanation: 'Expand and collect the $(1.0025)^n$ terms.', workingOut: '300\\,000 = 1\\,600\\,000 - 1\\,000\\,000(1.0025)^n' },
      { explanation: 'Rearrange to isolate the exponential.', workingOut: '1\\,000\\,000(1.0025)^n = 1\\,300\\,000 \\Rightarrow (1.0025)^n = 1.3' },
      { explanation: 'Take logarithms of both sides and solve for $n$.', workingOut: 'n = \\frac{\\ln 1.3}{\\ln 1.0025} \\approx 105.07' },
      { explanation: 'Round up to the nearest whole month and convert to years and months.', workingOut: 'n \\approx 105 \\text{ months} \\approx 8 \\text{ years } 9 \\text{ months}' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Integrate the acceleration to find velocity, adding a constant.', workingOut: 'v = \\int(6t - 12)\\,dt = 3t^2 - 12t + C' },
      { explanation: 'Apply the initial condition "at rest" ($v = 0$) at $t = 0$ to find $C$.', workingOut: 't = 0,\\ v = 0 \\Rightarrow C = 0 \\Rightarrow v = 3t^2 - 12t' },
      { explanation: 'Integrate the velocity to find displacement, adding a constant.', workingOut: 'x = \\int(3t^2 - 12t)\\,dt = t^3 - 6t^2 + C_1' },
      { explanation: 'Apply the initial condition "at $O$" ($x = 0$) at $t = 0$ to find $C_1$.', workingOut: 't = 0,\\ x = 0 \\Rightarrow C_1 = 0 \\Rightarrow x = t^3 - 6t^2' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'The particle is at rest when its velocity is zero. Set $v = 0$ and factorise.', workingOut: '3t^2 - 12t = 0 \\Rightarrow 3t(t - 4) = 0' },
      { explanation: 'Solve for $t$. Since $t = 0$ is the initial instant, the next rest is at $t = 4$.', workingOut: 't = 0 \\text{ or } t = 4 \\Rightarrow \\text{next at } t = 4 \\text{ s}' },
      { explanation: 'Substitute $t = 4$ into the displacement to find the position.', workingOut: 'x = 4^3 - 6(4)^2 = 64 - 96 = -32' },
      { explanation: 'Interpret the negative sign: the particle is $32$ m to the left of the origin.', workingOut: 'x = -32 \\text{ m (32 m left of } O)' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set up the area as the integral of the curve from $0$ to $k$ (the curve is positive for $x > 0$).', workingOut: 'A = \\int_0^k \\frac{e^x - e^{-x}}{e^x + e^{-x}}\\,dx' },
      { explanation: 'Notice the numerator is exactly the derivative of the denominator, so the integral has the form $\\int \\frac{f\'(x)}{f(x)}\\,dx = \\ln|f(x)|$.', workingOut: '\\frac{d}{dx}(e^x + e^{-x}) = e^x - e^{-x}' },
      ...
      { explanation: 'Combine the logarithms using the quotient law.', workingOut: '= \\ln\\!\\left(\\frac{e^k + e^{-k}}{2}\\right) \\quad \\blacksquare' }
    ]
  },

  {
    ...
    solutionSteps: [
      { explanation: 'Set the area expression equal to $1$.', workingOut: '\\ln\\!\\left(\\frac{e^k + e^{-k}}{2}\\right) = 1' },
      { explanation: 'Exponentiate both sides (apply $e^{(\\cdot)}$) and multiply out.', workingOut: '\\frac{e^k + e^{-k}}{2} = e \\Rightarrow e^k + e^{-k} = 2e' },
      { explanation: 'Multiply through by $e^k$ to clear the negative exponent and rearrange into a quadratic in $e^k$.', workingOut: '(e^k)^2 - 2e\\,e^k + 1 = 0' },
      { explanation: 'Let $u = e^k$ and apply the quadratic formula.', workingOut: 'u = \\frac{2e \\pm \\sqrt{4e^2 - 4}}{2} = e \\pm \\sqrt{e^2 - 1}' },
      { explanation: 'Since $k > 0$ we need $u = e^k > 1$. The root $e - \\sqrt{e^2-1} \\approx 0.19 < 1$ gives $k < 0$ and is rejected; keep the larger root.', workingOut: 'u = e + \\sqrt{e^2 - 1}' },
      { explanation: 'Take logarithms to solve for $k$.', workingOut: 'k = \\ln\\!\\left(e + \\sqrt{e^2 - 1}\\right)' }
    ]
  },
