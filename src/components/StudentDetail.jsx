...
const StudentDetail = ({ studentId, onBack }) => {
  ...
  // 1. Independent Student Data Listener
  useEffect(() => {
    if (!studentId) return;
  ...
  // 2. Independent Stats Listener
  useEffect(() => {
    if (!studentId || !student?.id) return;
    
    const colName = student.source === 'manual' ? 'students' : 'users';
    const unsubStats = onSnapshot(collection(db, colName, studentId, 'daily_stats'), (snap) => {
  ...
  if (!student) return <div className="app-empty">Student data not found.</div>;
  ...
  let chaptersList = CURRICULUM_DATA[currentYear] || [];
  if (!Array.isArray(chaptersList)) {
    chaptersList = chaptersList[currentCourse] || [];
  }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center', background: '#f8fafc', padding: '8px 16px', borderRadius: '20px', border: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <select 
                    ...
                    style={{ background: 'transparent', border: 0, fontWeight: 800, color: '#6366f1', cursor: 'pointer', outline: 'none' }}
                  >
                    {Object.keys(CURRICULUM_DATA).map(yr => <option key={yr} value={yr}>{yr}</option>)}
                  </select>
                </div>
              </div>
            </div>
      ...
      case 'schedule':
        return (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={styles.card} className="profile-card-mobile">
            <div className="section-title" style={{ fontSize: '0.75rem', fontWeight: 800, color: '#94a3b8', letterSpacing: '0.1em', marginBottom: '20px', textTransform: 'uppercase' }}>SCHEDULE</div>
            <p style={{ color: '#64748b' }}>Schedule management is currently active.</p>
          </motion.div>
        );
      case 'challenge':
        return (
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="student-detail-container" style={{ padding: '20px' }}>
