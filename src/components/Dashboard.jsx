...
import { Plus, Search, Bell, Users, Clock, CheckCircle2, GraduationCap, LogOut, Calendar, BookOpen } from 'lucide-react';
...
const Dashboard = ({ students, onAddStudent, onSelectStudent, setActiveTab }) => {
  ...
  const handleScheduleClick = () => {
    console.log("Schedule Lesson button clicked");
    if (setActiveTab) {
      setActiveTab('Schedule');
    } else {
      console.error("setActiveTab prop is missing in Dashboard");
    }
  };
        ...
        {!isAdmin && (
          <div className="vision-card-wrapper" style={{ marginBottom: '24px' }}>
            <div className="app-panel vision-card" style={{ 
              ...
              backgroundSize: 'cover', backgroundPosition: 'center', height: '460px', borderRadius: '36px', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '48px', color: 'white', overflow: 'hidden', boxShadow: '0 30px 70px rgba(0,0,0,0.25)', border: 'none'
            }}>
              <div className="vision-card__content" style={{ zIndex: 1 }}>
                <p style={{ fontFamily: "'Poppins', sans-serif", fontSize: '1.1rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '8px', textShadow: '0 2px 10px rgba(0,0,0,0.4)' }}>
                  {getGreeting()}
                </p>
                <h1 style={{ fontFamily: "'Poppins', sans-serif", fontSize: '3.2rem', fontWeight: 900, margin: 0, lineHeight: 1.0, color: '#1a1c2c', letterSpacing: '-0.03em', textShadow: '-1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff, 0 4px 20px rgba(0,0,0,0.2)' }}>
                  {userName}
                </h1>
                {profile?.dreamJob && (
                  <div style={{ marginTop: '16px', padding: '6px 14px', background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)', borderRadius: '12px', display: 'inline-block', border: '1px solid rgba(255,255,255,0.2)', boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
                    <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 800, fontSize: '0.82rem', color: 'white', letterSpacing: '0.08em', textShadow: '0 1px 3px rgba(0,0,0,0.3)' }}>
                ...
                <div className="activity-list">
                  {students.length > 0 ? (
                    students.slice(0, 5).map(student => (
                      <StudentRow key={student.id} {...student} onClick={() => onSelectStudent(student.id)} />
                    ))
                  ) : (
                    <div className="app-empty">No students added yet.</div>
                  )}
                </div>
              </div>
              ...
              <div className="app-page-column">
                <div className="app-panel dashboard-card">
                  ...
                  <div className="app-action-buttons" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <button onClick={onAddStudent} className="app-button app-button--primary">
                      <Plus size={20} /> Add New Student
                    </button>
                    <button onClick={handleScheduleClick} className="app-button app-button--secondary">
                      <Calendar size={20} /> Schedule Lesson
                    </button>
                    <button className="app-button app-button--secondary">
                      <BookOpen size={20} /> Record Progress
                    </button>
                  </div>
                </div>
