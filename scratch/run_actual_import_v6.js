...
console.log('=== STARTING FIRESTORE IMPORT PROCESS (V6 UNIFIED) ===');
...
const questions8A = JSON.parse(readFileSync('scratch/questions8A.json', 'utf8'));
...
async function runImport() {
  ...
  // ==========================================
  // D. YEAR 11 ADVANCED CHAPTER 8A (INDICES)
  // ==========================================
  console.log('\n--- POPULATING YEAR 11 ADVANCED CHAPTER 8A (INDICES) ---');
  
  const ch8Snap = await db.collection('questions')
    .where('chapterId', '==', 'y11a-8')
    .where('topicCode', '==', '8A')
    .get();
  
  console.log(`Found ${ch8Snap.size} existing questions in 8A. Deleting...`);
  
  const ch8Batch = db.batch();
  ch8Snap.forEach(doc => ch8Batch.delete(doc.ref));
  await ch8Batch.commit();
  console.log('Chapter 8A clean completed.');

  console.log(`Uploading ${questions8A.length} questions to 8A...`);
  let count8 = 0;
  ...
  console.log(`Successfully synced ${count8} questions for Year 11 Chapter 8A!`);
