...
const figure = (gd) => {
  ...
  // legacy bare jsxgraph config at top level — kept verbatim under `raw` (MathView only renders
  // `.jsxGraph`, so these do not draw today; wrapping them would change what students see)
...
// The app's own resolver (src/utils/mcOptionShuffle.js) decides what "the correct option" is
// today from `answer`/`a` + `isManual` — so the published index is, by construction, exactly what
// students are graded against now. `options[].isCorrect` is ignored by the app; it is only a
// last resort here when the app's resolver finds nothing.
const resolveMcAnswer = (opts, raw) => {
  const options = opts.map((o) => (o && typeof o === 'object' ? { text: str(o.text) } : { text: str(o) }));
  const cands = [raw.answer, raw.a, raw.correctAnswer, raw.answerStr].filter((x) => x != null && x !== '');
  for (const a of cands) {
    try { const i = resolveCorrectOptionIndex({ answer: a, isManual: raw.isManual !== false }, options); if (i >= 0) return i; } catch { /* fall through */ }
  }
  const ic = opts.findIndex((o) => o && typeof o === 'object' && o.isCorrect === true);
  return ic >= 0 ? ic : null;
};
...
const merge = (id) => {
  ...
  const fsMc = f.type === 'multiple_choice' || f.type === 'mcq';
  if (fsMc && !f.subQuestions?.length && !(f.options || []).length && sOpts.length) { m.options = sOpts; m.answer = s.a ?? s.answer; m.a = undefined; m.correctAnswer = undefined; m.answerStr = undefined; m.isManual = s.isManual; bump('options', 'seed(fs empty)'); }
  // MC answer conflict detection
  const fo = (f.options || []).map(optText), so = (s.opts || s.options || []).map(optText);
  if (fo.length && so.length) {
    const fi = resolveMcAnswer(f.options, f), si = resolveMcAnswer(s.opts || s.options, s);
...
const canon = (raw, id, depth = 0) => {
  ...
  const q = { id, type };
  const diff = str(raw.difficulty).toLowerCase(); if (['easy', 'medium', 'hard'].includes(diff)) q.difficulty = diff;
  ...
  if (type === 'mc') {
    ...
    q.options = rich
      ...
      : opts.map(optText);
    const ai = resolveMcAnswer(opts, raw);
    ...
    // parents sometimes carry options too; an MC parent (options + sub-parts) is flagged `mc`
    const pOpts = raw.options?.length ? raw.options : (raw.opts || []);
    if (pOpts.length) q.options = pOpts.map(optText);
    if ((raw.type === 'multiple_choice' || raw.type === 'mcq') && pOpts.length) {
      q.mc = true;
      const ai = resolveMcAnswer(pOpts, raw); if (ai != null) q.answer = ai;
    } else if (nz(raw.answer ?? raw.a)) q.answer = str(raw.answer ?? raw.a);
  } else {
    ...
    // some review/short docs carry options too (converted-but-not-retyped); keep them verbatim
    const oOpts = raw.options?.length ? raw.options : (raw.opts || []);
    if (oOpts.length) q.options = oOpts.map(optText);
  }
...
for (const id of ids) {
  ...
  let topicId = str(m.topicId); let synthetic = false; if (!topicId) { topicId = chapterId; synthetic = true; R.orphanTopic++; }
  const q = canon(m, id);
  ...
  if (!ch.topics.has(topicId)) ch.topics.set(topicId, { topicId, ...(synthetic ? { synthetic: true } : {}), code: str(m.topicCode ?? m.c), title: str(m.topicTitle ?? m.t), questions: [] });
