...
// y8-5h-q2f_alpha and beta:
// Recalculating P (2alpha), R interior (alpha), and R exterior (beta) to have perfectly uniform radii and curve inwards.
const updates = {
  'y8-5h-q2f_alpha': `<svg viewBox="0 0 400 180" width="100%" height="180" style="background:#f8fafc; border-radius:16px; display:block; margin:0 auto;">
  ...
  <!-- Angles -->
  <!-- P: 2alpha (curves inwards at r=18, sweep-flag 0) -->
  <path d="M 120 48 A 18 18 0 0 0 135 40" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
  <text x="135" y="58" text-anchor="middle" fill="#f43f5e" font-size="13" font-weight="900" font-family="'Outfit'">2α</text>
  
  <!-- R interior: alpha (curves inwards at r=22, sweep-flag 0) -->
  <path d="M 252 118 A 22 22 0 0 0 248 130" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
  <text x="238" y="122" text-anchor="middle" fill="#f43f5e" font-size="14" font-weight="900" font-family="'Outfit'">α</text>
</svg>`,
  ...
  <!-- R exterior: beta (perfectly matched at r=22, curves inwards with sweep-flag 1) -->
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Updating triangle Q/R sweep flags and uniform arcs for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully completed uniform Q/R arc correction!');
  process.exit(0);
}
