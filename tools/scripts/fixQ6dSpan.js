...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // HOTFIX: Q6D-ALPHA and Q6D-BETA 105 DEGREE SPAN
    if (q.id === 'y9-7a-q6d-alpha' || q.id === 'y9-7a-q6d-beta') {
      geo.angles = [
        { at: 'P', label: 'α' },
        { at: 'Q' },
        { at: 'Q' },
        { at: 'Q', label: '105°', span: 2 },
        { at: 'S', label: 'β' }
      ];
    }
...
async function run() {
  const ids = ['y9-7a-q6d-alpha', 'y9-7a-q6d-beta'];
  ...
  for (const id of ids) {
    ...
    console.log(`Successfully fixed spanning angle 105° for ${id}!`);
  }
