...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // Intersection Snapping for all q3
  ...
  if (geo.segments) {
    for (const seg of geo.segments) {
      ...
      if (p1 && p2) {
        ...
        // Explicit, visible <g> transform for parallel markers!
        if (seg.marks) {
...
async function run() {
  ...
  snapshot.forEach(doc => {
    const id = doc.id;
    // We update all y9-7a to make sure any marks/fixes apply globally!
  ...
  console.log(`Prepared ${count} questions to render robust parallel markers without transparent strokes.`);
  if (count > 0) {
    await batch.commit();
