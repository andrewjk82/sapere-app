...
(async () => {
  try {
    const snap = await db.collection('questions').where('topicId', '==', 'y10-4d').get();
    console.log(`Total questions in topic y10-4d: ${snap.size}`);
    ...
    docs.forEach((doc, i) => {
      ...
      console.log(`  question: "${data.question ? data.question.substring(0, 100) : ''}"`);
