...
function geometryToSvg(geo) {
  ...
  for(const vKey in abv){
    ...
    labelItems.forEach((angItem,idx)=>{
      ...
      const rArc = geo.rArc || 24;
      const rLabel = geo.rLabel || 42;
...
const geo = {
  ...
  segments: [
    { from:'A', to:'B', arrow:true, marks:1 },
    { from:'C', to:'D', arrow:true, marks:1 },
    { from:'B', to:'F' },
    { from:'E', to:'F', tick:1 },
    { from:'E', to:'D', tick:1 },
    { from:'D', to:'F' }
  ],
  // Swapped labels to match database answers and logic:
  // - α is at E (top-right, interior) -> 105 degrees.
  // - β is at E (bottom-left) -> 105 degrees.
  // - γ is at F (right vertex, base angle of isosceles) -> 37.5 degrees.
  angles: [
    { at:'B' },
    { at:'B', label:'75°' },
    { at:'E' },
    { at:'E', label:'β' }, // Sector 1 (bottom-left)
    { at:'E' },
    { at:'E', label:'α' }, // Sector 3 (top-right, interior)
    { at:'F', label:'γ' } // Sector 0 (right vertex)
  ]
};

async function run() {
  const ids = ['y9-7a-q8g-alpha', 'y9-7a-q8g-beta', 'y9-7a-q8g-gamma'];
  ...
  for (const id of ids) {
    ...
    console.log(`Successfully updated swapped labels for ${id}!`);
  }
}
