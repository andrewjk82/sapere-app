...
(async () => {
  const snapL = await db.collection('questions').where('topicId', '==', 'y7-4l').get();
  let invalidCount = 0;
  
  snapL.docs.forEach(doc => {
    const data = doc.data();
  ...
  console.log('Final 4L validation audit invalid counts:', invalidCount);
  process.exit(0);
})();
