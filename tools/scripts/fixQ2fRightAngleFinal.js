...
const geo = {
  points: { O:[0,0], P:[0,2], Q:[2*Math.cos(60*Math.PI/180), 2*Math.sin(60*Math.PI/180)], R:[2*Math.cos(40*Math.PI/180), 2*Math.sin(40*Math.PI/180)], S:[2,0] },
  segments: [
    { from:'O', to:'P', arrow:true },
    { from:'O', to:'S', arrow:true },
    { from:'O', to:'Q', arrow:true },
    { from:'O', to:'R', arrow:true }
  ],
  angles: [
    { at:'O' }, // Sector 0 (S to P) - empty
    { at:'O', label:'3α' }, // Sector 1 (P to Q)
    { at:'O', label:'2α' }, // Sector 2 (Q to R)
    { at:'O', label:'4α' }, // Sector 3 (R to S)
    { at:'O' }, // Padding
    { at:'O', right:true, span:3 } // Maps to Sector 1, spans 3 sectors (1, 2, 3) to form the perfect 90° right angle marker
  ]
};
...
async function run() {
  ...
  console.log('Successfully fixed right angle marker sector mapping in y9-7a-q2f!');
}
