...
import { Y7_CH1D_QUESTIONS } from '../../src/constants/seedYear7Ch1DQuestions.js';
...
const run = async () => {
  try {
    ...
    // Clear old seed questions for y7-1d
    console.log("Clearing existing seed questions for y7-1d...");
    const snap = await collRef.where('topicId', '==', 'y7-1d').where('origin', '==', 'seed').get();
    ...
    // Write new questions
    console.log(`Writing ${Y7_CH1D_QUESTIONS.length} questions...`);
    for (let i = 0; i < Y7_CH1D_QUESTIONS.length; i += batchSize) {
      const batch = db.batch();
      Y7_CH1D_QUESTIONS.slice(i, i + batchSize).forEach(raw => {
    ...
    // Also update seed_hashes for y7-1d
    const str = Y7_CH1D_QUESTIONS.map((q) => q.id || '').sort().join(',');
    let h = 5381;
    ...
    console.log("Successfully completed sync for Y7 Ch1D!");
    process.exit(0);
  } catch (err) {
