...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // HOTFIX: Q8C Alpha Migration
    if (q.id === 'y9-7a-q8c') {
      // Remove alpha from M
      geo.angles = geo.angles.filter(a => !(a.at === 'M' && a.label === 'α'));
      // Add right angle span 2 to O (idx 0 -> Sector 0), and alpha to O (idx 1 -> Sector 1)
      geo.angles = geo.angles.filter(a => a.at !== 'O'); // clear existing O
      geo.angles.push({ at: 'O', right: true, span: 2 }); // angle AOB
      geo.angles.push({ at: 'O', label: 'α' }); // angle MOA
    }
...
async function run() {
  const targetId = 'y9-7a-q8c';
  const docRef = db.collection('questions').doc(targetId);
  const data = (await docRef.get()).data();
  if (!data) return;
  
  const geo = seedMap[targetId];
  if (!geo) return;
  
  const beautifulSvg = geometryToSectorSvg(geo, targetId);
  let question = data.question;
  question = question.replace(/<svg[\s\S]*?<\/svg>\s*/g, '');
  question = `${beautifulSvg}\n\n${question}`;
  
  await docRef.update({ question });
  console.log(`Successfully migrated alpha to O in ${targetId}!`);
}
