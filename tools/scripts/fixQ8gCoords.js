...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // HOTFIX: Q8G Coordinate Alignment
    if (q.id.startsWith('y9-7a-q8g-')) {
      // E was [1, 3], which makes E-D slope 3, not parallel to A-B slope 2.
      // Change E to [2, 3] to make E-D slope 2.
      // This makes E, X[0,-1], D[-1,-3] perfectly collinear and widens the diagram!
      geo.points.E = [2, 3];
    }
...
async function run() {
  ...
  for (const id of targetIds) {
    ...
    console.log(`Successfully fixed coordinates for ${id}!`);
  }
