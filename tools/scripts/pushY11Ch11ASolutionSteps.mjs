/**
 * Push rewritten solutionSteps for y11a-11A from the seed file to Firestore.
 * Only updates solutionSteps (+ solution if present). Skips origin: 'teacher'.
 *
 * Usage: node tools/scripts/pushY11Ch11ASolutionSteps.mjs
 */
...
const seedPath = path.resolve(__dirname, '../../src/constants/seedYear11Ch11AQuestions.js');
const { Y11_CH11A_QUESTIONS } = await import(pathToFileURL(seedPath).href);

const targets = Y11_CH11A_QUESTIONS.filter((q) => q.id);
console.log(`Pushing solutionSteps for ${targets.length} y11a-11A questions…`);

// Validate all have ≥4 steps before writing
...
console.log('stepStats', stepStats);
