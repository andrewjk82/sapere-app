...
function geometryToSvg(geo) {
  ...
  if(geo.segments) for(const seg of geo.segments){
    ...
    if(seg.tick){
      const mx=(p1[0]+p2[0])/2,my=(p1[1]+p2[1])/2,angle=Math.atan2(p2[1]-p1[1],p2[0]-p1[0])*180/Math.PI + 90;
      svg+=`  <g transform="translate(${mx},${my}) rotate(${angle})"><line x1="0" y1="-6" x2="0" y2="6" stroke="#475569" stroke-width="1.8" /></g>\n`;
    }
  }
...
const geo = {
  ...
  angles: [
    { at:'A', label:'β' },
    { at:'B', label:'70°' }, // No padding! Maps directly to Sector 0 (interior)
    { at:'C', label:'α' } // No padding! Maps directly to Sector 0 (interior)
  ]
};

async function run() {
  const ids = ['y9-7a-q6a-alpha', 'y9-7a-q6a-beta'];
  ...
  for (const id of ids) {
    ...
    console.log(`Successfully fixed interior sector mapping for ${id}!`);
  }
}
