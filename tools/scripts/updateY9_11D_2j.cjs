...
board.create('functiongraph', [function(x){ return -5*x; }], {strokeColor: 'blue', strokeWidth: 2});
board.unsuspendUpdate();`;
  ...
  batch.update(db.collection('questions').doc('y9-11d-q2j'), {
    'type': 'multiple_choice',
    'question': 'Write down the gradient and \\(y\\)-intercept of the line \\(y = -5x\\).',
    'answer': '0',
    'options': [
      { text: '$m = -5,\\quad c = 0$', imageUrl: '' },
      { text: '$m = 5,\\quad c = 0$', imageUrl: '' },
      { text: '$m = -5,\\quad c = 5$', imageUrl: '' },
      { text: '$m = 0,\\quad c = -5$', imageUrl: '' }
    ],
  ...
  await batch.commit();
  console.log("Updated y9-11d-q2j to MCQ, fixed graph, and removed top-level graph.");
  process.exit(0);
}
