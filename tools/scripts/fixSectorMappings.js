...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    if (q.id === 'y9-7a-q2c') {
      const a = q.graphData.geometry.angles;
      // original: α, 100°, 4α
      // need: {}, 100°, 4α, α (to skip Sector 0 which is below the line)
      q.graphData.geometry.angles = [{}, a[1], a[2], a[0]];
    }
    if (q.id === 'y9-7a-q2d') {
      const a = q.graphData.geometry.angles;
      // original: θ, right, 50°
      // need: {}, {}, θ, right, 50° (to skip Sectors 0 and 1 which are above the line)
      q.graphData.geometry.angles = [{}, {}, a[0], a[1], a[2]];
    }
    if (q.id === 'y9-7a-q2e') {
      const a = q.graphData.geometry.angles;
      // original: α, α, α
    ...
    if (q.id === 'y9-7a-q2f') {
      const a = q.graphData.geometry.angles;
      // original: right, 3α, 2α, 4α
      // need: 3α, 2α, 4α (right will be manually drawn)
      q.graphData.geometry.angles = [a[1], a[2], a[3]];
    }
...
function geometryToSectorSvg(geo, id) {
  ...
  for (const vKey in anglesByVertex) {
    ...
    // Global right angle override for q2f (between P and S)
    if (id === 'y9-7a-q2f' && vKey === 'O') {
       // P is UP (270 deg), S is RIGHT (0 / 360 deg)
       // Draw an orthogonal right angle symbol between them
       const sqSize = 13;
       const a1 = 1.5 * Math.PI; // 270 degrees
       const a2 = 0; // 0 degrees
       const pxCorner = vx + Math.cos(a1)*sqSize + Math.cos(a2)*sqSize;
       const pyCorner = vy + Math.sin(a1)*sqSize + Math.sin(a2)*sqSize;
       svg += `  <path d="M ${vx + Math.cos(a1)*sqSize},${vy + Math.sin(a1)*sqSize} L ${pxCorner},${pyCorner} L ${vx + Math.cos(a2)*sqSize},${vy + Math.sin(a2)*sqSize}" fill="none" stroke="#3b82f6" stroke-width="1.5" stroke-linejoin="miter" />\n`;
    }
...
async function run() {
  ...
  console.log(`Prepared ${count} questions for advanced sector remapping.`);
  if (count > 0) {
    await batch.commit();
