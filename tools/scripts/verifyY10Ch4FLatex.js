...
(async () => {
  const snapF = await db.collection('questions').where('topicId', '==', 'y10-4f').get();
  let invalidCount = 0;
  
  snapF.docs.forEach(doc => {
    const data = doc.data();
  ...
  console.log('Final y10-4f validation audit invalid counts:', invalidCount);
  process.exit(0);
})();
