...
async function run() {
  ...
  // Standard drafting dimension lines:
  // Outer box: x=37.5 to 262.5, y=37.5 to 262.5
  // Inner box: x=82.5 to 217.5, y=82.5 to 217.5
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 320" width="300" height="320" style="font-family:sans-serif;">
  <defs>
    <marker id="arrow-end" markerWidth="12" markerHeight="8" refX="12" refY="4" orient="auto">
      <path d="M0,0 L12,4 L0,8 Z" fill="#555"/>
    </marker>
    <marker id="arrow-start" markerWidth="12" markerHeight="8" refX="0" refY="4" orient="auto">
      <path d="M12,0 L0,4 L12,8 Z" fill="#555"/>
    </marker>
  </defs>
  ...
  <!-- We'll draw them upwards from the inner hole to avoid overlapping the 15mm dimension, or draw them across the hole. -->
  <!-- Let's draw them horizontally inside the hole as standard drafting, with extension lines from the inner side walls -->
...
*(Assume both the outer and inner shapes are squares)*`;
  ...
  console.log("Updated dimension lines for y8-15a-q2c to standard drafting style");
  process.exit(0);
}
