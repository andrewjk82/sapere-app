...
function elementsToSvg(elements) {
  ...
  const scale = 22; // Slightly larger scale for better spacing
  const ox = 150;
  const oy = 150;
  ...
  elements.filter(e => e.type === 'point' && e.on).forEach(e => {
    ...
    pts[e.id] = {
      ...
      sx: ox + mx * scale,
      sy: oy - my * scale,
      angleFromCenter: -rad // y is inverted in SVG coordinate system
    };
  });
  ...
  // Polygons (Triangle / Quadrilateral fills)
  elements.filter(e => e.type === 'polygon').forEach(e => {
    let pList = e.points.map(id => `${pts[id].sx},${pts[id].sy}`).join(' ');
    // Sleek muted fill, no heavy stroke
    svg += `  <polygon points='${pList}' fill='#f8fafc' stroke='#e2e8f0' stroke-width='1.5' stroke-linejoin='round' />\\n`;
  });
  ...
  // Segments (Lines connecting points)
  elements.filter(e => e.type === 'segment').forEach(e => {
    let p1 = pts[e.from];
    let p2 = pts[e.to];
    let stroke = e.color || '#64748b'; // slate-500
  ...
  elements.filter(e => e.type === 'angle').forEach(e => {
    ...
    if (e.name) {
      let midA = a1 + diff / 2;
      let tr = r + 12; // Distance from vertex
      let tx = p2.sx + tr * Math.cos(midA);
      let ty = p2.sy + tr * Math.sin(midA) + 4; // middle vertical align
  ...
  // Right angles (clean little squares)
  ...
  Object.values(pts).forEach(p => {
    ...
    if (p.name) {
      ...
      // Intelligent label positioning so they don't overlap with the circle/lines
      if (p.angleFromCenter !== undefined) {
        let dist = 16; // Push out from the circle
        ...
        if (Math.sin(p.angleFromCenter) > 0.4) ty += 10;
        else if (Math.sin(p.angleFromCenter) < -0.4) ty -= 2;
        else ty += 4; 
      } else {
        tx = p.sx + 10;
        ty = p.sy + 10;
...
async function run() {
  ...
  for (const id of modifiedIds) {
    ...
    if (doc.exists) {
      ...
      console.log('Restyled SVG for', id);
    }
  }
