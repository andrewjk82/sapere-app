...
// Correcting the B and C arcs to touch the horizontal lines exactly and curve inwards.
const updates = {
  ...
  <!-- B: alpha (ends exactly at y=50 on the horizontal side BC, curves inwards with sweep-flag 1) -->
  <path d="M 120 69 A 22 22 0 0 1 152 50" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
  ...
  <!-- C: beta (ends exactly at y=50, curves inwards with sweep-flag 0) -->
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Updating trapezium arcs for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully refined trapezium arcs on Firestore!');
  process.exit(0);
}
