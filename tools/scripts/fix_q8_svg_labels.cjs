...
function getSvg(regionType) {
  ...
  if (regionType === 'increasing') {
    ...
    // Bottom left to avoid Max point at top left
    regionsHtml += `<text x="${tx(-4)}" y="280" font-family="sans-serif" font-size="14" fill="#166534" text-anchor="middle" font-weight="bold">Increasing (x &lt; -2)</text>`;
    // Top right to avoid Min point at bottom right
    ...
    // Top center is safe
    regionsHtml += `<text x="${tx(1)}" y="25" font-family="sans-serif" font-size="14" fill="#166534" text-anchor="middle" font-weight="bold">Decreasing (-2 &lt; x &lt; 4)</text>`;
  } else if (regionType === 'concave_up') {
    let w = tx(maxX) - tx(1);
    regionsHtml += `<rect x="${tx(1)}" y="0" width="${w}" height="${height}" fill="${fillColor}" />`;
    // Top right
    regionsHtml += `<text x="${tx(5)}" y="20" font-family="sans-serif" font-size="14" fill="#166534" text-anchor="middle" font-weight="bold">Concave Up (x &gt; 1)</text>`;
  } else if (regionType === 'concave_down') {
    let w = tx(1) - tx(minX);
    regionsHtml += `<rect x="${tx(minX)}" y="0" width="${w}" height="${height}" fill="${fillColor}" />`;
    // Bottom left
    regionsHtml += `<text x="${tx(-3)}" y="280" font-family="sans-serif" font-size="14" fill="#166534" text-anchor="middle" font-weight="bold">Concave Down (x &lt; 1)</text>`;
  }
  ...
  <text x="${maxPx + 8}" y="${maxPy}" font-family="sans-serif" font-size="13" fill="#333" text-anchor="start" font-weight="bold" dominant-baseline="middle">Max(-2, 38)</text>

  <circle cx="${minPx}" cy="${minPy}" r="5" fill="#ef4444" />
  <text x="${minPx + 8}" y="${minPy}" font-family="sans-serif" font-size="13" fill="#333" text-anchor="start" font-weight="bold" dominant-baseline="middle">Min(4, -70)</text>

  <circle cx="${infPx}" cy="${infPy}" r="5" fill="#8b5cf6" />
  <text x="${infPx + 8}" y="${infPy}" font-family="sans-serif" font-size="13" fill="#333" text-anchor="start" font-weight="bold" dominant-baseline="middle">Inflection(1, -16)</text>
</svg>`;
}
...
async function updateDb() {
  ...
  console.log('Successfully updated q8a-d with fixed labels in SVG graphData.');
  process.exit(0);
}
updateDb();
