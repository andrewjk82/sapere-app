...
import { Y10_CH3C_QUESTIONS } from '../../src/constants/seedYear10Ch3CQuestions.js';
...
const mapQuestion = (raw) => {
  ...
  return {
    ...
    topicId: "y10-3c",
    topicCode: "3C",
    topicTitle: "More difficult linear equations and inequalities",
    year: "Year 10",
    isManual: true,
    origin: "seed",
    id: raw.id,
...
async function syncQuestions() {
  ...
  console.log(`Importing ${Y10_CH3C_QUESTIONS.length} questions for ${topicId}...`);
  const BATCH_SIZE = 400;
  for (let i = 0; i < Y10_CH3C_QUESTIONS.length; i += BATCH_SIZE) {
    const batch = db.batch();
    const chunk = Y10_CH3C_QUESTIONS.slice(i, i + BATCH_SIZE);
  ...
  console.log('Done! All Y10 Ch3C questions synced.');
  process.exit(0);
}
