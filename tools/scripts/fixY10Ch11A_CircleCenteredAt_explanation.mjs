...
async function run() {
  ...
  for (let q of questions) {
    if (q.solutionSteps) {
      ...
      for (let s of q.solutionSteps) {
        if (s.workingOut && s.workingOut.includes('Circle centered at')) {
          let text = s.workingOut;
          
          // Strip \( and \) from ends if present
          text = text.replace(/^\\\(/, '').replace(/\\\)$/, '');
          // Strip trailing newline if any
          text = text.replace(/\\n/g, ' ');
          text = text.replace(/\n/g, ' ');
          
          // Wrap radius value in math mode
          text = text.replace(/radius ([^.]+)\./g, 'radius \\\\($1\\\\).');
          
          // Wrap coordinates in math mode
  ...
  if (updateCount > 0) {
    ...
    console.log(`Updated ${updateCount} questions by moving long text to explanation.`);
  }
