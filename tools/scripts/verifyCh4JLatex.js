...
(async () => {
  const snapJ = await db.collection('questions').where('topicId', '==', 'y7-4j').get();
  let invalidCount = 0;
  
  snapJ.docs.forEach(doc => {
    const data = doc.data();
  ...
  console.log('Final 4J validation audit invalid counts:', invalidCount);
  process.exit(0);
})();
