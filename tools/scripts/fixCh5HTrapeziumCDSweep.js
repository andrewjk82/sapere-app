...
// Setting C's sweep-flag to 1 and D's sweep-flag to 0 to curve inwards correctly.
const updates = {
  ...
  <!-- C: beta (curves inwards with sweep-flag 1) -->
  <path d="M 258 50 A 22 22 0 0 1 289 70" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Updating trapezium C/D sweep flags for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully corrected trapezium C/D sweep flags on Firestore!');
  process.exit(0);
}
