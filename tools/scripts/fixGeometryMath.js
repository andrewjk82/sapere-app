...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // Q2C
    if (q.id === 'y9-7a-q2c') {
      const g = q.graphData.geometry;
      // Exact math: 100, 64, 16
      g.points.K = [-2, 0]; // 180 deg
      g.points.L = [2, 0];  // 0 deg
      g.points.R = [2 * Math.cos(80 * Math.PI/180), 2 * Math.sin(80 * Math.PI/180)];
      g.points.S = [2 * Math.cos(16 * Math.PI/180), 2 * Math.sin(16 * Math.PI/180)];
      const a = g.angles;
      g.angles = [{ at: 'N' }, a[1], a[2], a[0]];
    }
    ...
    if (q.id === 'y9-7a-q2d') {
      ...
      g.points.A = [-2, 0];
      g.points.D = [2, 0];
      g.points.B = [2 * Math.cos(140 * Math.PI/180), 2 * Math.sin(140 * Math.PI/180)];
      g.points.C = [2 * Math.cos(50 * Math.PI/180), 2 * Math.sin(50 * Math.PI/180)];
      g.points.E = [-g.points.B[0], -g.points.B[1]];
      const a = g.angles;
      g.angles = [{ at: 'N' }, { at: 'N' }, a[0], a[1], a[2]];
    }
    ...
    if (q.id === 'y9-7a-q2e') {
      const g = q.graphData.geometry;
      // Exact math: 60, 60, 60...
      g.points.F = [2 * Math.cos(30 * Math.PI/180), 2 * Math.sin(30 * Math.PI/180)];
      g.points.C = [2 * Math.cos(90 * Math.PI/180), 2 * Math.sin(90 * Math.PI/180)];
      g.points.A = [2 * Math.cos(150 * Math.PI/180), 2 * Math.sin(150 * Math.PI/180)];
      g.points.E = [2 * Math.cos(210 * Math.PI/180), 2 * Math.sin(210 * Math.PI/180)];
      g.points.D = [2 * Math.cos(270 * Math.PI/180), 2 * Math.sin(270 * Math.PI/180)];
      g.points.B = [2 * Math.cos(330 * Math.PI/180), 2 * Math.sin(330 * Math.PI/180)];
      const a = g.angles;
      g.angles = [a[0], { at: 'N' }, a[1], { at: 'N' }, a[2], { at: 'N' }];
    }
...
function geometryToSectorSvg(geo, id) {
  ...
  for (const vKey in anglesByVertex) {
    ...
    // Global right angle override for q2f
...
async function run() {
  ...
  snapshot.forEach(doc => {
    const id = doc.id;
    if (['y9-7a-q2c', 'y9-7a-q2d', 'y9-7a-q2e', 'y9-7a-q2f'].includes(id)) {
  ...
  console.log(`Prepared ${count} questions for exact mathematical geometries.`);
  if (count > 0) {
    await batch.commit();
