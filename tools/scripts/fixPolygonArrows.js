...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    // HOTFIX: REMOVE ARROWS FROM CLOSED POLYGON EDGES
...
function geometryToSectorSvg(geo, id) {
  ...
  // NOTE: Dots are deliberately not drawn anymore globally, keeping with user's earlier request.
...
async function run() {
  const targetIds = [
    'y9-7a-q3c-alpha', 'y9-7a-q3c-beta', 
    'y9-7a-q3d-alpha', 'y9-7a-q3d-beta', 'y9-7a-q3d-gamma',
    'y9-7a-q8a-alpha', 'y9-7a-q8a-beta', 'y9-7a-q8a-gamma',
    'y9-7a-q8b',
    'y9-7a-q8h-alpha', 'y9-7a-q8h-beta', 'y9-7a-q8h-gamma'
  ];
  ...
  for (const id of targetIds) {
    ...
    console.log(`Successfully removed segment end arrows from polygon in ${id}!`);
  }
