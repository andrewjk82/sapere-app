...
async function run() {
  ...
  console.log('List from index 36 (37th question) to 46 (47th question):');
  const slice = list.slice(36, 46);
  slice.forEach((item, index) => {
    console.log(`${index + 37}. ID: ${item.id} | Type: ${item.type} | Q: ${item.question}`);
  });
  process.exit(0);
}
