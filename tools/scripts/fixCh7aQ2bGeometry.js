...
const geo = {
  points: { O:[0,0], A:[-2.5, -0.625], B:[2.5, 0.625], C:[1, 2], D:[-1, -2] }, // Intersection lines
  segments: [
    { from:'A', to:'B', arrow:true },
    { from:'B', to:'A', arrow:true },
    { from:'C', to:'D', arrow:true },
    { from:'D', to:'C', arrow:true }
  ],
  // Map labels to correct sectors:
  // Sector 0 (Bottom-Right) -> empty
  // Sector 1 (Bottom-Left) -> 120°
  // Sector 2 (Top-Left) -> β
  // Sector 3 (Top-Right) -> α (vertically opposite to 120°)
  angles: [
    { at:'O' },
    { at:'O', label:'120°' },
    { at:'O', label:'β' },
...
async function run() {
  const ids = ['y9-7a-q2b-alpha', 'y9-7a-q2b-beta'];
