...
import { Y10_CH2B_QUESTIONS } from '../../src/constants/seedYear10Ch2BQuestions.js';
...
async function run() {
  console.log(`Loading Y10 Ch2B questions from local seed. Total: ${Y10_CH2B_QUESTIONS.length}`);
  
  const batch = db.batch();
  
  // 1. Delete all current y10-2b questions from DB to ensure no leftovers
  const snap = await db.collection('questions')
    .where('topicId', '==', 'y10-2b')
    .get();
  
  console.log(`Deleting ${snap.size} current y10-2b questions from Firestore...`);
  snap.forEach(doc => {
  ...
  console.log(`\n🎉 Successfully uploaded ${Y10_CH2B_QUESTIONS.length} y10-2b questions to Firestore!`);
