...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    if (q.id.startsWith('y9-7a-q8g-')) {
      geo.points.E = [2, 3]; // Align coordinates
      ...
      // Gamma redesign
      // 1. Remove meaningless vertical line A-D
      if (geo.segments) {
        geo.segments = geo.segments.filter(s => !(s.from === 'A' && s.to === 'D'));
        
        // 2. Complete the isosceles triangle by adding base C-D
        const hasCD = geo.segments.some(s => (s.from === 'C' && s.to === 'D') || (s.from === 'D' && s.to === 'C'));
        if (!hasCD) {
          geo.segments.push({ from: 'D', to: 'C' });
        }
      }
      
      // Since A-D is gone, the rays at D are D->E (or D->X) and D->C.
      // D->X is 296.6 degrees. D->C is 333.5 degrees.
      // Sector 0 is the interior angle between them.
      // Gamma is the only label at D, so it automatically maps to Sector 0!
    }
...
async function run() {
  ...
  for (const id of targetIds) {
    ...
    // Specifically update the answer and solution for gamma
    if (id === 'y9-7a-q8g-gamma') {
      updates.answer = 0; // Index 0 is 37.5 degrees
      
      const solutionSteps = data.solutionSteps || [];
      if (solutionSteps.length > 0) {
        // Adjust the first step to say base angles equal for triangle with vertex angle 105
        solutionSteps[0] = {
          explanation: "Base angles equal",
          workingOut: "\\(2\\gamma + 105^\\circ = 180^\\circ\\)",
          graphData: null
        };
      }
    ...
    console.log(`Successfully redesigned geometry and math for ${id}!`);
  }
