...
// 4-fold backslashes (\\\\) to guarantee that they are saved as a single backslash (\) in the Firestore database document fields.
const part1Sub = [
  {
    id: "y10-4g-q13a_i_a",
...
(async () => {
  try {
    console.log('[Firestore Escape Fix] Overwriting parts with 4-fold backslashes to bypass double parsing...');
    
    await db.collection('questions').doc('y10-4g-q13_part1').update({
      subQuestions: part1Sub
    });
    console.log('  [PART 1 ESCAPE FIXED]');

    await db.collection('questions').doc('y10-4g-q13_part2').update({
      subQuestions: part2Sub
    });
    console.log('  [PART 2 ESCAPE FIXED]');
