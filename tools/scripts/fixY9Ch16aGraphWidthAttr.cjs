...
const fixWidthAttribute = (svgStr) => {
  return svgStr.replace(/width="100%"/g, 'width="250"');
};
...
questions = questions.map(q => {
  if (q.id && q.id.startsWith('y9-16a-')) {
    ...
    if (q.graphData && q.graphData.svg) {
      const newSvg = fixWidthAttribute(q.graphData.svg);
    ...
    if (q.solutionSteps) {
      q.solutionSteps.forEach(step => {
        if (step.graphData && step.graphData.svg) {
          const newSvg = fixWidthAttribute(step.graphData.svg);
...
console.log(`Locally updated ${updatedQuestionIds.length} questions for width attribute fix.`);
