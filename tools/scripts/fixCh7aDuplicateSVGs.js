...
async function run() {
  ...
  for (const q of questions) {
    ...
    batch.update(docRef, {
      ...
      // CRITICAL: Delete legacy fields to prevent duplicate/old rendering
      diagramSvg: FieldValue.delete(),
      diagramSource: FieldValue.delete(),
      conversionNote: FieldValue.delete(),
      geometry: FieldValue.delete()
    });
    console.log(`Updated and cleared legacy fields for: ${q.id}`);
  }
  await batch.commit();
  console.log('\n✅ Successfully removed duplicates and updated Firebase!');
}
