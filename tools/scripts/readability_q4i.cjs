...
q.graphData.jsxGraph.elements.find(e => e.id === 'P').angle = 170;
q.graphData.jsxGraph.elements.find(e => e.type === 'angle' && e.points.includes('P') && e.points.includes('R') && e.points.includes('Q')).name = '95°';
...
async function save() {
  ...
  await db.collection('questions').doc('y10-13a-q4i').update({
    opts: q.opts,
    options: q.options,
    a: q.a,
    answer: q.answer,
  ...
  console.log('Successfully adjusted q4i for better readability!');
  process.exit(0);
}
