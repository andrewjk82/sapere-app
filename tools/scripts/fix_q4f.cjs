...
// Modify the angle definition for gamma to be 'O', 'P', 'Q' instead of 'R', 'P', 'Q'
const gammaAngle = q.graphData.jsxGraph.elements.find(e => e.type === 'angle' && e.name === '\\gamma');
if (gammaAngle) {
  gammaAngle.points = ['O', 'P', 'Q'];
}

// Re-generate the SVG using the same script logic
function dist(pA, pB) {
...
function elementsToSvg(elements) {
  ...
  elements.filter(e => e.type === 'segment').forEach(e => {
    ...
    let p2 = pts[e.to];
    if(!p1 || !p2) return;
    svg += `  <line x1='${p1.sx}' y1='${p1.sy}' x2='${p2.sx}' y2='${p2.sy}' stroke='#64748b' stroke-width='1.5' stroke-linecap='round' />\n`;
  });
  ...
  elements.filter(e => e.type === 'angle').forEach(e => {
    ...
    if (e.name) {
      ...
      // Special offset for beta to ensure it looks balanced
      if (e.name === '\\beta') {
        tx -= 2;
      }
      if (e.name === '\\gamma') {
...
// Now update the distractors and answer options for [100, 20, 30] instead of [100, 20, 50]
let data = { ans: ["100", "20", "30"], vars: ["\\alpha", "\\beta", "\\gamma"] };
...
q.solutionSteps[1].workingOut = "Assuming O is the centre, triangle OQR is isosceles so angle ORQ = OQR = 40^\\circ. Therefore \\(\\alpha = 180^\\circ - 40^\\circ - 40^\\circ = 100^\\circ\\). Triangle OQP is isosceles, so \\(\\gamma = (180^\\circ - 120^\\circ)/2 = 30^\\circ\\). Triangle ORP is also isosceles. The central angle ROP is \\(360^\\circ - 120^\\circ - 100^\\circ = 140^\\circ\\). Therefore \\(\\beta = (180^\\circ - 140^\\circ)/2 = 20^\\circ\\).";
...
async function save() {
  ...
  await db.collection('questions').doc('y10-13a-q4f').update({
  ...
  console.log('Fixed q4f ambiguity.');
  process.exit(0);
}
