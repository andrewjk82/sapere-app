...
// Changing C's sweep-flag to 1 so it curves inwards (right side of the chord vector).
const updates = {
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Updating trapezium C sweep flag to 1 for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully refined trapezium C sweep flag on Firestore!');
  process.exit(0);
}
