...
// Inverting sweep flags to curve arcs inwards (concentric to their respective vertices)
const updates = {
  ...
  <!-- Angles (inverted sweep flags to curve inwards) -->
  <!-- A: 40deg -->
  ...
  <!-- B: alpha -->
  <path d="M 162 50 A 22 22 0 0 0 131 70" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
  ...
  <!-- D: gamma -->
  <path d="M 238 140 A 22 22 0 0 0 269 120" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Inverting sweep flags for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully inverted sweep flags to curve inward!');
  process.exit(0);
}
