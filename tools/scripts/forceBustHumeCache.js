...
// Plain text labels inside graph to prevent backslash issues
const cleanGraph = {
  jsxGraph: {
    width: 300,
    height: 300,
...
(async () => {
  try {
    console.log('[Bust Cache] Deleting the old cached ID y10-4g-q13...');
    const oldRef = db.collection('questions').doc('y10-4g-q13');
    await oldRef.delete();
    console.log('  [DELETED] y10-4g-q13');

    // Recreate Part 1 as y10-4g-q13_part1 to force clients to fetch fresh data
    console.log('[Bust Cache] Recreating Part 1 as y10-4g-q13_part1...');
    const newRef = db.collection('questions').doc('y10-4g-q13_part1');
    await newRef.set({
      year: "Year 10",
      course: "Mathematics",
      chapterId: "y10-4",
    ...
    console.log('  [CREATED] y10-4g-q13_part1');
