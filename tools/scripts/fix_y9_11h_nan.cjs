...
function formatMathText(str) {
  // convert \sqrt{21} to √21, clean stray parens
  let s = str.replace(/\\sqrt\{(\d+)\}/g, '√$1').replace(/\\sqrt(\d+)/g, '√$1');
  // keep exactly one pair of surrounding parens
  s = s.replace(/^\(*/, '').replace(/\)*$/, '');
  return '(' + s + ')';
}

function generateSvg(r2, px, py, hasPoint, rawPointStr) {
  ...
  if (hasPoint) {
    ...
    const pointStr = formatMathText(rawPointStr);
    
    // adjust text anchoring if point is far right so it doesn't clip
    let textAnchor = cx > 220 ? "text-anchor='end' x='" + (cx - 8) + "'" : "text-anchor='start' x='" + (cx + 8) + "'";
    
    svg += "  <circle cx='" + cx + "' cy='" + cy + "' r='5' fill='" + color + "' />\\n" +
           "  <text " + textAnchor + " y='" + (cy - 8) + "' font-family='sans-serif' font-size='12' font-weight='bold' fill='" + color + "'>" + pointStr + "</text>\\n";
  } else {
    svg += "  <!-- Intercepts -->\\n" +
