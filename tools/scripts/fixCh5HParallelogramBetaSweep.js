...
// Inverting sweep flags for C (beta) and D (gamma) to 0 to curve inwards
const updates = {
  ...
  <!-- C: beta (sweep flag 0 curves inward) -->
  ...
  <!-- D: gamma (sweep flag 0 curves inward) -->
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Setting corrected C/D sweep flags for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully corrected C/D sweep flags to 0!');
  process.exit(0);
}
