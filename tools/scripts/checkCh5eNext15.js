...
async function run() {
  ...
  console.log('List from index 10 (11th question) to 25 (26th question):');
  const slice = list.slice(10, 25);
  slice.forEach((item, index) => {
    console.log(`${index + 11}. ID: ${item.id} | Type: ${item.type} | Q: ${item.question} | Ans: ${item.answer}`);
  });
  process.exit(0);
}
