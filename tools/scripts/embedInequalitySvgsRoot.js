...
async function run() {
  ...
  console.log('Embedding SVG diagrams directly in the root graphData of Year 11 Advanced Chapter 4A questions...');
  let updatedCount = 0;
  ...
  snap.forEach(d => {
    ...
    if (svg) {
      console.log('Embedding SVG in question root graphData for: ' + d.id);
      batch.update(d.ref, {
        graphData: {
          diagramSvg: svg
        }
  ...
  if (updatedCount > 0) {
    ...
    await batch.commit();
    console.log('Successfully updated root graphData SVGs for ' + updatedCount + ' questions!');
  } else {
