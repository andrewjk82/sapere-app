...
const qIndex = questions.findIndex(x => x.id === 'y10-13a-q4i');
let q = questions[qIndex];

q.question = "Find the values of \\(\\alpha\\) and \\(\\beta\\). Enter your answer as two numbers separated by a comma.";
q.opts = [
  "\\(\\alpha = 65^\\circ, \\beta = 80^\\circ\\)",
  "\\(\\alpha = 55^\\circ, \\beta = 65^\\circ\\)",
  "\\(\\alpha = 65^\\circ, \\beta = 35^\\circ\\)",
  "\\(\\alpha = 115^\\circ, \\beta = 80^\\circ\\)"
];
q.a = 0;
q.answer = 0;
...
q.solutionSteps = [
  ...
  },
  {
    ...
    workingOut: "We are given that \\(\\angle PQR = 115^\\circ\\). Therefore, \\(\\alpha = 180^\\circ - 115^\\circ = 65^\\circ\\).",
    graphData: null
  },
  {
  ...
  },
  {
    ...
    workingOut: "The total angle at \\(Q\\) is \\(\\angle PQR = 115^\\circ\\). Since \\(\\angle OQR = 35^\\circ\\), the remaining part of the angle is \\(\\angle PQO = 115^\\circ - 35^\\circ = 80^\\circ\\). Because \\(\\Delta OPQ\\) is isosceles (\\(OP = OQ\\)), the base angles are equal: \\(\\beta = \\angle OPQ = \\angle PQO = 80^\\circ\\).",
    graphData: null
  }
];
...
q.graphData = {
  jsxGraph: {
    ...
    elements: [
      ...
      { type: 'point', id: 'O', coords: [0, 0], name: 'O', color: 'red', label: { offset: [8, 8] } },
      { type: 'point', id: 'R', on: 'c1', angle: 0, name: 'R', label: { offset: [8, 0] } },
      { type: 'point', id: 'Q', on: 'c1', angle: 110, name: 'Q', label: { offset: [0, 8] } },
      { type: 'point', id: 'P', on: 'c1', angle: 130, name: 'P', label: { offset: [-8, 8] } },
      { type: 'point', id: 'S', on: 'c1', angle: 270, name: 'S', label: { offset: [0, -10] } },
      { type: 'polygon', points: ['P', 'Q', 'R', 'S'] },
      { type: 'segment', from: 'O', to: 'P', dash: true },
      { type: 'segment', from: 'O', to: 'R', dash: true },
      { type: 'angle', points: ['P', 'Q', 'R'], name: '115°', radius: 1.2, label: { offset: [0, 0] } },
      { type: 'angle', points: ['Q', 'R', 'O'], name: '35°', radius: 1.8, label: { offset: [0, 0] } },
      { type: 'angle', points: ['R', 'S', 'P'], name: 'α', radius: 1, label: { offset: [0, 0] } },
      { type: 'angle', points: ['O', 'P', 'Q'], name: 'β', radius: 1.2, label: { offset: [0, 0] } }
    ]
  }
};

// Now we must convert it to SVG using our generator logic
function dist(pA, pB) {
...
async function save() {
  ...
  await db.collection('questions').doc('y10-13a-q4i').set(q);
  ...
  console.log('Successfully made q4i harder!');
  process.exit(0);
}
