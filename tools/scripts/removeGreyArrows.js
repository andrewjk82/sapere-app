...
function geometryToSvg(geo) {
  ...
  if(geo.segments) for(const seg of geo.segments){
    ...
    // Don't add arrow if arrow is false/undefined
...
const q3f_geo = {
  points: { A:[-2,2], D:[2,2], F:[2,-1.5], C:[-2,-1.5], E:[0.5,0] },
  segments: [
    { from:'A', to:'D', marks:1 },
    { from:'E', to:'F' },
    { from:'C', to:'F', marks:1 },
...
async function run() {
  // Update 3f
  const doc3f = db.collection('questions').doc('y9-7a-3f');
  const svg3f = geometryToSvg(q3f_geo);
  const qText3f = `${svg3f}\n\nFind the angle marked with the pronumeral \\(\\theta\\).`;
  await doc3f.update({ question: qText3f });
  console.log('Successfully removed grey arrows from y9-7a-3f');

  // Update 8e
  const doc8e = db.collection('questions').doc('y9-7a-8e');
  const svg8e = geometryToSvg(q8e_geo);
  const qText8e = `${svg8e}\n\nFind the angle marked with \\(\\theta\\).`;
  await doc8e.update({ question: qText8e });
  console.log('Successfully removed grey arrows from y9-7a-8e');
}
