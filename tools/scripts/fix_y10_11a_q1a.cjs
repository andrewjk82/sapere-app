...
const svgCode = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 300 300' width='100%' height='100%'>\\n" +
  ...
  "  <text x='258' y='145' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(4, 0)</text>\\n" +
  "  \\n" +
  "  <!-- (-4, 0) -->\\n" +
  "  <circle cx='50' cy='150' r='4' fill='#3b82f6' />\\n" +
  "  <text x='15' y='145' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(-4, 0)</text>\\n" +
  "  \\n" +
  "  <!-- (0, 4) -->\\n" +
  "  <circle cx='150' cy='50' r='4' fill='#3b82f6' />\\n" +
  "  <text x='158' y='45' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(0, 4)</text>\\n" +
  "  \\n" +
  "  <!-- (0, -4) -->\\n" +
  "  <circle cx='150' cy='250' r='4' fill='#3b82f6' />\\n" +
  "  <text x='158' y='265' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(0, -4)</text>\\n" +
  "</svg>";
...
async function run() {
  ...
  const docRef = db.collection('questions').doc('y10-11a-q1a');
  ...
  if (doc.exists) {
    ...
    if (data.solutionSteps && data.solutionSteps[1]) {
      data.solutionSteps[1].workingOut = "\\\\(x\\\\text{-intercepts}: (\\\\pm 4, 0), \\\\quad y\\\\text{-intercepts}: (0, \\\\pm 4)\\\\)";
    }
    ...
    console.log('Fixed y10-11a-q1a in Firestore');
  }
  ...
  // It's easier to just use the replace script logic for the seed file
  const Y10_CH11A_QUESTIONS = eval(content.match(/export const Y10_CH11A_QUESTIONS = (\[[\s\S]*\]);$/m)[1]);
  const target = Y10_CH11A_QUESTIONS.find(q => q.id === 'y10-11a-q1a');
  if (target) {
    ...
    if (target.solutionSteps && target.solutionSteps[1]) {
      target.solutionSteps[1].workingOut = "\\(x\\text{-intercepts}: (\\pm 4, 0), \\quad y\\text{-intercepts}: (0, \\pm 4)\\)";
    }
    
    const newArrayStr = JSON.stringify(Y10_CH11A_QUESTIONS, null, 2)
      .replace(/"([^"]+)":/g, '$1:');
    
    const finalContent = content.replace(/export const Y10_CH11A_QUESTIONS = \[\s\S]*\];$/m, 'export const Y10_CH11A_QUESTIONS = ' + newArrayStr + ';\\n');
    
    // actually, let's just use string replace on the match
