...
async function run() {
  ...
  console.log('List from index 15 (16th question) to 30 (31st question):');
  const slice = list.slice(15, 30);
  slice.forEach((item, index) => {
    console.log(`${index + 16}. ID: ${item.id} | Type: ${item.type} | Q: ${item.question} | Ans: ${item.answer}`);
  });
  process.exit(0);
}
