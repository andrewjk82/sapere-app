...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    if (q.id === 'y9-7a-q2f') {
      ...
      // RESTORE RIGHT ANGLE WITH SPAN 3
    ...
    if (q.id === 'y9-7a-q8c') {
      ...
      if (oAngle) oAngle.span = 2; // Span across the two 45-degree sectors created by the median!
    }
...
async function run() {
  const targetIds = ['y9-7a-q8c', 'y9-7a-q2f'];
  ...
  for (const id of targetIds) {
    ...
    console.log(`Successfully fixed right angle span in ${id}!`);
  }
