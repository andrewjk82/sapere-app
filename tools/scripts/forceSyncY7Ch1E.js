...
import { Y7_CH1E_QUESTIONS } from '../../src/constants/seedYear7Ch1EQuestions.js';
...
const run = async () => {
  try {
    ...
    // Clear old seed questions for y7-1e
    console.log("Clearing existing seed questions for y7-1e...");
    const snap = await collRef.where('topicId', '==', 'y7-1e').where('origin', '==', 'seed').get();
    ...
    // Write new questions
    console.log(`Writing ${Y7_CH1E_QUESTIONS.length} questions...`);
    for (let i = 0; i < Y7_CH1E_QUESTIONS.length; i += batchSize) {
      const batch = db.batch();
      Y7_CH1E_QUESTIONS.slice(i, i + batchSize).forEach(raw => {
    ...
    // Also update seed_hashes for y7-1e
    const str = Y7_CH1E_QUESTIONS.map((q) => q.id || '').sort().join(',');
    let h = 5381;
    ...
    console.log("Successfully completed sync for Y7 Ch1E!");
    process.exit(0);
  } catch (err) {
