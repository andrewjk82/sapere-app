...
// Inverting the sweep flags for both C and D to bulge the opposite way.
// - C: Changed sweep-flag from 1 to 0.
// - D: Changed sweep-flag from 0 to 1.
const updates = {
  ...
  <!-- Angles -->
  <!-- D: 55deg (sweep-flag changed to 1) -->
  ...
  <!-- C: beta (sweep-flag changed to 0) -->
  <path d="M 248 50 A 22 22 0 0 0 281 69" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Inverting C and D bulges for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully inverted both C and D arcs bulge direction!');
  process.exit(0);
}
