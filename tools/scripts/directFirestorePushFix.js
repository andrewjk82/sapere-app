...
// Raw, clean text formulas that will be written into Firestore fields exactly as-is.
// We use JSON parsing of raw strings to avoid javascript engine backslash stripping during variable assignment.
const part1DataRaw = `{
  "subQuestions": [
    {
      "id": "y10-4g-q13a_i_a",
...
(async () => {
  try {
    console.log('[Direct Firestore Clean Push] Parsing JSON string to bypass JS backslash assignment stripping...');
    const parsedPart1 = JSON.parse(part1DataRaw);
    const parsedPart2 = JSON.parse(part2DataRaw);

    console.log('[Direct Firestore Clean Push] Updating y10-4g-q13_part1...');
    await db.collection('questions').doc('y10-4g-q13_part1').update({
      subQuestions: parsedPart1.subQuestions
    });
    console.log('  [PART 1 PUSH SUCCESS]');

    console.log('[Direct Firestore Clean Push] Updating y10-4g-q13_part2...');
    await db.collection('questions').doc('y10-4g-q13_part2').update({
      subQuestions: parsedPart2.subQuestions
    });
    console.log('  [PART 2 PUSH SUCCESS]');

    console.log('\nFirestore Database directly updated and cleaned.');
    process.exit(0);
  } catch (err) {
