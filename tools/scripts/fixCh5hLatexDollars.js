...
async function run() {
  ...
  batch.update(q14aiRef, {
    solutionSteps: [
      {
        explanation: "Convert the variable cost per kilometer from cents to dollars. Since there are 100 cents in a dollar, 5 cents is equal to \\(\\text{달러 } 0.05\\).",
        workingOut: "5\\text{ cents} = \\text{\\$}\\frac{5}{100} = \\text{\\$}0.05"
      },
      {
    ...
    solutionSteps: [
      {
        ...
        workingOut: "7\\text{ cents} = \\text{\\$}\\frac{7}{100} = \\text{\\$}0.07"
      },
      {
  ...
  await batch.commit();
  console.log('Successfully fixed LaTeX dollar sign escaping for q14ai, q14aii, q14b in Firestore!');
  process.exit(0);
}
