...
async function run() {
  ...
  // viewBox enlarged for all to scale them down
  const SVG_Q2D_BASE = `<svg viewBox="-60 0 340 220" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  ...
  <circle cx="80" cy="70" r="${R}" fill="#000"/>
  <circle cx="40" cy="140" r="${R}" fill="#000"/>
  <circle cx="120" cy="140" r="${R}" fill="#000"/>
  <circle cx="200" cy="140" r="${R}" fill="#000"/>
  ...
  const SVG_Q2D_ALPHA = SVG_Q2D_BASE + `
  <text x="58" y="135" ${P_FONT}>α</text>
</svg>`;

  const SVG_Q2D_BETA = SVG_Q2D_BASE + `
  <text x="175" y="135" ${P_FONT}>β</text>
</svg>`;

  const SVG_Q2D_Y = SVG_Q2D_BASE + `
  ...
  const updates = {
    'y10-8a-q2d-alpha': { graphData: { svg: SVG_Q2D_ALPHA } },
    'y10-8a-q2d-beta': { graphData: { svg: SVG_Q2D_BETA } },
    'y10-8a-q2d-y': { graphData: { svg: SVG_Q2D_Y } },
  ...
  console.log('Finished fixing batch 2 svgs');
  process.exit(0);
}
