...
function elementsToSvg(elements) {
  ...
  elements.filter(e => e.type === 'angle').forEach(e => {
    ...
    // Don't cap radius too aggressively if it's reflex, because it draws outside
...
async function run() {
  ...
  for (let q of questions) {
    if (q.id.startsWith('y10-13a-q') && q.graphData && q.graphData.svg) {
      ...
      console.log('Regenerated SVG for', q.id);
    }
  }
  
  console.log('Total SVGs regenerated in DB:', updatedCount);
  process.exit(0);
}
