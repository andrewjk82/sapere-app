...
const svgCode = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 300 300' width='100%' height='100%'>\\n" +
  ...
  "  \\n" +
  "  <!-- Grid lines (scale: 50px = 1 unit, radius=2 units = 100px) -->\\n" +
  ...
  "  <text x='258' y='145' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(2, 0)</text>\\n" +
  "  \\n" +
  "  <!-- (-2, 0) -->\\n" +
  "  <circle cx='50' cy='150' r='4' fill='#3b82f6' />\\n" +
  "  <text x='15' y='145' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(-2, 0)</text>\\n" +
  "  \\n" +
  "  <!-- (0, 2) -->\\n" +
  "  <circle cx='150' cy='50' r='4' fill='#3b82f6' />\\n" +
  "  <text x='158' y='45' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(0, 2)</text>\\n" +
  "  \\n" +
  "  <!-- (0, -2) -->\\n" +
  "  <circle cx='150' cy='250' r='4' fill='#3b82f6' />\\n" +
  "  <text x='158' y='265' font-family='sans-serif' font-size='12' font-weight='bold' fill='#0f172a'>(0, -2)</text>\\n" +
  "</svg>";
...
async function run() {
  ...
  const docRef = db.collection('questions').doc('y10-11a-q1b');
  ...
  if (doc.exists) {
    ...
    if (data.solutionSteps && data.solutionSteps[1]) {
      data.solutionSteps[1].workingOut = "\\\\(x\\\\text{-intercepts}: (\\\\pm 2, 0), \\\\quad y\\\\text{-intercepts}: (0, \\\\pm 2)\\\\)";
    }
    ...
    console.log('Fixed y10-11a-q1b in Firestore');
  }
  ...
  const target = Y10_CH11A_QUESTIONS.find(q => q.id === 'y10-11a-q1b');
  if (target) {
    ...
    if (target.solutionSteps && target.solutionSteps[1]) {
      target.solutionSteps[1].workingOut = "\\(x\\text{-intercepts}: (\\pm 2, 0), \\quad y\\text{-intercepts}: (0, \\pm 2)\\)";
    }
