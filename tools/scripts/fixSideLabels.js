...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    if (q.id === 'y9-7a-q5d-alpha') {
      geo.angles = [ { at: 'R', label: '120°', span: 2 }, { at: 'S', label: 'α' }, { at: 'T', label: '70°' }, { at: 'T', label: '30°' }, { at: 'U', label: '60°' } ];
    }
    if (q.id === 'y9-7a-q5d-beta') {
      geo.angles = [ { at: 'R', label: '120°', span: 2 }, { at: 'R', label: '50°' }, { at: 'T', label: 'β' }, { at: 'T', label: '30°' }, { at: 'U', label: '60°' } ];
    }
      ...
      if (p1 && p2) {
        ...
        if (seg.ticks) {
          ...
          if (seg.ticks === 1) {
            ...
            svg += `  <g transform="translate(${mx}, ${my}) rotate(${angle})"><line x1="-2.5" y1="-5" x2="-2.5" y2="5" stroke="#475569" stroke-width="1.8" /><line x1="2.5" y1="-5" x2="2.5" y2="5" stroke="#475569" stroke-width="1.8" /></g>\n`;
          } else if (seg.ticks === 3) {
            svg += `  <g transform="translate(${mx}, ${my}) rotate(${angle})"><line x1="-4" y1="-5" x2="-4" y2="5" stroke="#475569" stroke-width="1.8" /><line x1="0" y1="-5" x2="0" y2="5" stroke="#475569" stroke-width="1.8" /><line x1="4" y1="-5" x2="4" y2="5" stroke="#475569" stroke-width="1.8" /></g>\n`;
          }
        }
      }
    }
  ...
  if (geo.sideLabels) {
    ...
    if (ptCount > 0) {
      ...
      for (const sLabel of geo.sideLabels) {
        ...
        // Also add a white stroke for text outline for better legibility against grid/lines
...
async function run() {
  const ids = ['y9-7a-q6c', 'y9-7a-q6e-x', 'y9-7a-q6e-y', 'y9-7a-q8f-alpha', 'y9-7a-q8f-beta', 'y9-7a-q8f-x'];
  ...
  for (const id of ids) {
    ...
    // Strip incorrect degree symbols for side lengths
    if (['y9-7a-q6c', 'y9-7a-q6e-x', 'y9-7a-q6e-y', 'y9-7a-q8f-x'].includes(id)) {
      data.options = data.options.map(opt => {
        opt.text = opt.text.replace(/\\^\\circ/g, '');
        return opt;
      });
    }
    
    batch.update(docRef, { question, options: data.options });
    console.log(`Successfully fixed side labels rendering and stripped degrees for ${id}!`);
  }
