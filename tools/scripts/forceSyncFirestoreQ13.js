...
// Plain text labels inside graph
const cleanGraph = {
  jsxGraph: {
    width: 300,
    height: 300,
    ...
    script: `
      ...
      board.create('text', [2.7, 8, 't (h)'], {fontSize: 10, fontFamily: 'sans-serif', strokeColor: '#475569', fixed: true});
      board.create('text', [0.1, 185, 'd (km)'], {fontSize: 10, fontFamily: 'sans-serif', strokeColor: '#475569', fixed: true});
      
      for (var t = 0.5; t <= 2.5; t += 0.5) {
        board.create('segment', [[t, -3], [t, 3]], {strokeColor: '#cbd5e1', strokeWidth: 1, fixed: true});
        board.create('text', [t - 0.08, -12, t.toFixed(1)], {fontSize: 8.5, strokeColor: '#64748b', fixed: true});
      }
      for (var d = 30; d <= 180; d += 30) {
        board.create('segment', [[-0.05, d], [0.05, d]], {strokeColor: '#cbd5e1', strokeWidth: 1, fixed: true});
        board.create('text', [-0.35, d - 3, d], {fontSize: 8.5, strokeColor: '#64748b', fixed: true});
      }
      
      board.create('segment', [[0, 30], [2.5, 180]], {strokeColor: '#3b82f6', strokeWidth: 2.5, fixed: true});
      board.create('text', [1.8, 120, 'Car A'], {fontSize: 10, strokeColor: '#3b82f6', fixed: true});
      
      board.create('segment', [[0, 120], [1.5, 0]], {strokeColor: '#ef4444', strokeWidth: 2.5, fixed: true});
      board.create('text', [0.8, 80, 'Car B'], {fontSize: 10, strokeColor: '#ef4444', fixed: true});
...
(async () => {
  try {
    console.log('[Firestore Sync] Purging old document and enforcing Q13 Part 1 and Part 2...');
    
    // 1. Delete old y10-4g-q13
    await db.collection('questions').doc('y10-4g-q13').delete();
    console.log('  [DELETED] y10-4g-q13');

    // 2. Set y10-4g-q13_part1
    await db.collection('questions').doc('y10-4g-q13_part1').set({
      year: "Year 10",
      course: "Mathematics",
      chapterId: "y10-4",
    ...
    console.log('  [SET] y10-4g-q13_part1');

    // 3. Set y10-4g-q13_part2
    await db.collection('questions').doc('y10-4g-q13_part2').set({
      year: "Year 10",
      course: "Mathematics",
      chapterId: "y10-4",
      chapterTitle: "Chapter 4: Lines and linear equations",
    ...
    console.log('  [SET] y10-4g-q13_part2');

    console.log('\nFirestore synchronization completed successfully.');
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
