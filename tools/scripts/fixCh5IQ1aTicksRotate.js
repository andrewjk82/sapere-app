...
// Rotating the parallel ticks on slanted lines by 90 degrees so they cut perpendicular to the lines.
const updates = {
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Rotating parallel marks on: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully rotated parallel ticks on Q1a!');
  process.exit(0);
}
