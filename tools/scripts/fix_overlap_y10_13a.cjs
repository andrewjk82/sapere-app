...
function elementsToSvg(elements) {
  ...
  elements.filter(e => e.type === 'point' && e.on).forEach(e => {
    ...
    pts[e.id] = {
      ...
      sx: ox + mx * scale,
      sy: oy - my * scale,
      angleFromCenter: rad // keep math angle
    };
  });
  ...
  elements.filter(e => e.type === 'angle').forEach(e => {
    ...
    let y1 = p2.sy - r * Math.sin(a1); // -sin because SVG Y is down
    let x2 = p2.sx + r * Math.cos(a2);
    let y2 = p2.sy - r * Math.sin(a2);
    
    let largeArcFlag = diff > Math.PI ? 1 : 0;
    // In SVG, sweepFlag=0 means counter-clockwise rotation, which matches the positive mathematical direction
    let sweepFlag = 0; 
  ...
  elements.filter(e => e.type === 'rightangle').forEach(e => {
    ...
    if (a2 < a1) a2 += 2*Math.PI;
    let diff = a2 - a1;
    let sign = Math.sign(diff); // Always 1
    ...
    let p3x = p2.sx + r * Math.cos(a1 + Math.PI/2);
    let p3y = p2.sy - r * Math.sin(a1 + Math.PI/2);
    
    let px = p2.sx + r * Math.cos(a1) + r * Math.cos(a1 + Math.PI/2);
    let py = p2.sy - r * Math.sin(a1) - r * Math.sin(a1 + Math.PI/2);
  ...
  Object.values(pts).forEach(p => {
    ...
    if (p.name) {
      ...
      if (p.angleFromCenter !== undefined) {
        ...
        ty = p.sy - dist * Math.sin(p.angleFromCenter); // -sin for SVG Y
        
        if (Math.cos(p.angleFromCenter) > 0.4) anchor = 'start';
        else if (Math.cos(p.angleFromCenter) < -0.4) anchor = 'end';
        
        if (Math.sin(p.angleFromCenter) < -0.4) ty += 12; // math y < 0 -> svg y > 0
        else if (Math.sin(p.angleFromCenter) > 0.4) ty -= 4; // math y > 0 -> svg y < 0
        else ty += 4; 
      } else if (p.isCenter) {
        // Safe offset for O
        tx = p.sx - 8;
...
async function run() {
  ...
  for (const id of modifiedIds) {
    ...
    if (doc.exists) {
      ...
      console.log('Fixed overlapping SVG for', id);
    }
  }
