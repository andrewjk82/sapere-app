...
import { Y10_CH2D_QUESTIONS } from '../../src/constants/seedYear10Ch2DQuestions.js';
...
async function run() {
  console.log(`Loading Y10 Ch2D questions from local seed. Total: ${Y10_CH2D_QUESTIONS.length}`);
  
  const batch = db.batch();
  
  // 1. Delete all current y10-2d questions from DB to ensure no leftovers
  const snap = await db.collection('questions')
    .where('topicId', '==', 'y10-2d')
    .get();
  
  console.log(`Deleting ${snap.size} current y10-2d questions from Firestore...`);
  snap.forEach(doc => {
  ...
  console.log(`\n🎉 Successfully uploaded ${Y10_CH2D_QUESTIONS.length} y10-2d questions to Firestore!`);
