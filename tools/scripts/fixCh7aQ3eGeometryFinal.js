...
const geo = {
  points: { A:[-2,1], B:[2,1], C:[-2,-1], D:[2,-1], P:[-1,-2], Q:[1,2], I1:[0.5,1], I2:[-0.5,-1] },
  segments: [
    { from:'A', to:'B', marks:1 }, // Removed arrow:true
    { from:'C', to:'D', marks:1 }, // Removed arrow:true
    { from:'P', to:'Q', arrow:true },
    { from:'Q', to:'P', arrow:true }
  ],
  // 3θ is in Sector 0 of I1 (Bottom-Right, interior)
  // θ should be in Sector 3 of I2 (Top-Right, interior) to be co-interior!
  angles: [
    { at:'I1', label:'3θ' },
    { at:'I2' }, // Sector 0 (empty)
    { at:'I2' }, // Sector 1 (empty)
    { at:'I2' }, // Sector 2 (empty)
    { at:'I2', label:'θ' } // Sector 3 (Top-Right, interior)
  ]
};

async function run() {
  const docRef = db.collection('questions').doc('y9-7a-q3e');
  ...
  console.log('Successfully updated geometry for y9-7a-q3e to make angles co-interior!');
}
