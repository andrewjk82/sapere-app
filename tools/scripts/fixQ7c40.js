  ...
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    if (q.id === 'y9-7a-q7c') {
      geo.angles = [
        ...
        { at: 'C', right: true },
        // Pad D to place 40° in Sector 1 (between D->Q (UP) and D->E (UP-RIGHT))
        { at: 'D' }, { at: 'D', label: '40°' }
      ];
    }
...
async function run() {
  ...
  for (const id of targetIds) {
    ...
    console.log(`Successfully fixed 40-degree angle rendering sector in ${id}!`);
  }
