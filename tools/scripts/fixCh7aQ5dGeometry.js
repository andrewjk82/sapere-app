...
const geo = {
  ...
  angles: [
    { at:'S', label:'α' },
    { at:'T', label:'β' },
    { at:'R', label:'50°' }, // Sector 0 (RUT interior)
    { at:'R', label:'30°' }, // Sector 1 (RSU interior)
    { at:'U', label:'70°' }, // Sector 0 (RSU interior)
    { at:'U', label:'60°' }  // Sector 1 (RUT interior)
  ]
};

async function run() {
  const ids = ['y9-7a-q5d-alpha', 'y9-7a-q5d-beta'];
  ...
  for (const id of ids) {
    ...
    // Also fix the explanation RST typo to RUT (or URT)
    let steps = data.solutionSteps || [];
    steps = steps.map(step => {
      if (step.explanation && step.explanation.includes('RST')) {
        step.explanation = step.explanation.replace('RST', 'RUT');
      }
      return step;
    });
    
    await docRef.update({ question, solutionSteps: steps });
    console.log(`Successfully fixed complete geometry labels for ${id}!`);
  }
}
