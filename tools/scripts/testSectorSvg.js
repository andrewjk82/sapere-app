...
function geometryToSectorSvg(geo) {
  ...
  // Draw angle arcs and labels per vertex
  for (const vKey in anglesByVertex) {
    ...
    angList.forEach((angItem, idx) => {
      ...
      if (angItem.right) {
        // Draw right angle square symbol
        const sqSize = 12;
...
// Test on y9-7a-q2a and write to scratch SVG to inspect
const testGeo = seedMap['y9-7a-q2a'];
if (testGeo) {
  const resultSvg = geometryToSectorSvg(testGeo);
