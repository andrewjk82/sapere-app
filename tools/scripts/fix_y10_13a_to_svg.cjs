...
function elementsToSvg(elements) {
  ...
  elements.filter(e => e.type === 'angle').forEach(e => {
    ...
    let r = e.radius ? e.radius * scale : 20;
  ...
  elements.filter(e => e.type === 'tick').forEach(e => {
    ...
    if (e.count === 1) {
      ...
      svg += `  <line x1='${t1x}' y1='${t1y}' x2='${t2x}' y2='${t2y}' stroke='#1e293b' stroke-width='2' />\\n`;
    } else {
      for(let i=-1; i<=1; i+=2) {
  ...
  Object.values(pts).forEach(p => {
    if (p.name !== 'O') {
      ...
      svg += `  <circle cx='${p.sx}' cy='${p.sy}' r='3' fill='#64748b' />\\n`;
    }
    if (p.name) {
      let tx = p.sx + 8;
      let ty = p.sy - 8;
...
async function run() {
  ...
  for (const id of modifiedIds) {
    ...
    if (doc.exists) {
      ...
      console.log('Fixed SVG rendering for', id);
    }
  }
