...
// y8-5h-q3d:
// Correcting D (130deg) endpoint to (265, 129) and using sweep-flag 0 to curve inwards.
const updates = {
  ...
  <!-- B: 130deg (curves inwards with sweep-flag 1) -->
  ...
  <!-- D: 130deg (endpoint corrected to 265, 129, curves inwards with sweep-flag 0) -->
  <path d="M 248 140 A 12 12 0 0 0 265 129" fill="none" stroke="#6366f1" stroke-width="1.8"/>
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Correcting D endpoint coordinates for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully corrected D endpoint and sweep-flag!');
  process.exit(0);
}
