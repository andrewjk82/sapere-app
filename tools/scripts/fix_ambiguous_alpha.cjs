...
async function run() {
  ...
  const P_FONT = 'font-family="serif" font-weight="bold" font-style="italic" font-size="12" fill="#000"';
  const V_FONT = 'font-family="serif" font-size="12" fill="#000"';
  const SW = '1';
  const SW_ARC = '0.8';
  const R = '1.2';
  ...
  <!-- Add arc to clearly define alpha as the entire Angle DGC (55 degrees) -->
  <path d="M 75 150 A 15 15 0 0 0 68.5 138" fill="none" stroke="#000" stroke-width="${SW_ARC}"/>
  <text x="78" y="142" ${P_FONT}>α</text>
</svg>`;

  const SVG_Q2F_BETA = SVG_Q2F_BASE + `
  <path d="M 45 150 A 15 15 0 0 1 51.5 138" fill="none" stroke="#000" stroke-width="${SW_ARC}"/>
  ...
  console.log('Finished updating q2f ambiguous angles');
  process.exit(0);
}
