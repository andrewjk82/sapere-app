...
const filePath = '/Users/andrewkim/Desktop/sapere1/src/constants/seedYear11Ch5Questions.js';
let content = fs.readFileSync(filePath, 'utf8');
const match = content.match(/export const CH5_QUESTIONS_Y11A = (\[[\s\S]*?\]);\n\nexport const /m);
let questionsStr = match ? match[1] : content.match(/export const CH5_QUESTIONS_Y11A = (\[[\s\S]*?\]);\n*(?:export|$)/m)[1];
let questions = eval(questionsStr);
...
board.create('point', [3 - Math.sqrt(7), 0], {name:'3-√7', size:3, color:'green', label:{offset:[-30,10]}});
board.create('point', [3 + Math.sqrt(7), 0], {name:'3+√7', size:3, color:'green'});
...
const targetIds = [
  'y11a-5i-q12b-sketch',
  'y11a-5i-q12c', 'y11a-5i-q12c-sketch',
  'y11a-5i-q12d', 'y11a-5i-q12d-sketch'
];

for (let id of targetIds) {
  let q = questions.find(x => x.id === id);
  if (q) {
    let letter = id.charAt(11); // 'b', 'c', or 'd'
    let upd = configs[letter];
    
    q.graphData = null; // delete from question
    q.type = 'teacher_review';
    delete q.opts;
    delete q.a;
    delete q.answer;
    
    q.solutionSteps = JSON.parse(JSON.stringify(upd.steps));
...
fs.writeFileSync(filePath, content.replace(questionsStr, newArrayStr), 'utf8');
...
async function updateDb() {
  ...
  console.log('Successfully updated sketch questions in CH5_QUESTIONS_Y11A and Firestore.');
  process.exit(0);
}
updateDb();
