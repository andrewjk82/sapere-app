...
// Main graph SVG update (move 10 cm label from x=174, y=38 to x=238, y=86)
...
if (questions[qIndex].solutionSteps && questions[qIndex].solutionSteps.length > 2) {
  questions[qIndex].solutionSteps[0].graphData = { svg: mainSvg }; // Default step 1 shows original shape
...
async function run() {
  ...
  console.log('Successfully updated 10 cm label positioning in y9-16a-q10g.');
  process.exit(0);
}
