...
// Recalculating C (beta) to have perfectly uniform radius 22 (start at 248, 50 and end at 281, 69) and curve inwards (sweep-flag 1).
const updates = {
  ...
  <!-- Angles -->
  <!-- D: 55deg (curves inwards with sweep-flag 0) -->
  <path d="M 296 140 A 24 24 0 0 0 308 119" fill="none" stroke="#6366f1" stroke-width="1.8"/>
  <text x="292" y="122" text-anchor="middle" fill="#6366f1" font-size="12" font-weight="900" font-family="'Outfit'">55°</text>
  
  <!-- C: beta (perfectly concentric at r=22, curves inwards with sweep-flag 1) -->
  <path d="M 248 50 A 22 22 0 0 1 281 69" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
  <text x="252" y="78" text-anchor="middle" fill="#f43f5e" font-size="15" font-weight="900" font-family="'Outfit'">β</text>
</svg>`
};
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Applying uniform radius C arc to: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully completed uniform C arc correction!');
  process.exit(0);
}
