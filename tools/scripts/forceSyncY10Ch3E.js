...
import { Y10_CH3E_QUESTIONS } from '../../src/constants/seedYear10Ch3EQuestions.js';
...
const mapQuestion = (raw) => {
  ...
  return {
    ...
    topicId: "y10-3e",
    topicCode: "3E",
    topicTitle: "Factorising a difference of two squares",
    year: "Year 10",
    isManual: true,
    origin: "seed",
    id: raw.id,
...
async function syncQuestions() {
  ...
  if (existing.size > Y10_CH3E_QUESTIONS.length) {
    console.error(`⚠️ Safety warning: Existing question count (${existing.size}) is greater than new count (${Y10_CH3E_QUESTIONS.length}). Aborting.`);
    process.exit(1);
  }
  ...
  console.log(`Importing ${Y10_CH3E_QUESTIONS.length} questions for ${topicId}...`);
  const BATCH_SIZE = 400;
  for (let i = 0; i < Y10_CH3E_QUESTIONS.length; i += BATCH_SIZE) {
    const batch = db.batch();
    const chunk = Y10_CH3E_QUESTIONS.slice(i, i + BATCH_SIZE);
  ...
  console.log('Done! All Y10 Ch3E questions synced.');
  process.exit(0);
}
