...
function geometryToSvg(geo) {
  ...
  for(const vKey in abv){
    ...
    labelItems.forEach((angItem,idx)=>{
      ...
      const rArc=22, rLabel=36; // Slightly tighter arcs for cleaner appearance in multi-line grids
...
const widerGeo = {
  ...
  angles: [
    { at:'I1_L', label:'β' }, // Sector 0 (Bottom-Left)
...
async function run() {
  ...
  for (const id of ids) {
    ...
    const svg = geometryToSvg(widerGeo);
    ...
    console.log(`Successfully widened layout for ${id}!`);
  }
}
