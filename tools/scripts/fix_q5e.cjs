...
const qIndex = questions.findIndex(x => x.id === 'y10-13a-q5e');
let q = questions[qIndex];

// Fix overlapping arcs by assigning vastly different radii
let angle160 = q.graphData.jsxGraph.elements.find(e => e.type === 'angle' && e.name === '160°');
if (angle160) angle160.radius = 2.0;

let angleAlpha = q.graphData.jsxGraph.elements.find(e => e.type === 'angle' && e.name === 'α');
if (angleAlpha) angleAlpha.radius = 0.7;
    ...
    if (e.name) {
      ...
      for(let pt of placedTexts) {
        if (Math.abs(pt.tx - tx) < 15 && Math.abs(pt.ty - ty) < 15) {
          // Extra push for q5e specifically
          tr += 15;
...
async function save() {
  ...
  console.log('Successfully adjusted q5e for better readability!');
  process.exit(0);
}
