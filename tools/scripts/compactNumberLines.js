...
const makeCompactJSXGraphScript = (minX, maxX, targets) => {
  return {
    jsxGraph: {
      width: 500,
      height: 60, // Reduced from 120 to 60 to eliminate vertical empty space
      boundingbox: [minX, 0.6, maxX, -0.6], // Squashed from [-2, 2] to [-0.6, 0.6]
      ...
      script: `
        ...
        for (var x = ${Math.ceil(minX) + 1}; x <= ${Math.floor(maxX) - 1}; x++) {
          board.create('segment', [[x, -0.1], [x, 0.1]], {strokeColor: '#94a3b8', strokeWidth: 1.2, fixed: true});
          board.create('text', [x, -0.22, x.toString()], {
            anchorX: 'middle',
            anchorY: 'top',
            fontSize: 9.5,
  ...
  "y8-4a-q2a": {
    ...
    solutionSteps: [
      ...
      },
      {
        ...
        graphData: makeCompactJSXGraphScript(-13, 8, [-10, -5, 0, 5])
      }
    ]
  },
  "y8-4a-q2b": {
    ...
    solutionSteps: [
      ...
      },
      {
        ...
        graphData: makeCompactJSXGraphScript(-10, 9, [-7, -2, 3, 6])
      }
    ]
  },
  "y8-4a-q2c": {
    ...
    solutionSteps: [
      ...
      },
      {
        ...
        graphData: makeCompactJSXGraphScript(-6, 6, [-3, -2, -1, 0, 1, 2, 3])
      }
    ]
  },
  "y8-4a-q2d": {
    ...
    solutionSteps: [
      ...
      },
      {
        ...
        graphData: makeCompactJSXGraphScript(-8, 4, [-5, -4, -3, -2, -1, 0, 1])
      }
    ]
  }
};
...
(async () => {
  try {
    console.log('[JSXGraph Compact Lines] Resizing bounds and eliminating blank spaces...');
    for (const [id, update] of Object.entries(updates)) {
      ...
      if (doc.exists) {
        ...
        console.log(`  [RESIZED & SQUASHED] ${id}`);
      }
    }
    console.log('\nAll number lines have been successfully compacted.');
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
