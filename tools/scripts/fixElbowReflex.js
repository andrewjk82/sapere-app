...
function geometryToSectorSvg(geo, id) {
  ...
  for (const vKey in anglesByVertex) {
    ...
    // NEW HOTFIX: Auto-interior mapping for elbow joints
    if (sectors.length === 2 && labelItems.length === 1) {
      if (sectors[0].diff > Math.PI) {
        // Sector 0 is a massive reflex angle! Push the label to Sector 1 (interior)
...
async function run() {
  ...
  console.log(`Prepared ${count} questions to fix reflex angle mapping at elbow joints.`);
  if (count > 0) {
    await batch.commit();
