...
async function run() {
  ...
  console.log('List from index 45 (46th question) to 60 (61st question):');
  const slice = list.slice(45, 60);
  slice.forEach((item, index) => {
    console.log(`${index + 46}. ID: ${item.id} | Type: ${item.type} | Q: ${item.question} | Ans: ${item.answer}`);
  });
  process.exit(0);
}
