...
// y8-5h-q2e_beta:
// Changing B's sweep-flag to 1 so it curves inwards correctly.
const updates = {
  ...
  <!-- Angles -->
  <!-- B: beta (sweep-flag 1 curves inward) -->
  <path d="M 232 130 A 18 18 0 0 1 237 117" fill="none" stroke="#f43f5e" stroke-width="1.8"/>
...
async function run() {
  ...
  for (const [id, html] of Object.entries(updates)) {
    console.log(`Updating triangle B sweep flag to 1 for: ${id}`);
  ...
  await batch.commit();
  console.log('Successfully refined triangle B sweep flag on Firestore!');
  process.exit(0);
}
