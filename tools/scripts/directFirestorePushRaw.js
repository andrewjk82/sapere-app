...
// String.raw preserves every single backslash exactly as typed!
// We only need 2 backslashes in String.raw to output exactly 2 backslashes in memory, which yields 2 backslashes in Firestore!
const part1Sub = [
  {
    id: "y10-4g-q13a_i_a",
    ...
    solutionSteps: [
      {
        ...
        workingOut: String.raw`(t, \; d) = (2.5, \; 180)`,
        graphData: null
      },
      {
        explanation: "Read the distance value from the coordinates.",
        workingOut: String.raw`d_{\text{finish}} = 180`,
        graphData: null
      }
    ]
  },
    ...
    solutionSteps: [
      {
        ...
        workingOut: String.raw`t_{\text{taken}} = t_{\text{finish}} - t_{\text{start}}`,
        graphData: null
      },
      {
        explanation: "Substitute the values: t = 2.5 at finish, and t = 0 at start.",
        workingOut: String.raw`t_{\text{taken}} = 2.5 - 0 = 2.5`,
        graphData: null
      }
    ]
  },
    ...
    solutionSteps: [
      {
        ...
        workingOut: String.raw`\text{Distance} = 180 - 30 = 150`,
        graphData: null
      },
      {
        explanation: "The average speed is the total distance divided by the time taken.",
        workingOut: String.raw`\text{Speed} = \dfrac{\text{Distance}}{\text{Time}} = \dfrac{150}{2.5}`,
        graphData: null
      },
      {
        explanation: "Evaluate the fraction.",
        workingOut: String.raw`\text{Speed} = 60`,
        graphData: null
      }
    ]
  },
...
(async () => {
  try {
    console.log('[Direct Firestore String.raw Push] Enforcing absolute raw backslash strings...');
    
    await db.collection('questions').doc('y10-4g-q13_part1').update({
      subQuestions: part1Sub
    });
    console.log('  [PART 1 RAW PUSH SUCCESS]');

    await db.collection('questions').doc('y10-4g-q13_part2').update({
      subQuestions: part2Sub
    });
    console.log('  [PART 2 RAW PUSH SUCCESS]');

    console.log('\nFirestore Database has been successfully overwritten with clean, un-stripped LaTeX values.');
    process.exit(0);
  } catch (err) {
    console.error('Fatal Raw Push Error:', err);
    process.exit(1);
  }
})();
