...
seedData.forEach(q => {
  if (q.id && q.graphData && q.graphData.geometry) {
    ...
    if (q.id === 'y9-7a-q2d') {
      ...
      const a = geo.angles;
      geo.angles = [{ at: 'N' }, { at: 'N' }, a[0], a[1], a[2]];
    }
    if (q.id === 'y9-7a-q2e') {
      ...
      const a = geo.angles;
      geo.angles = [a[0], { at: 'N' }, a[1], { at: 'N' }, a[2], { at: 'N' }];
    }
    if (q.id === 'y9-7a-q2f') {
      geo.points.P = [0, 2];
      ...
      geo.points.S = [2, 0];
      const a = geo.angles;
      geo.angles = [{ at: 'O' }, a[1], a[2], a[3]];
    }
    
    // HOTFIX FOR Q5C EXTERIOR ANGLE
    if (q.id === 'y9-7a-q5c') {
      const a = geo.angles;
      // Pad sector 0 (interior angle) with an empty label so theta falls on sector 1 (exterior angle)
...
function geometryToSectorSvg(geo, id) {
  ...
  for (const vKey in anglesByVertex) {
    ...
    if (sectors.length === 2 && labelItems.length === 1) {
...
async function run() {
  const docRef = db.collection('questions').doc('y9-7a-q5c');
  const data = (await docRef.get()).data();
  if (!data) return;
  
  const geo = seedMap['y9-7a-q5c'];
  const beautifulSvg = geometryToSectorSvg(geo, 'y9-7a-q5c');
  let question = data.question;
  question = question.replace(/<svg[\s\S]*?<\/svg>\s*/g, '');
  question = `${beautifulSvg}\n\n${question}`;
  
  await docRef.update({ question });
  console.log(`Successfully fixed exterior angle rendering for y9-7a-q5c!`);
}
