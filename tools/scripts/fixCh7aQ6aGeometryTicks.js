...
function geometryToSvg(geo) {
  ...
  if(geo.segments) for(const seg of geo.segments){
    ...
    if(seg.tick){
      // Fixed rotation logic: Rotating a vertical line (90 deg) by segment angle makes it perpendicular to the segment (90 + angle).
...
async function run() {
  ...
  for (const id of ids) {
    ...
    console.log(`Successfully fixed tick marks for ${id}!`);
  }
}
