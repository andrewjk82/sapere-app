...
import { Y10_CH2E_QUESTIONS } from '../../src/constants/seedYear10Ch2EQuestions.js';
...
async function run() {
  console.log(`Loading Y10 Ch2E questions from local seed. Total: ${Y10_CH2E_QUESTIONS.length}`);
  
  const batch = db.batch();
  
  // 1. Delete all current y10-2e questions from DB to ensure no leftovers
  const snap = await db.collection('questions')
    .where('topicId', '==', 'y10-2e')
    .get();
  
  console.log(`Deleting ${snap.size} current y10-2e questions from Firestore...`);
  snap.forEach(doc => {
  ...
  Y10_CH2E_QUESTIONS.forEach(q => {
    ...
    const docData = {
      ...
      t: q.t || "Rationalising denominators",
      createdAt: new Date(),
      updatedAt: new Date()
    };
  ...
  console.log(`\n🎉 Successfully uploaded ${Y10_CH2E_QUESTIONS.length} y10-2e questions to Firestore!`);
