...
b) What is the width of the house (in metres)?`;
  ...
  const solutionSteps = [
    ...
    {
      explanation: 'Find the width of the house on the plan. The total width is 7 cm.',
      workingOut: '\\(\\text{Width on plan} = 7\\text{ cm}\\)',
      graphData: null
    },
    {
      explanation: 'Multiply the plan width by the scale factor to get the real-life width.',
      workingOut: [
        '\\(\\text{Real width} = 7 \\times 3\\)',
        '\\(\\text{Real width} = 21\\text{ m}\\)'
      ].join('\n'),
      graphData: null
    }
  ];

  // Answer: 21 m → D (index 3), shuffled
  await db.collection('questions').doc('y8-15a-q10b').update({
    type: 'multiple_choice',
    question: question,
    options: [
      { text: '\\(20\\text{ m}\\)', imageUrl: '' },
      { text: '\\(22\\text{ m}\\)', imageUrl: '' },
      { text: '\\(7\\text{ m}\\)', imageUrl: '' },
      { text: '\\(21\\text{ m}\\)', imageUrl: '' }  // Correct → D
    ],
    answer: '3',
  ...
  console.log("Updated y8-15a-q10b");
  process.exit(0);
}
