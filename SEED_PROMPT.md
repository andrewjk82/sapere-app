...
## ✏️ 설정값 (여기만 수정)
학년: Year 5 | 챕터 번호: 1 | chapterId: y5-1
서브토픽 ID: y5-1 토픽 코드: 1J | 서브토픽 제목: Revision

난이도: Q1~Q6 easy / Q7~Q13 medium / Q14~ hard (명백히 다르면 조정)
...
- a: 채점용 정답만(short/MC 필수; solution과 절대 혼용 금지)
  ★MC 전용 절대규칙: a는 반드시 opts 배열의 zero-based 숫자 인덱스(0·1·2·3)여야 함.
  "Option A", "A", "x=5" 등 문자열 절대 금지. opts[a]가 정답 텍스트와 일치하는지 반드시 검증 후 작성.
  예) opts:["2","4","6","8"], 정답이 6이면 → a:2 (opts[2]==="6")
...
### [규칙 6] 서술형 및 증명형 문제(`teacher_review`)의 줄바꿈 규칙
   ...
   ### [규칙 7] multiple_choice answer 인덱스 무결성 (⚠️ 최우선 규칙)
   - MC 문항의 `a` 필드는 **반드시 정수(0, 1, 2, 3)** 이어야 합니다. 문자열 불가.
   - 작성 순서:
     1. opts 배열을 먼저 확정합니다.
     2. 정답이 몇 번째(0-based)인지 확인합니다.
     3. 그 인덱스를 `a`에 기입합니다.
     4. ★자가검증: opts[a] === 정답 텍스트 인지 눈으로 확인 후 저장.
   - 위반 예시 (절대 금지):
     * a: "Option A"  → ❌
     * a: "6"         → ❌ (문자열)
     * a: "x = 5"    → ❌
   - 올바른 예시:
     * opts: ["\\(x=2\\)", "\\(x=3\\)", "\\(x=5\\)", "\\(x=7\\)"], a: 2  → ✅ (opts[2]==="\\(x=5\\)")
